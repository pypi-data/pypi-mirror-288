...
class WiFiManager:
    ...
    def forget_network(self, ssid):
        """
        Forgets a WiFi network with the given SSID.

        Args:
            ssid (str): The SSID of the network to forget.
        
        Returns:
            bool: True if the network is forgotten successfully, False otherwise.
        """
        try:
            result = subprocess.run(
            ...
            if result.returncode == 0:
                print(f"Forgotten network: {ssid}")
                return True
            else:
                print("Error forgetting network. Check SSID.")
                return False
        except subprocess.CalledProcessError as e:
            print(f"Error forgetting network: {e}")
            return False
        except Exception as e:
            print(f"Unexpected error forgetting network: {e}")
            return False
