...
class LeadGenerationCompanyFilters(BaseModel):
    ...
    def to_mongo_filters(
        ...
        if self.declared_best_nace_codes_regex:
            nace_section_codes = []
            naces_codes_regex = []
            for code_regex in self.declared_best_nace_codes_regex:
            ...
            if naces_codes_regex:
                filters["$or"].append({"nace_codes": {"$elemMatch": {"number": {"$regex": naces_codes_regex}}}})
                filters["$or"].append(
                    {
                        "establishments": {
                            "$elemMatch": {"nace_codes": {"$elemMatch": {"number": {"$regex": naces_codes_regex}}}}
                        }
                    },
                )
        ...
        if self.inoopa_best_nace_codes_regex:
            if len(self.inoopa_best_nace_codes_regex) == 1:
                filters["best_nace_codes.first_best_nace_code.section_code"] = {
                    "$in": self.inoopa_best_nace_codes_regex
                }
            else:
                filters["best_nace_codes.first_best_nace_code.number"] = {"$regex": self.inoopa_best_nace_codes_regex}
