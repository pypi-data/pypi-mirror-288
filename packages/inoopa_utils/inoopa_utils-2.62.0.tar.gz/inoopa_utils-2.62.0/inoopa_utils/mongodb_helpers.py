...
from inoopa_utils.custom_types.delivery_filters import LeadGenerationCompanyFilters, MongoFilters
...
def create_filters_lead_generation(
    fitlers: LeadGenerationCompanyFilters,
    ...
    """
    filters = {
        "country": {"$in": fitlers.countries},
        "legal_form": {"$in": fitlers.legal_forms},
        "employee_category_formatted": {"$in": fitlers.employee_categories},
        "address.region": {"$in": fitlers.regions},
        "number_of_establishments": {
            "$gte": fitlers.minimum_number_of_estanlishments,
            "$lte": fitlers.maximum_number_of_estanlishments,
        },
    }
    if fitlers.declared_best_nace_codes:
        filters["$or"] = [
            {"nace_codes": {"$elemMatch": {"number": {"$in": fitlers.declared_best_nace_codes}}}},
            {
                "establishments": {
                    "$elemMatch": {"nace_codes": {"$elemMatch": {"number": {"$in": fitlers.declared_best_nace_codes}}}}
                }
            },
        ]
    if fitlers.inoopa_best_nace_codes:
        filters["best_nace_codes.first_best_nace_code.number"] = {"$in": fitlers.inoopa_best_nace_codes}

    if fitlers.legal_form_types:
        filters["legal_form_type"] = {"$in": fitlers.legal_form_types}
    if fitlers.created_before and not fitlers.created_after:
        filters["start_date"] = {"$lte": fitlers.created_before}
    if fitlers.created_after and not fitlers.created_before:
        filters["start_date"] = {"$gte": fitlers.created_after}
    if fitlers.created_before and fitlers.created_after:
        filters["start_date"] = {"$lte": fitlers.created_before, "$gte": fitlers.created_after}
    if fitlers.zipcodes:
        filters["address.postal_code"] = {"$in": fitlers.zipcodes}
    if fitlers.excluded_companies:
        filters["_id"] = {"$nin": fitlers.excluded_companies}
