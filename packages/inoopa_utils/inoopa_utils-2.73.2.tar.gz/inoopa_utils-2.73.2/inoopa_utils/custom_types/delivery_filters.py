...
class LeadGenerationCompanyFilters(BaseModel):
    ...
    def to_mongo_filters(
        ...
        if self.inoopa_best_nace_codes_regex:
            nace_codes = []
            nace_section_codes = []
            for code_regex in self.inoopa_best_nace_codes_regex:
                # if the regex is only one letter, it's a nace section code
                if len(code_regex) == 1:
                    nace_section_codes.append(code_regex)
                else:
            ...
            if nace_codes:
                filters["best_nace_codes.first_best_nace_code.number"] = {"$regex": nace_codes}
            if nace_section_codes:
                filters["best_nace_codes.first_best_nace_code.section_code"] = {"$in": nace_section_codes}
