...
class STMSinkEnum(enum.Enum):
    """Enumeration of different TRACE sinks."""
...
class TraceConfig(ExtractedConfig):
    ...
    stmsink: STMSinkEnum = STMSinkEnum.NONE
    portconfig: int = 0

    def to_record(self, **kwargs) -> Record:
        record = Record()
        if self.stmsink == STMSinkEnum.ETR:
            record["trace_etrsink_sources_0"] = {"STMMAINCORE": 0}
        elif self.stmsink == STMSinkEnum.ETB:
            record["trace_etbsink_sources_0"] = {"STMMAINCORE": 0}
        elif self.stmsink == STMSinkEnum.TPIU:
            record["trace_tpiusink_sources_0"] = {"STMMAINCORE": 0}
...
def extract_trace_configs(
    ...
    for node in nodes:
        prop_stmsink = node.properties.get_val("stmsink", "none")
        prop_portconfig = node.properties.get_val("portconfig", 0)
        if prop_stmsink == "etr":
            trace_config = TraceConfig(stmsink=STMSinkEnum.ETR)
            trace_configs.append(trace_config)
            _append_log_config(ConfigTypeEnum.TRACE, trace_config)
        elif prop_stmsink == "etb":
            trace_config = TraceConfig(stmsink=STMSinkEnum.ETB)
            trace_configs.append(trace_config)
            _append_log_config(ConfigTypeEnum.TRACE, trace_config)
        elif prop_stmsink == "tpiu":
            trace_config = TraceConfig(
                stmsink=STMSinkEnum.TPIU, portconfig=prop_portconfig
            )
            trace_configs.append(trace_config)
            _append_log_config(ConfigTypeEnum.TRACE, trace_config)
...
def _log_trace_configs(trace_log_info):
    ...
    log_vrb(f"Trace Sink {trace_config.stmsink.name}")
    if trace_config.stmsink == STMSinkEnum.TPIU:
        log_vrb(f"Portconfig 0x{trace_config.portconfig:08x}")
