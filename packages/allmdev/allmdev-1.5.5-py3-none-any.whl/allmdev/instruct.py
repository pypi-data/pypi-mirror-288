...
def load_model(model):
            ...
            if model_files:
                    ...
                    elif model=='Mistral_instruct' or model=='mistral_instruct' or model=='mistral-instruct' or model=='Mistral-instruct':
                        model_id = "TheBloke/Mistral-7B-Instruct-v0.2-GGUF"
                        filename = hf_hub_download(repo_id=model_id, filename="mistral-7b-instruct-v0.2.Q8_0.gguf")
                        shutil.move(filename,model_dir)
                        model_path = os.path.join(model_dir, filename)
...
def infer(model, temperature=0.5, max_new_tokens=512, model_kwargs={"n_gpu_layers":0}):
        

        model_files = [f for f in os.listdir(model_dir) if f.endswith('.gguf')]
        model_path = load_model(model_files[0]) if model_files else load_model(model)
        print(model_path)
