...
from .connect_device import READABILITY, WRITABILITY, ConnectDeviceProfile, ConnectMainDevice, ConnectSubDevice


class OvenProfile(ConnectDeviceProfile):
    """Oven Profile."""

    _RESOURCE_MAP = {"info": "info"}

    _PROFILE = {"info": {"type": "oven_type"}}
    ...
    def __init__(self, profile: dict[str, Any]):
        ...
        self._PROFILE = OvenSubProfile._PROFILE


class OvenSubProfile(ConnectDeviceProfile):
    """Oven Profile Sub."""
    ...
    _PROFILE = {
        ...
        "operation": {"ovenOperationMode": "oven_operation_mode"},
        "cook": {"cookMode": "cook_mode"},
        "remoteControlEnable": {"remoteControlEnabled": "remote_control_enabled"},
        "temperature": {
            "C": "target_temperature_c",
            "F": "target_temperature_f",
        },
        "timer": {
            ...
            "timerHour": "timer_hour",
            "timerMinute": "timer_minute",
...
class OvenSubDevice(ConnectSubDevice):
    ...
    async def set_oven_operation_mode(self, mode: str) -> ThinQApiResponse:
    ...
    async def set_cook_mode(self, mode: str) -> ThinQApiResponse:
        payload = self.profiles.get_enum_attribute_payload("cook_mode", mode)
        return await self._do_attribute_command({"location": {"locationName": self._location_name}, **payload})

    async def _set_target_temperature(self, target_temperature: int, unit: str) -> ThinQApiResponse:
        temperature_map = self.profiles._PROFILE["temperature"]
        payload = self.profiles.get_range_attribute_payload(temperature_map.get(unit), target_temperature)
        return await self._do_attribute_command({"location": {"locationName": self._location_name}, **payload})

    async def set_target_temperature_f(self, target_temperature: int) -> ThinQApiResponse:
        return await self._set_target_temperature(target_temperature, "F")

    async def set_target_temperature_c(self, target_temperature: str) -> ThinQApiResponse:
...
class OvenDevice(ConnectMainDevice):
    ...
    def get_sub_device(self, location_name: str) -> OvenSubDevice:
