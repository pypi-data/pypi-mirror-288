...
class ThinQMQTTClient:
    ...
    async def async_init(self):
        ...
        self._mqtt_server = route_response.body.get("mqttServer").replace("mqtts://", "").split(":", maxsplit=1)[0]
    ...
    async def _on_disconnect(self) -> None:
        ...
        result: ThinQApiResponse = await self._thinq_api.async_delete_client_register(payload=CLIENT_BODY)
        _LOGGER.debug("Delete client register, result: %s", result.body)
    ...
    async def issue_certificate(self) -> bool:
        ...
        response: ThinQApiResponse = await self._thinq_api.async_post_client_certificate(body)
        if response.body is None:
            return False

        _LOGGER.debug("Request client certificate, result: %s", response.body)
        certificate_pem: str = response.body.get("result").get("certificatePem")
        subscriptions: list[str] = response.body.get("result").get("subscriptions")
