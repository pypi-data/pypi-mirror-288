...
TEST_ACCESS_TOKEN = "23a0d011b8c01db02e9a1eda0227290621f70f92d24dbdcfe190"
...
async def test_device_apis(thinq_api):
    ...
    assert response.status == 200, "Failed to get device list"
    device_list = response.body
    logger.info("device_list : %s", response.body)
    ...
    assert response.status == 200, "Failed to get device profile"
    ...
    logger.info("push list : %s", response.body)

    response = await thinq_api.async_post_push_subscribe(device_id)
    logger.info("push subscribe : %s", response.body)

    response = await thinq_api.async_delete_push_subscribe(device_id)
    logger.info("push unsubscribe : %s", response.body)

    response = await thinq_api.async_get_event_list()
    logger.info("event list : %s", response.body)

    response = await thinq_api.async_post_event_subscribe(device_id)
    logger.info("event subscribe : %s", response.body)

    response = await thinq_api.async_delete_event_subscribe(device_id)
    logger.info("event unsubscribe : %s", response.body)

    response = await thinq_api.async_post_push_devices_list()
    logger.info("push devices list : %s", response.body)

    response = await thinq_api.async_post_push_devices_subscribe()
    logger.info("push devices subscribe : %s", response.body)

    response = await thinq_api.async_post_push_devices_list()
    logger.info("push devices list : %s", response.body)

    response = await thinq_api.async_delete_push_devices_subscribe()
    logger.info("push devices unsubscribe : %s", response.body)
