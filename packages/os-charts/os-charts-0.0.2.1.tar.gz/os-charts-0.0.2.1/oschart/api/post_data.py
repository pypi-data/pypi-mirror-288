...
class OsPost:
    ...
    def __params_init_and_check(self) -> None:
        ...
        """
        if not self.page_id:
            raise ValueError(f"page_id 参数不能为空")
        if not self.platform:
            raise ValueError(f"platform 参数不能为空")
        ...
        if not self.post_id and not self.date_start and not self.date_end:
            raise ValueError("date_stat/date_end 与 post_id 参数不可同时为空")
        if self.platform not in platform_str_list():
            raise ValueError(f"{self.platform } not in {platform_str_list()}")
        if not isinstance(self.post_id, (str, list)):
            raise ValueError(f"{self.post_id} type must str or list")
        if isinstance(self.post_id, str):
            self.post_ids = [self.post_id]
        elif isinstance(self.post_id, list):
            self.post_ids = self.post_id
