...
class Component:
    ...
    def upload_target(self, scanned_image, temp_png_image):
        ...
        ----------------------------------------------------------------------
        returns
            selected_image : file-like object
        ----------------------------------------------------------------------
        """
        ...
        if extension in {'.tif', '.tiff'}:
            ...
            selected_image = temp_png_image
        else:
            selected_image = scanned_image
    ...
    def upload_image(self):
        ...
        with open(self.file, 'rb') as scanned_image_flo, tempfile.NamedTemporaryFile() as temp_png_image_flo:

            # convert to PNG if necessary
            scan_for_upload = self.upload_target(scanned_image_flo, temp_png_image_flo)
            self.report_upload_size(scan_for_upload)

            filename = f'{self.qrcode}-{self.side}-{self.timestamp_file}_TEST_ONLY.png'
            ...
            # images should be around 60MB in size, so no problems are
            # expected with memory usage. SHA256 is used instead of blake2b
            # simply because sha256sum is available at the far end from the
            # command line.
