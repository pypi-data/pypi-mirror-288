...
class comment_email():
    ...
    def gen_href(cls, comment, user):
        ...
        href = "mailto:{addr}?subject={subject}".format(
            addr=dd.plugins.inbox.comment_reply_addr,
            ...
            for key in m.iterkeys():
                try:
                    ...
                    if comment is None or user is None:
                        ...
                        m.flush()
                        logger.info(
                            "Failed to add comment-reply-email {subject} key:{key} added to "
                            .format(subject=message.get("subject"), key=key) +
                            fail._file.name)
                        continue
                    ...
                    logger.info("New comment {} via email key: {}".format(
                        new_comment.id, key))
                    processed.flush()
                    m.discard(key)
                    m.flush()
