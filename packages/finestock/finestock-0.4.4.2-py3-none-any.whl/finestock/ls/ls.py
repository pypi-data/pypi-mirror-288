...
class LS(API):
    ...
    def _parse_price(self, code, data):
        ...
        return finestock.Price(today, code, data['price'], int(data['open']), int(data['high']),
                               int(data['low']), int(data['price']), int(data['cvolume']), int(data['value']))

    def _parse_index(self, code, data):
        today = datetime.datetime.now().strftime('%Y%m%d')
        return finestock.Price(today, code, data['jisu'], float(data['openjisu']), float(data['highjisu']),
