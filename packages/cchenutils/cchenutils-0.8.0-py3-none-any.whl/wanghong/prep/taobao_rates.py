...
5. Adjust dates for default ratings
6. [Moved] Adjust dates for nondefault ratings by 0-15 (and random 0-15) days
7. [Moved] Adjust dates by 3-5 days and random 3-5 days for all ratings
...
if __name__ == '__main__':
    ...
    # # Adjust dates for nondefault ratings by 0-15 (and random 0-15) days
    # rdates = {}
    # for dr in trange(0, 16, desc='Adjusting dates for reviewing process'):  # days for reviewing
    ...
    #
    # # Adjust dates by 3-5 days and random 3-5 days for all ratings
    # dates = {}
    # for col in tqdm(rdates.keys()):
    #     for ds in trange(3, 6, desc=col):  # days for shipping
    #         dates[f'{col}_s{ds}'] = rdates[col].apply(lambda x: adj_date_by(x, -ds))
    #     seed(1)
    #     dates[f'{col}_sr35'] = rdates[col].apply(lambda x: adj_date_by(x, -randint(3, 5)))
    #     seed(1)
    #     dates[f'{col}_sr15'] = rdates[col].apply(lambda x: adj_date_by(x, -randint(1, 5)))
    #
    # df_rates = pd.concat([df_rates, pd.DataFrame(dates)], axis=1)
