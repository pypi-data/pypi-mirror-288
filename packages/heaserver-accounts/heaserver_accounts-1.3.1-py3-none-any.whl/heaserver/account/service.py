...
from heaobject.root import Permission, ShareImpl, ViewerPermissionContext
...
async def get_accounts(request: web.Request) -> web.Response:
    async with DesktopObjectActionLifecycle(request=request,
        ...
        if not account_ids and (cached_value:=request.app[HEA_CACHE].get(cache_key)) is not None:
            account_view_dicts, volume_id, permissions, attribute_permissions = cached_value
        else:
            try:
                aws_accounts, _, _, _ = await _aws_account_ids_to_aws_accounts(request, tuple(a.split('^')[1] for a in account_ids), calc_permissions=False)
            except IndexError:
            ...
            if not account_ids:
                request.app[HEA_CACHE][cache_key] = (account_view_dicts, volume_id, permissions, attribute_permissions)
...
async def _aws_account_ids_to_aws_accounts(request: web.Request, aws_account_ids: Sequence[str], calc_permissions=True) -> tuple[list[AWSAccount], list[str], list[Permission], list[dict[str, list[Permission]]]]:
    logger = logging.getLogger(__name__)
    sub = request.headers.get(SUB, NONE_USER)
