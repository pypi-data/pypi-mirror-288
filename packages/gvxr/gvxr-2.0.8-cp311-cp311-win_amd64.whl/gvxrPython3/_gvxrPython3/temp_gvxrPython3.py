...
def computeRMSE(*args):
    ...
    `aReferenceSet` $(\theta)$ and `aTestImage` $\left(\hat{\theta}\right)$.
    When RMSE is close to zero, then $\theta$ and $\hat{\theta}$ are similar.  

    \begin{eqnarray*}
       MSE(\theta, \hat{\theta}) &=& \frac{\sum_{i = 0}^{I-1}\sum_{j =
    0}^{J-1}\sum_{k = 0}^{K-1}\left(\theta(i,j,k) -
...
def addControlLineSegment(aLabel, aPosition1X, aPosition1Y, aPosition1Z, aPosition2X, aPosition2Y, aPosition2Z, aUnitOfLength, aDentityFunction, aParameter1, aParameter2=0.0):
    r"""
    ...
    * `aParameterB` :  
    ...
    """
    return _temp_gvxrPython3.addControlLineSegment(aLabel, aPosition1X, aPosition1Y, aPosition1Z, aPosition2X, aPosition2Y, aPosition2Z, aUnitOfLength, aDentityFunction, aParameter1, aParameter2)
...
def makeFoamInCuboid(aPorosityName, aBoundingVolumeName, aCuboidWidth, aCuboidHeight, aCuboidDepth, aPorosityRadius, aPercentageOfVolume, aPercentageDeviation, aVoxelSpacing, aUnitOfLength, aVerboseLevel=0):
    ...
    Create a foam structure, i.e. porosities into a bounding volume and set both
    their labels in the scenegraph (i.e. identifiers). Note that they are not added
...
def createOpenGLContext(aWindowID=-1, aRendererMajorVersion=3, aRendererMinorVersion=2):
    ...
    * `aRendererMajorVersion` :  
        Select the major version of the renderer. (default value: 3)  
    * `aRendererMajorVersion` :  
        Select the minor version of the renderer. (default value: 2)  
...
def terminate():
    ...
    Close and destroy all the windows and OpenGL contexts that have been created.
    Same as void destroyAllWindows(void);.  
