...
class TargetProxy():
    ...
    async def add_action(self, args: ActionArgs, handler: Callable[[ActionArgs, Dict], None]):
        action_proxy = ActionProxy(self, args, self.client, handler)
        await action_proxy.save()
        self.client.actions[action_proxy.id()] = action_proxy
        return action_proxy
