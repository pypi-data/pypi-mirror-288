...
class RAGEvaluator:
    ...
    def generate_report(self, question, response, reference):
        ...
        results = self.evaluate_all(question, response, reference)

        report = f"""
        ## RAG Model Evaluation Report
        ...
        **Reference Answer:** {reference}
        ...
        {tabulate(
            [
                ...
                ["BLEU", f"{results['BLEU']:.2f}", "Overlap of words/phrases (higher is better)"],
                ["ROUGE-1", f"{results['ROUGE-1']:.2f}", "Overlap of individual words (higher is better)"],
                ["BERT F1", f"{results['BERT F1']:.2f}", "Semantic similarity using BERT (higher is better)"],
                ["METEOR", f"{results['METEOR']:.2f}", "Considers synonyms/paraphrases (higher is better)"],
                ["CHRF", f"{results['CHRF']:.2f}", "Character-level overlap (higher is better)"],
                ["Perplexity", f"{results['Perplexity']:.2f}", "Fluency (lower is better)"],
                ["Diversity", f"{results['Diversity']:.2f}", "Variety of words/phrases"],
                ["Racial Bias", f"{results['Racial Bias']:.2f}", "Potential for bias (lower is better)"],
                ["Toxicity", f"{results['Toxicity']:.2f}", "Potential for toxicity (lower is better)"],
                ["Flesch Reading Ease", f"{results['Flesch Reading Ease']:.2f}", "Readability (higher is easier)"],
                ["Flesch-Kincaid Grade", f"{results['Flesch-Kincaid Grade']:.2f}", "U.S. grade level readability"],
                ["Semantic Similarity", f"{results['Semantic Similarity']:.2f}", "Overall meaning similarity (higher is better)"],
            ],
            headers="firstrow",
            tablefmt="grid"  # Use a grid format for better readability
        )}


        ### Other Considerations:

        * **Factual Consistency:** {'✅ Consistent' if results['Factual Consistency'] else '❌ Inconsistent'}
        * **Question Relevance:** {'✅ Relevant' if results['Question Relevance'] else '❌ Not Relevant'}
    ...
    def generate_html_report(self, question, response, reference, filename="rag_evaluation_report.html"):
        """Generates an HTML evaluation report and saves it to a file.

        Args:
            question (str): The input question.
            response (str): The generated response.
            reference (str): The reference answer.
            filename (str, optional): The name of the HTML file to save. 
                                    Defaults to "rag_evaluation_report.html".
        """
        ...
        <html>
        <head>
            <title>RAG Model Evaluation Report</title>
            <style>
        ...
        <body>
            <h1>RAG Model Evaluation Report</h1>
            ...
            <table>
                ...
                <tr>
                    <td>BLEU</td>
                    <td>{results['BLEU']:.2f}</td>
                    <td>Overlap of words/phrases (higher is better)</td>
                </tr>
                <tr>
                    <td>ROUGE-1</td>
                    <td>{results['ROUGE-1']:.2f}</td>
                    <td>Overlap of individual words (higher is better)</td>
                </tr>
                <tr>
                    <td>BERT F1</td>
                    <td>{results['BERT F1']:.2f}</td>
                    <td>Semantic similarity using BERT (higher is better)</td>
                </tr>
                <tr>
                    <td>METEOR</td>
                    <td>{results['METEOR']:.2f}</td>
                    <td>Considers synonyms/paraphrases (higher is better)</td>
                </tr>
                <tr>
                    <td>CHRF</td>
                    <td>{results['CHRF']:.2f}</td>
                    <td>Character-level overlap (higher is better)</td>
                </tr>
                <tr>
                    <td>Perplexity</td>
                    <td>{results['Perplexity']:.2f}</td>
                    <td>Fluency (lower is better)</td>
                </tr>
                <tr>
                    <td>Diversity</td>
                    <td>{results['Diversity']:.2f}</td>
                    <td>Variety of words/phrases</td>
                </tr>
                <tr>
                    <td>Racial Bias</td>
                    <td>{results['Racial Bias']:.2f}</td>
                    <td>Potential for bias (lower is better)</td>
                </tr>
                <tr>
                    <td>Toxicity</td>
                    <td>{results['Toxicity']:.2f}</td>
                    <td>Potential for toxicity (lower is better)</td>
                </tr>
                <tr>
                    <td>Flesch Reading Ease</td>
                    <td>{results['Flesch Reading Ease']:.2f}</td>
                    <td>Readability (higher is easier)</td>
                </tr>
                <tr>
                    <td>Flesch-Kincaid Grade</td>
                    <td>{results['Flesch-Kincaid Grade']:.2f}</td>
                    <td>U.S. grade level readability</td>
                </tr>
                <tr>
                    <td>Semantic Similarity</td>
                    <td>{results['Semantic Similarity']:.2f}</td>
                    <td>Overall meaning similarity (higher is better)</td>
                </tr>
                </tr>
                <tr>
                    <td>Toxicity</td>
                ...
                <tr> 
                    ...
                    <td>{'✅ Relevant' if results['Question Relevance'] else '❌ Not Relevant'}</td> 
                    <td>Whether the answer is relevant to the context.</td> 
                </tr> 
                <tr> 
                    <td>Answer Relevance</td> 
                    <td>{'✅ Relevant' if results['Factual Consistency'] else '❌ Not Relevant'}</td> 
                    <td>Whether the answer is relevant to the question.</td> 
                </tr> 
            </table>

            <h2>Other Considerations:</h2>
            <ul>
                <li><b>Factual Consistency:</b> {'✅ Consistent' if results['Factual Consistency'] else '❌ Inconsistent'}</li>
                <li><b>Question Relevance:</b> {'✅ Relevant' if results['Question Relevance'] else '❌ Not Relevant'}</li>
            </ul>
        </body>
        </html>
        """
