...
class HistoricalRecords:
    ...
    def __init__(
        ...
        self.external_connection = 'log_db'
    ...
    def get_history_model_name(self, model):
        if not self.custom_model_name:
            # Construir o nome da tabela de log com o formato desejado.
            app_label = model._meta.app_label
            model_name = model._meta.model_name
            return f"log_{app_label}_{model_name}"
    ...
    def create_history_model(self, model, inherited):
        ...
        attrs.update(fields)
        extra_fields = self.get_extra_fields(model, fields)
        
        # Garantir que history_id seja a primeira coluna
        history_id_field = self._get_history_id_field()
        attrs = {"history_id": history_id_field, **attrs}
        
        # Adicionar os campos extras depois, exceto history_id
        attrs.update({k: v for k, v in extra_fields.items() if k != "history_id"})

        # Set as the default then check for overrides
        name = self.get_history_model_name(model)

        # Ensure custom table name
        db_table_name = name

        attrs.update(Meta=type("Meta", (), self.get_meta_options(model)))
        attrs["Meta"].db_table = db_table_name
    ...
    def get_extra_fields(self, model, fields):
        ...
        # Renomear o campo de ID original para "nomedamodel_id"
        model_id_field_name = f"{model._meta.model_name}_id"

        fields[model_id_field_name] = self._get_history_id_field()
