...
def get_router(get_db, UPLOAD_FOLDER="data"):
    ...
    def download_file(uid: int, db: Session = Depends(get_db)):
        # 检查文件是否存在
        db_file = db.exec(select(FileModel).where(FileModel.id == uid)).first()
        if not db_file:
        ...
        response.headers["Content-Disposition"] = f"attachment; filename={db_file.nickname}"
        file.close()
    ...
    def get_file(
        ...
        # 根据关键字查询
        if name:
            query = query.where(FileModel.name.like(name))
        # 根据后缀查询
        if suffix:
            query = query.where(FileModel.suffix == suffix)
        # 根据保存目录查询
        if save_dir:
            query = query.where(FileModel.save_dir == save_dir)
        # 根据添加时间查询
        if add_time_gte:
            query = query.where(FileModel.add_time >= add_time_gte)
        if add_time_lte:
            query = query.where(FileModel.add_time <= add_time_lte)
        # 根据大小查询
        if size_gte:
            query = query.where(FileModel.size >= size_gte)
        if size_lte:
            query = query.where(FileModel.size <= size_lte)
