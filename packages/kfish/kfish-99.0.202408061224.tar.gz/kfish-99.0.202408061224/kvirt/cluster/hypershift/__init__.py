...
def scale(config, plandir, cluster, overrides):
    ...
    nodepoolcmd = "oc get nodepool -n %s %s -o jsonpath='{.spec.replicas}'" % (namespace, nodepool)
    current_replicas = os.popen(nodepoolcmd).read()
    if current_replicas == "":
    ...
    if baremetal_hosts:
        if assisted:
            ...
            number = overrides.get('workers', 2) - current_replicas if nodepool != cluster else len(all_baremetal_hosts)
