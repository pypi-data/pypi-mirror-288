...
class TinyMoss():
    ...
    def __init__(self, nodes_conf_path: str = 'moss.nodes'):
        self._nodes_conf_path = nodes_conf_path if nodes_conf_path.endswith('.json') else f'{nodes_conf_path}.json'
                ...
                if filename.endswith('.py') and filename != '__init__.py':
                    ...
                    for attr in dir(module):
                        ...
                        if isinstance(n, type) and issubclass(n, MossNode) and n is not MossNode:
                            self._nodes.append(n(amqp=amqp, node_id=node.id, services=node.services, config=node.config, node_data=node.nodeData))
    ...
    def reboot(self):
        ...
        downcount = 5
        print(f'\nMoss will restart after {downcount} sec', end='\r')

        while downcount > 0:
            time.sleep(1)
            downcount -= 1
            print(f'Moss will restart after {downcount} sec', end='\r')
