...
class SuperbLabel(SuperbApps):
    ...
    def make_bbox_annotation(
        ...
        - annotation (list)
            ...
            - video: A list containing the coordinates of the bounding box in the order [x, y, width, height, frame_num, tracking_id].
        ...
        if data_type == "image":
            ...
            if len(annotation) == 4:
                raise ValueError(
                    "The 'frame_num', 'tracking_id' parameter is required."
                )
            elif len(annotation) == 5:
                raise ValueError("The 'tracking_id' parameter is required.")
            bbox = {
                "class_name": class_name,
                "annotation_type": "box",
                "tracking_id": annotation[5],
                "annotations": [
                    {
                        "coord": {
                            "x": annotation[0],
