...
def read_version_file(schema: str, s3=S3_CLIENT) -> Dict[str, List[Dict[str, Any]]]:
    ...
    try:
        ...
        version_info: Dict[str, List[Dict[str, Any]]] = {"tables": [{"schema_name": schema, "tables": []}]}
...
def update_table_entry_of_version_info(schema_entry: Dict, updated_table_entry: Dict, table_name: str, schema_name: str) -> Dict:
...
def update_file_entry(
    ...
    if not file_entry:
        ...
        existing_version = next(
            (v for v in file_entry["versions"] if v["file"] == file),
            None
        )
        
        if existing_version:
            print(f"File {file} already exists in {file_name}. Skipping update.")
            return file_entry
...
def update_file_info(
    ...
    if file_entry:
        ...
        updated_file_entry = create_file_name_entry(
            table_entry, file_name, file, file_size, s3_location, timestamp, fyear, upload_date
        )
...
def update_version_tracker(
    ...
    updated_schema_entry = update_table_entry_of_version_info(
        schema_entry, updated_table_entry, table_name, schema_name  # Added schema_name here
    )
