...
class malaria_inference:
    ...
    def predict(self, images_path):
        outdir_slice_ims = os.path.join(PROJECT_ROOT, 'data', 'thin', 'slice_images')
        slice_sep = '_'
        ...
        results = []
        results = model(source, save=True, save_txt=True, save_conf=True, conf=0.25, project='results/yolov8_detect')
        ...
        #     with open(txt_regress_path, 'r') as file:
        #         lines = file.readlines()
        #     # 遍历每行结果
        #     for line in lines:
        #         line = line.strip().split(' ')
        #         class_label, x, y, w, h, conf = map(float, line)
        #         pre = [x, y, w, h, class_label, conf]
        #         results.boxes = pre
        #
        # print(results)
