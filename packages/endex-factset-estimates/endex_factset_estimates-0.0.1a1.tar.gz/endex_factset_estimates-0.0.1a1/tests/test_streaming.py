...
from endex_factset_estimates._streaming import Stream, AsyncStream, ServerSentEvent


@pytest.mark.asyncio
@pytest.mark.parametrize("sync", [True, False], ids=["sync", "async"])
async def test_basic(sync: bool, client: EndexFactsetEstimates, async_client: AsyncEndexFactsetEstimates) -> None:
...
async def test_data_missing_event(
    sync: bool, client: EndexFactsetEstimates, async_client: AsyncEndexFactsetEstimates
) -> None:
    def body() -> Iterator[bytes]:
        yield b'data: {"foo":true}\n'
        yield b"\n"
...
async def test_special_new_line_character(
    ...
    async_client: AsyncEndexFactsetEstimates,
) -> None:
    def body() -> Iterator[bytes]:
