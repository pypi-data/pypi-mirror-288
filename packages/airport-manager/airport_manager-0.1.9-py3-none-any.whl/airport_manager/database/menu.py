...
def handle_database_menu(choice, token, raw):
    console.clear()
    data = None
    if choice == "1":
        airport_code = Prompt.ask("Enter airport code")
        data = get_api_token(token, airport_code)
    elif choice == "2":
        airport_code = Prompt.ask("Enter airport code")
        data = get_airport_coords(token, airport_code)
    elif choice == "3":
        lat = None
        lon = None
        while lat is None:
        ...
        while coords is None:
            try:
                ...
                coords = None
        data = find_areas_for_coordinates(token, coords)
