...
class URL:
    ...
    def getattr(self, name):
        path = self.url.path
        return self.of(self.url._replace(path = f"{path}{'' if path.endswith('/') else '/'}{quote(name, safe = '')}"), self.kwargs, self.lift, self.ttl).handle
    ...
    def call(self, args, kwargs):
        ...
        if self.ttl:
            return self.of(self.url, kwargs, None, self.ttl - 1).handle
        ...
        with urlopen(urlunsplit([self.url.scheme, self.url.netloc, self.url.path, '&'.join(query), self.url.fragment])) as f:
            return xform(f.read())
