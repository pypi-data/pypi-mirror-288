...
def test_limited_sync(plus_sync_cmd, initialized, rclone_config, subjects_in_folder):
...
def test_limit_subjects_sync(plus_sync_cmd, initialized, rclone_config, subjects_in_folder, hashed):
    subjects, tmpdir_path = subjects_in_folder(5, 'testdata')
    plus_sync_cmd(['add', 'rclone', 'test_rclone', 'test', str(Path(tmpdir_path, 'testdata')), '--globs', '*.txt'])
...
@pytest.mark.parametrize('hashed', [True, False])
def test_limit_subjects_wrong_subject(plus_sync_cmd, initialized, rclone_config, subjects_in_folder, hashed):
    subjects, tmpdir_path = subjects_in_folder(3, 'testdata')
    plus_sync_cmd(['add', 'rclone', 'test_rclone', 'test', str(Path(tmpdir_path, 'testdata')), '--globs', '*.txt'])
