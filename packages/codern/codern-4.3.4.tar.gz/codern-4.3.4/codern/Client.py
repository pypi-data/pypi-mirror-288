...
def get_video_thumbnail_bytes(video_path):
    ...
    except ModuleNotFoundError:raise ModuleNotFoundError("Not model moviepy")
...
class rubino():
    ...
    def add_Post(self, post_file: str, caption: str = None, time: int = 1, size: list = [668, 798], thumbnail_file: str = None, profile_id: str = None):
        if post_file.startswith("https://"):
            content_type = self.check_url_content_type(post_file)
            ...
            if content_type == 'image':
                ...
                return "Invalid file type url"
            post_file_content = requests.get(post_file).content
        else:
            post_extension = post_file.split(".")[-1].lower()
            if post_extension in ["mp4", "mov", "mkv"]:
                post_type = "Video"
            elif post_extension in ["jpg", "jpeg", "png"]:
                post_type = "Picture"
            else:
                return "file address error"
            with open(post_file, 'rb') as file:
                post_file_content = file.read()

        try:
            if thumbnail_file and thumbnail_file.startswith("https://"):
                response = requests.get(thumbnail_file)
                content_type = response.headers.get('Content-Type', '').lower()
                if 'image' in content_type:
                    thumbnail_content = response.content
                else:
                    print("Not Image Url In Thumbnail")
                    thumbnail_content = confing.th
            else:
                thumbnail_content = self.image_to_bytes(thumbnail_file) if isinstance(thumbnail_file, str) else confing.th

            tumb_res = self.upload_file(thumbnail_content, "Picture", profile_id)
        except ModuleNotFoundError:
            print("pip install moviepy")
            tumb_res = self.upload_file(confing.th, "Picture", profile_id)

        post_res = self.upload_file(post_file_content, post_type, profile_id)
        ...
        if post_type == "Video":
            data["duration"] = str(time)
            data["snapshot_file_id"] = tumb_res[0]["file_id"]
            data["snapshot_hash_file_receive"] = tumb_res[1]
