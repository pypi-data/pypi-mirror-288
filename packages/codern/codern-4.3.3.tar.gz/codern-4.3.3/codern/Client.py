...
class rubino():
    ...
    def add_Post(self,post_file:str,caption:str=None,time:int=1,size:list=[668,798],thumbnail_file:str=None,profile_id:str=None):
        if post_file.split(".")[-1] == "mp4" or post_file.split(".")[-1] == "mov" or post_file.split(".")[-1] == "mkv" or "https://" in post_file:
            try:
                if thumbnail_file and "https://" in thumbnail_file:
                    response = requests.get(
                        thumbnail_file
                        )
                    content_type = response.headers.get('Content-Type')
                    if content_type and 'image' in content_type.lower():
                        tumb_res = req(self.auth).upload(
                            response.content if type(thumbnail_file) is str else confing.th ,
                            "Picture",
                            profile_id)
                    else:
                    ...
                    tumb_res = req(self.auth).upload(
                        image_to_bytes(thumbnail_file) if type(thumbnail_file) is str else confing.th ,
                        "Picture",
                        profile_id
                        )
            ...
            if "https://" in post_file :
                clear = requests.get(post_file)
                if clear.status_code == 200:
                    post_res = req(self.auth).upload(
                        clear.content,
                        "Video",profile_id
                        )
                else:
        ...
        return req(self.auth).send_request(
            data,
            "addPost"
            )
