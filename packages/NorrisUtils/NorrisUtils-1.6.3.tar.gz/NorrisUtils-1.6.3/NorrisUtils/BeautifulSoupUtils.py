...
def find_tags_by_query(url, query, regex, tag="a", **kwargs):
    """
    根据查询字符串和正则表达式查找指定标签的元素。

    :param url: 网页的URL
    :param query: 查询字符串
    :param regex: 正则表达式，用于筛选标签内容
    :param tag: 要查找的标签名称，默认为'a'
    :param kwargs: 其他关键字参数，包括src（页面源码）和redis（Redis数据库实例）
    :return: 匹配的标签元素列表
    """
    try:
        # 设置默认值
...
#
# tags = find_tags_by_query(url, query='京东', regex='', tag="blockquote", redis=redis_db)
# print('\n aaaaa:' + str(tags))
