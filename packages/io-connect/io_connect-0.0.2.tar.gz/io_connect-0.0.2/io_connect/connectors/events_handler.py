...
class EventsHandler:
    ...
    def __init__(
        ...
        ----------
        user_id : str
            Stores the user ID provided during initialization.

        third_party_server : str
            Stores the third-party server URL or IP address provided during initialization.

        on_prem : bool
            Stores the on-premises flag provided during initialization.

        tz : Union[pytz.BaseTzInfo, timezone]
            Stores the timezone provided during initialization.

        Example:
        -------
        >>> import pytz
        >>> handler = EventsHandler(user_id="user123", third_party_server="https://api.example.com", on_prem=True, tz=pytz.timezone('America/New_York'))
        >>> print(handler.user_id)
        user123
        >>> print(handler.third_party_server)
        ...
        self.third_party_server = third_party_server
    ...
    def publish_event(
        ...
        try:
            ...
            url = c.PUBLISH_EVENT_URL.format(
                protocol=protocol, third_party_server=self.third_party_server
            )
