...
### note: Extra does not work currently, since records dont get its property, a global logging override can be done on logging.logger.makerecord to add this property ###
        ...
        class JSON(_logging.Formatter):
            ...
            def format(self, record: _logging.LogRecord) -> str:
                ...
                msg = {
                    ...
                    **({'context': record.extra} if hasattr(record, 'extra') else {}),
                }
...
class FileLogger:
    ...
    def GetLogger(cls, 
        ...
        logger = _logging.getLogger(f"{cls._logNamePrefix}{hash((filepath,minimumLogLevel,useUTCTime,formatOptions))}")
    ...
    def CreateHandler(cls, filepath:str, useUTCTime=False, formatOptions=_GetLogger_DefaultFormatOptions):
        ...
        handler.setFormatter(_BaseLogger.Formatters.Plain(formatOptions=formatOptions, useUTCTime=useUTCTime))
        return handler
...
class FileLogger_JSON(FileLogger):
    ...
    def CreateHandler(cls, filepath:str, useUTCTime=False, formatOptions=FileLogger._GetLogger_DefaultFormatOptions):
        ...
        handler = _logging.FileHandler(filepath, encoding='utf-8')
        handler.setFormatter(_BaseLogger.Formatters.JSON(formatOptions=formatOptions, useUTCTime=useUTCTime))
        return handler
...
class RotatingFileLogger:
    ...
    def GetLogger(cls, 
        ...
        logger = _logging.getLogger(f"{cls._logNamePrefix}{hash((filepath,minimumLogLevel,maxBytes,maxRotations,useUTCTime,formatOptions))}")
        if(registerGlobalUnhandledExceptions):
            _BaseLogger.RegisterAsUnhandledExceptionHandler(logger)
        if(logger.hasHandlers()):
            return logger
...
class StreamLogger:
    ...
    def GetLogger(cls, 
                  ...
                  formatOptions=_GetLogger_DefaultFormatOptions):
        formatOptions.InheritDefaultOptions(cls._GetLogger_DefaultFormatOptions)
        stdoutLogger = _logging.getLogger(f"__STDOUTLOGGER__{hash((minimumLogLevel,useUTCTime,formatOptions,stream))}")
        if(registerGlobalUnhandledExceptions):
