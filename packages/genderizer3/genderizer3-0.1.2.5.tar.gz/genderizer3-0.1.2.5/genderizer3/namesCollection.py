...
class NamesCollection(object):
    ...
    names and their extra information from the source file. So this step
    may take some time.
    ...
    def loadFromSource(cls):
        ...
        with open(dir_path + cls.collectionSourceFile, encoding="utf-8") as f:
            for i in f:
                item = i.strip().split(",")
                ...
                item_dict = {item[2]: item[1]}

                if firstName in items:
                    items[firstName].update(item_dict)
                else:
                    items[firstName] = item_dict
