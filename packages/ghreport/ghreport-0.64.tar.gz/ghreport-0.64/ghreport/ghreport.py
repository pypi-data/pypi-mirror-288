...
async def get_raw_pull_requests(owner:str, repo:str, token:str, state:str = 'OPEN', \
                                chunk:int = 250, since: datetime|None=None,
        ...
        while True:
            ...
            if 'nodes' in data:
                for pull_request in data['nodes']:
                    if pull_request['createdAt'] >= since:
...
def get_pull_requests(owner:str, repo:str, token:str, state: str='OPEN', \
               chunk:int = 250, raw_pull_requests: list[dict[str,str]]|None=None, \
...
def plot_median_time_to_close_prs(formatter: FormatterABC, 
    ...
    pull_requests = get_pull_requests(org, repo, token, state='CLOSED', since=since, verbose=verbose)
    ...
    # Plot the results  
    plot_line(medians, f"Median time to close PRs for {org}/{repo}", "Month", "Days", width=0.9)
    return formatter.plot('median_time_to_close_prs')
...
def create_report(org: str, repo: str, token: str, out: str|None=None, 
    ...
    if show_all and fmt != '.txt':
        ...
        pr_close_time_chart = plot_median_time_to_close_prs(formatter, org, repo, token, verbose)
    else:
        open_bugs_chart = ''
        pr_close_time_chart = ''

    result = formatter.report(org, repo, now, report, termranks, open_bugs_chart, pr_close_time_chart)
    output_result(out, result, now)
