...
try:
    ...
    from .utility_functions import allow_top_navigation, stringify_javascript_function
except Exception:
    from date_formatter import python_to_luxon_format
    from utility_functions import allow_top_navigation, stringify_javascript_function
...
if not _RELEASE:
    ...
    with content_column:
        with st.container(border=True):
            ...
            render_function_code = stringify_javascript_function(
                js_file_path, 'truncateText')

            common_js_functions = {
                "addPrefix": stringify_javascript_function(js_file_path, 'addPrefix')
            }
            props["columns"] = [
                {"data": "id", "title": "id"},
            ...
            dt_click = st_datatable(None, props=props, common_js_functions=common_js_functions, ajax_setup=ajax_setup, ajax_reload=ajax_reload,
                                    key="ajax_table", on_select="rerun", export_file_name=filename, enable_diagnostics=True)
            st.write(dt_click)
