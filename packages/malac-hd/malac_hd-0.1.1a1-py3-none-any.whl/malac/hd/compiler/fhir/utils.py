...
class FHIRPathUtils(object):
    ...
    def string_value(self, val):
        if isinstance(val, str):
            ...
            raise BaseException("Unexpected type")
    ...
    def union(self, val_a, val_b):
        ...
        for val in val_a + val_b:
            ...
            if value_annotation == type(val).__name__ + "Enum" or value_annotation == type(val).__name__ + "-primitive":
                key = val.value
            else:
                key = val
            result_dict[key] = val
    ...
    def membership(self, val_a, val_b):
        ...
        if value_annotation == type(val_a).__name__ + "Enum" or value_annotation == type(val_a).__name__ + "-primitive":
            key_a = val_a.value
        else:
            key_a = val_a
        for val in val_b:
        ...
        for root in roots:
            if root.gds_elementtree_node_.__class__.__module__ == "lxml.etree": # XML input
                ...
                if reference.startswith("#"):
                    ...
                    if elem:
                        return [get_module(root).node_dict[elem[0]]]
                    else:
                        elem = root.gds_elementtree_node_.xpath(".//*[local-name()='id' and @value='{0}']".format(reference[1:]))
                        if elem:
                            return [get_module(root).node_dict[elem[0]].parent_object_]
...
def get_type(var_type, element):
    ...
    if var_type:
        if element:
            ...
            if var_type.__name__ in ["TS"] and type_str == "string":
            ...
            elif type_str in ["string-primitive", "code-primitive", "uri-primitive", "uuid-primitive", "canonical-primitive", "oid-primitive", "SampledDataDataType-primitive", "id-primitive", "markdown-primitive", "base64Binary-primitive", "hl7_oid", "hl7_st", "NMTOKEN", "xs_string", "xs_NMTOKEN", "xs_NMTOKENS"]:
                var_type = str
            elif type_str == "boolean-primitive":
                var_type = bool
            elif type_str in ["integer-primitive", "unsignedInt-primitive", "positiveInt-primitive"]:
                var_type = int
            elif type_str == "time-primitive":
                var_type = datetime.time
            elif type_str == "instant-primitive":
                var_type = datetime.datetime
            elif type_str in ["dateTime-primitive", "date-primitive"]:
                var_type = dateString
            elif type_str in ["real", "decimal-primitive"]:
                var_type = decimal.Decimal
            elif type_str in ["EntityNamePartQualifier", "ActMood", "ContextControl", "ActClass"]:
