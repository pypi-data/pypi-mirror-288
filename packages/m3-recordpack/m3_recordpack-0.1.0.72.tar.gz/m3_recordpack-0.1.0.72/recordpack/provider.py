...
class DjangoModelProvider(BaseRecordProvider):
    ...
    def pre_process_query(self, query_object: QueryObject, data: QuerySet) -> QuerySet:
        """Точка расширения, возможность обработать data на основе query_object перед основными действиями.

        Args:
            query_object: экземпляр QueryObject
            data: собираемый кверисет

        Returns:
            обработанный кверисет
        """
    ...
    def get_records(self, query_object):
        ...
        data = self.pre_process_query(query_object, data)
