...
class BaseRecordListPack(BaseRecordPack, IMultiSelectablePack):
    ...
    def get_display_dict(self, key, value_field='id', display_field='name'):
        ...
        for key in filter(None, keys):
            if isinstance(key, (Model, BaseProxy)):
                ...
                remain_keys.append(key)

        if remain_keys:
            rows_from_keys = self.get_display_records(remain_keys)
            rows.extend(rows_from_keys)
    ...
    def get_display_record(self, key, attrs=None):
        ...
        :return: запись
        """
        qo = self.provider.get_query_object_class()(model=self.provider.data_source)
    ...
    def get_display_records(self, keys):
        """
        Подготовка к запросу к БД для получения записей по списку идентификаторов

        Args:
            keys: список идентификаторов
        """

        query_object = self.provider.get_query_object_class()(model=self.provider.data_source)
        query_object.view = FIELD_VIEW
        query_object.filter = BE(self.provider.key_field, BE.IN, keys)

        return self.get_records(query_object=query_object)

    def get_records(self, query_object=None):
        """
        Отправка запроса к провайдеру на получение записей.

        Args:
            query_object: подготовленный QueryObject с фильтром
        """

        query_object = query_object or self.provider.get_query_object()(model=self.provider.data_source)

        rows_data = self.provider.get_records(query_object)
