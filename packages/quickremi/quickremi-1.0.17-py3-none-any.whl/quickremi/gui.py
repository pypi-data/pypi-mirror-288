...
class GUI:
    ...
    def create_clickable_table(self, frame, lst, H, W, L, T, rowcommand=None, bg='seashell', fg='black',
        ...
        table = gui.Table(width=f'{W}%', height=f'{H}%', margin='10px')
        table.css_left = f'{L}%'
        table.css_top = f'{T}%'

        # Create table header
        header_row = gui.TableRow(style={'font-weight': 'bold'})
        for header in lst[0]:
            header_item = gui.TableItem(str(header), style={'background-color': bg})
    ...
    def create_editable_table(self, frame, lst, H, W, L, T, bg='seashell', fg='black', overflow='scroll',
        ...
        for row_index, row_data in enumerate(lst[1:]):
            ...
            for col_index, item in enumerate(row_data):
                text_input = gui.TextInput(single_line=True, style={'width': '100%'})
