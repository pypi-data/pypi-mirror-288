...
def main(database, configuration, output_dir):
    ...
    tmp = pandas.read_sql(
        ...
        '"experiment.cores","experiment.libblas","experiment.problem_type",'
    ...
    for problem_type in ["poisson", "elasticity"]:
        for scaling_type in ["weak", "strong"]:
            ...
            # Hover-over data
            tmp["BLAS"] = (
                tmp["experiment.libblas"] + ":" + tmp["experiment.libblas_version"]
            )
            tmp["MPI"] = tmp["experiment.mpi"] + ":" + tmp["experiment.mpi_version"]
            tmp["fenicsx-performance-tests"] = tmp[
