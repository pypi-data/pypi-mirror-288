...
class Client:
    ...
    async def fetch_paginated_data(self, endpoint: str, page: int = 1, size: int = 50):
        response = await self.client.get(endpoint, params={'page': page, 'size': size})
        response.raise_for_status()
        return response.json()

    async def fetch_paginated_data_with_params(self, endpoint: str, params: dict):
        response = await self.client.get(endpoint, params=params)
        response.raise_for_status()
        return response.json()

    async def post_paginated_data_with_params(self, endpoint: str, body: dict, page: int = 1, size: int = 50):
