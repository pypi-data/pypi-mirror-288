...
def read_docstring(filename, cls):
    main_file = open(package + "/" + filename + ".py").read()
    rex = r'class ' + cls + r':\n\s*"""([\W\w]+?)"""'
...
setup(name=package,
      ...
      long_description=open(package + "/README.rst").read()
                       + read_docstring("capfit", "capfit")
                       + read_docstring("capfit", "lsq_box")
                       + read_docstring("capfit", "lsq_lin")
