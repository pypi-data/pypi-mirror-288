// #define DEBUG 1
/*
 *  iaf_psc_exp_active_dendrite_nestml.cpp
 *
...
void
register_iaf_psc_exp_active_dendrite_nestml( const std::string& name )
{
  nest::register_node_model< iaf_psc_exp_active_dendrite_nestml >( name );
}

// ---------------------------------------------------------------------------
//   Recordables map
// ---------------------------------------------------------------------------
nest::RecordablesMap<iaf_psc_exp_active_dendrite_nestml> iaf_psc_exp_active_dendrite_nestml::recordablesMap_;
namespace nest
{

  // Override the create() method with one call to RecordablesMap::insert_()
  // for each quantity to be recorded.
template <> void RecordablesMap<iaf_psc_exp_active_dendrite_nestml>::create()
  {
    // add state variables to recordables map
   insert_(iaf_psc_exp_active_dendrite_nestml_names::_V_m, &iaf_psc_exp_active_dendrite_nestml::get_V_m);
   insert_(iaf_psc_exp_active_dendrite_nestml_names::_t_dAP, &iaf_psc_exp_active_dendrite_nestml::get_t_dAP);
   insert_(iaf_psc_exp_active_dendrite_nestml_names::_I_dAP, &iaf_psc_exp_active_dendrite_nestml::get_I_dAP);
   insert_(iaf_psc_exp_active_dendrite_nestml_names::_syn_kernel__X__spikes_in, &iaf_psc_exp_active_dendrite_nestml::get_syn_kernel__X__spikes_in);
   insert_(iaf_psc_exp_active_dendrite_nestml_names::_syn_kernel__X__spikes_in__d, &iaf_psc_exp_active_dendrite_nestml::get_syn_kernel__X__spikes_in__d);
    // add recordable inline expressions to recordables map
	insert_(iaf_psc_exp_active_dendrite_nestml_names::_I_syn, &iaf_psc_exp_active_dendrite_nestml::get_I_syn);
...
//   is a part of iaf_psc_exp_active_dendrite_nestml's constructor.
// ---------------------------------------------------------------------------

iaf_psc_exp_active_dendrite_nestml::Parameters_::Parameters_()
{
}

iaf_psc_exp_active_dendrite_nestml::State_::State_()
{
}
...
iaf_psc_exp_active_dendrite_nestml::Buffers_::Buffers_(iaf_psc_exp_active_dendrite_nestml &n):
  logger_(n)
...
iaf_psc_exp_active_dendrite_nestml::Buffers_::Buffers_(const Buffers_ &, iaf_psc_exp_active_dendrite_nestml &n):
  logger_(n)
  , spike_inputs_( std::vector< nest::RingBuffer >( NUM_SPIKE_RECEPTORS ) )
  , spike_inputs_grid_sum_( std::vector< double >( NUM_SPIKE_RECEPTORS ) )
{
...
iaf_psc_exp_active_dendrite_nestml::iaf_psc_exp_active_dendrite_nestml():StructuralPlasticityNode(), P_(), S_(), B_(*this)
{
...
iaf_psc_exp_active_dendrite_nestml::iaf_psc_exp_active_dendrite_nestml(const iaf_psc_exp_active_dendrite_nestml& __n):
...
iaf_psc_exp_active_dendrite_nestml::~iaf_psc_exp_active_dendrite_nestml()
{
}
...
// ---------------------------------------------------------------------------
void iaf_psc_exp_active_dendrite_nestml::calibrate_time( const nest::TimeConverter& tc )
{
  LOG( nest::M_WARNING,
...
void iaf_psc_exp_active_dendrite_nestml::init_state_internal_()
{
#ifdef DEBUG
  std::cout << "iaf_psc_exp_active_dendrite_nestml::init_state_internal_()" << std::endl;
#endif
...
void iaf_psc_exp_active_dendrite_nestml::init_buffers_()
{
#ifdef DEBUG
  std::cout << "iaf_psc_exp_active_dendrite_nestml::init_buffers_()" << std::endl;
#endif
  // spike input buffers
...
void iaf_psc_exp_active_dendrite_nestml::recompute_internal_variables(bool exclude_timestep) {
  const double __resolution = nest::Time::get_resolution().get_ms();  // do not remove, this is necessary for the resolution() function
...
void iaf_psc_exp_active_dendrite_nestml::pre_run_hook() {
  B_.logger_.init();
...
void iaf_psc_exp_active_dendrite_nestml::update(nest::Time const & origin,const long from, const long to)
{
  const double __resolution = nest::Time::get_resolution().get_ms();  // do not remove, this is necessary for the resolution() function
...
void iaf_psc_exp_active_dendrite_nestml::handle(nest::DataLoggingRequest& e)
{
  B_.logger_.handle(e);
}


void iaf_psc_exp_active_dendrite_nestml::handle(nest::SpikeEvent &e)
{
