// #define DEBUG 1
/*
 *  iaf_psc_alpha_adapt_curr_nestml.cpp
 *
...
void
register_iaf_psc_alpha_adapt_curr_nestml( const std::string& name )
{
  nest::register_node_model< iaf_psc_alpha_adapt_curr_nestml >( name );
}

// ---------------------------------------------------------------------------
//   Recordables map
// ---------------------------------------------------------------------------
nest::RecordablesMap<iaf_psc_alpha_adapt_curr_nestml> iaf_psc_alpha_adapt_curr_nestml::recordablesMap_;
namespace nest
{

  // Override the create() method with one call to RecordablesMap::insert_()
  // for each quantity to be recorded.
template <> void RecordablesMap<iaf_psc_alpha_adapt_curr_nestml>::create()
  {
    // add state variables to recordables map
   insert_(iaf_psc_alpha_adapt_curr_nestml_names::_V_m, &iaf_psc_alpha_adapt_curr_nestml::get_V_m);
   insert_(iaf_psc_alpha_adapt_curr_nestml_names::_I_sfa, &iaf_psc_alpha_adapt_curr_nestml::get_I_sfa);
   insert_(iaf_psc_alpha_adapt_curr_nestml_names::_I_kernel_exc__X__exc_spikes, &iaf_psc_alpha_adapt_curr_nestml::get_I_kernel_exc__X__exc_spikes);
   insert_(iaf_psc_alpha_adapt_curr_nestml_names::_I_kernel_exc__X__exc_spikes__d, &iaf_psc_alpha_adapt_curr_nestml::get_I_kernel_exc__X__exc_spikes__d);
   insert_(iaf_psc_alpha_adapt_curr_nestml_names::_I_kernel_inh__X__inh_spikes, &iaf_psc_alpha_adapt_curr_nestml::get_I_kernel_inh__X__inh_spikes);
   insert_(iaf_psc_alpha_adapt_curr_nestml_names::_I_kernel_inh__X__inh_spikes__d, &iaf_psc_alpha_adapt_curr_nestml::get_I_kernel_inh__X__inh_spikes__d);

    // Add vector variables  
  }
}
std::vector< std::tuple< int, int > > iaf_psc_alpha_adapt_curr_nestml::rport_to_nestml_buffer_idx =
{
  
  { iaf_psc_alpha_adapt_curr_nestml::EXC_SPIKES, iaf_psc_alpha_adapt_curr_nestml::INH_SPIKES },
};

// ---------------------------------------------------------------------------
//   Default constructors defining default parameters and state
//   Note: the implementation is empty. The initialization is of variables
//   is a part of iaf_psc_alpha_adapt_curr_nestml's constructor.
// ---------------------------------------------------------------------------

iaf_psc_alpha_adapt_curr_nestml::Parameters_::Parameters_()
{
}

iaf_psc_alpha_adapt_curr_nestml::State_::State_()
{
}
...
iaf_psc_alpha_adapt_curr_nestml::Buffers_::Buffers_(iaf_psc_alpha_adapt_curr_nestml &n):
  logger_(n)
...
iaf_psc_alpha_adapt_curr_nestml::Buffers_::Buffers_(const Buffers_ &, iaf_psc_alpha_adapt_curr_nestml &n):
  logger_(n)
  , spike_inputs_( std::vector< nest::RingBuffer >( NUM_SPIKE_RECEPTORS ) )
  , spike_inputs_grid_sum_( std::vector< double >( NUM_SPIKE_RECEPTORS ) )
{
...
iaf_psc_alpha_adapt_curr_nestml::iaf_psc_alpha_adapt_curr_nestml():StructuralPlasticityNode(), P_(), S_(), B_(*this)
{
...
iaf_psc_alpha_adapt_curr_nestml::iaf_psc_alpha_adapt_curr_nestml(const iaf_psc_alpha_adapt_curr_nestml& __n):
  ...
  P_.V_th = __n.P_.V_th;
  P_.tau_sfa = __n.P_.tau_sfa;
  P_.Delta_I_sfa = __n.P_.Delta_I_sfa;
  P_.I_e = __n.P_.I_e;

  // copy state struct S_
  S_.r = __n.S_.r;
  S_.V_m = __n.S_.V_m;
  S_.I_sfa = __n.S_.I_sfa;
  ...
  V_.__P__V_m__I_sfa = __n.V_.__P__V_m__I_sfa;
  ...
  V_.__P__I_sfa__I_sfa = __n.V_.__P__I_sfa__I_sfa;
...
iaf_psc_alpha_adapt_curr_nestml::~iaf_psc_alpha_adapt_curr_nestml()
{
}
...
// ---------------------------------------------------------------------------
void iaf_psc_alpha_adapt_curr_nestml::calibrate_time( const nest::TimeConverter& tc )
{
  LOG( nest::M_WARNING,
...
void iaf_psc_alpha_adapt_curr_nestml::init_state_internal_()
{
#ifdef DEBUG
  std::cout << "iaf_psc_alpha_adapt_curr_nestml::init_state_internal_()" << std::endl;
#endif
  ...
  // initial values for parameters
    ...
    P_.tau_sfa = 100; // as ms
    

    P_.Delta_I_sfa = 100; // as pA
...
void iaf_psc_alpha_adapt_curr_nestml::init_buffers_()
{
#ifdef DEBUG
  std::cout << "iaf_psc_alpha_adapt_curr_nestml::init_buffers_()" << std::endl;
#endif
  // spike input buffers
...
void iaf_psc_alpha_adapt_curr_nestml::recompute_internal_variables(bool exclude_timestep) {
  const double __resolution = nest::Time::get_resolution().get_ms();  // do not remove, this is necessary for the resolution() function
  ...
  if (exclude_timestep) {    
      ...
      V_.__P__V_m__I_sfa = 1.0 * P_.tau_m * P_.tau_sfa * (std::exp(V_.__h / P_.tau_m) - std::exp(V_.__h / P_.tau_sfa)) * std::exp((-V_.__h) * (P_.tau_m + P_.tau_sfa) / (P_.tau_m * P_.tau_sfa)) / (P_.C_m * (P_.tau_m - P_.tau_sfa)); // as real
      ...
      V_.__P__I_sfa__I_sfa = 1.0 * std::exp((-V_.__h) / P_.tau_sfa); // as real
...
void iaf_psc_alpha_adapt_curr_nestml::pre_run_hook() {
  B_.logger_.init();
...
void iaf_psc_alpha_adapt_curr_nestml::update(nest::Time const & origin,const long from, const long to)
{
  const double __resolution = nest::Time::get_resolution().get_ms();  // do not remove, this is necessary for the resolution() function
...
void iaf_psc_alpha_adapt_curr_nestml::handle(nest::DataLoggingRequest& e)
{
  B_.logger_.handle(e);
}


void iaf_psc_alpha_adapt_curr_nestml::handle(nest::SpikeEvent &e)
{
  ...
  else
  {
    ...
    if ( nestml_buffer_idx == iaf_psc_alpha_adapt_curr_nestml::PORT_NOT_AVAILABLE )
    {
      nestml_buffer_idx = std::get<0>(rport_to_nestml_buffer_idx[e.get_rport()]);
    }
    weight = -weight;
...
void iaf_psc_alpha_adapt_curr_nestml::handle(nest::CurrentEvent& e)
{
  assert(e.get_delay_steps() > 0);
