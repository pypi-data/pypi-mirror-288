// #define DEBUG 1
/*
 *  iaf_psc_alpha_adapt_thresh_OU_nestml.cpp
 *
...
#include "iaf_psc_alpha_adapt_thresh_OU_nestml.h"
void
register_iaf_psc_alpha_adapt_thresh_OU_nestml( const std::string& name )
{
  nest::register_node_model< iaf_psc_alpha_adapt_thresh_OU_nestml >( name );
}

// ---------------------------------------------------------------------------
//   Recordables map
// ---------------------------------------------------------------------------
nest::RecordablesMap<iaf_psc_alpha_adapt_thresh_OU_nestml> iaf_psc_alpha_adapt_thresh_OU_nestml::recordablesMap_;
namespace nest
{

  // Override the create() method with one call to RecordablesMap::insert_()
  // for each quantity to be recorded.
template <> void RecordablesMap<iaf_psc_alpha_adapt_thresh_OU_nestml>::create()
  {
    // add state variables to recordables map
   insert_(iaf_psc_alpha_adapt_thresh_OU_nestml_names::_V_m, &iaf_psc_alpha_adapt_thresh_OU_nestml::get_V_m);
   insert_(iaf_psc_alpha_adapt_thresh_OU_nestml_names::_Theta, &iaf_psc_alpha_adapt_thresh_OU_nestml::get_Theta);
   insert_(iaf_psc_alpha_adapt_thresh_OU_nestml_names::_I_noise, &iaf_psc_alpha_adapt_thresh_OU_nestml::get_I_noise);
   insert_(iaf_psc_alpha_adapt_thresh_OU_nestml_names::_I_kernel_exc__X__exc_spikes, &iaf_psc_alpha_adapt_thresh_OU_nestml::get_I_kernel_exc__X__exc_spikes);
   insert_(iaf_psc_alpha_adapt_thresh_OU_nestml_names::_I_kernel_exc__X__exc_spikes__d, &iaf_psc_alpha_adapt_thresh_OU_nestml::get_I_kernel_exc__X__exc_spikes__d);
   insert_(iaf_psc_alpha_adapt_thresh_OU_nestml_names::_I_kernel_inh__X__inh_spikes, &iaf_psc_alpha_adapt_thresh_OU_nestml::get_I_kernel_inh__X__inh_spikes);
   insert_(iaf_psc_alpha_adapt_thresh_OU_nestml_names::_I_kernel_inh__X__inh_spikes__d, &iaf_psc_alpha_adapt_thresh_OU_nestml::get_I_kernel_inh__X__inh_spikes__d);

    // Add vector variables  
  }
}
std::vector< std::tuple< int, int > > iaf_psc_alpha_adapt_thresh_OU_nestml::rport_to_nestml_buffer_idx =
{
  
  { iaf_psc_alpha_adapt_thresh_OU_nestml::EXC_SPIKES, iaf_psc_alpha_adapt_thresh_OU_nestml::INH_SPIKES },
};

// ---------------------------------------------------------------------------
//   Default constructors defining default parameters and state
//   Note: the implementation is empty. The initialization is of variables
//   is a part of iaf_psc_alpha_adapt_thresh_OU_nestml's constructor.
// ---------------------------------------------------------------------------

iaf_psc_alpha_adapt_thresh_OU_nestml::Parameters_::Parameters_()
{
}

iaf_psc_alpha_adapt_thresh_OU_nestml::State_::State_()
{
}
...
iaf_psc_alpha_adapt_thresh_OU_nestml::Buffers_::Buffers_(iaf_psc_alpha_adapt_thresh_OU_nestml &n):
  logger_(n)
...
iaf_psc_alpha_adapt_thresh_OU_nestml::Buffers_::Buffers_(const Buffers_ &, iaf_psc_alpha_adapt_thresh_OU_nestml &n):
  logger_(n)
  , spike_inputs_( std::vector< nest::RingBuffer >( NUM_SPIKE_RECEPTORS ) )
  , spike_inputs_grid_sum_( std::vector< double >( NUM_SPIKE_RECEPTORS ) )
{
...
iaf_psc_alpha_adapt_thresh_OU_nestml::iaf_psc_alpha_adapt_thresh_OU_nestml():StructuralPlasticityNode(), P_(), S_(), B_(*this)
{
...
iaf_psc_alpha_adapt_thresh_OU_nestml::iaf_psc_alpha_adapt_thresh_OU_nestml(const iaf_psc_alpha_adapt_thresh_OU_nestml& __n):
  ...
  P_.Theta_init = __n.P_.Theta_init;
  P_.tau_Theta = __n.P_.tau_Theta;
  P_.Delta_Theta = __n.P_.Delta_Theta;
  P_.I_noise0 = __n.P_.I_noise0;
  ...
  V_.__P__Theta__Theta = __n.V_.__P__Theta__Theta;
...
iaf_psc_alpha_adapt_thresh_OU_nestml::~iaf_psc_alpha_adapt_thresh_OU_nestml()
{
}
...
// ---------------------------------------------------------------------------
void iaf_psc_alpha_adapt_thresh_OU_nestml::calibrate_time( const nest::TimeConverter& tc )
{
  LOG( nest::M_WARNING,
...
void iaf_psc_alpha_adapt_thresh_OU_nestml::init_state_internal_()
{
#ifdef DEBUG
  std::cout << "iaf_psc_alpha_adapt_thresh_OU_nestml::init_state_internal_()" << std::endl;
#endif
  ...
  // initial values for parameters
    ...
    P_.Theta_init = (-55); // as mV
    

    P_.tau_Theta = 100; // as ms
    

    P_.Delta_Theta = 2; // as mV
    

    P_.I_noise0 = 0; // as pA
    

    P_.sigma_noise = 0; // as pA
  ...
  // initial values for state variables
    ...
    S_.Theta = P_.Theta_init; // as mV
    

    S_.I_noise = P_.I_noise0; // as pA
    

    S_.I_kernel_exc__X__exc_spikes = 0; // as real
    

    S_.I_kernel_exc__X__exc_spikes__d = 0 * pow(1000.0, (-1)); // as 1 / s
    

    S_.I_kernel_inh__X__inh_spikes = 0; // as real
    

    S_.I_kernel_inh__X__inh_spikes__d = 0 * pow(1000.0, (-1)); // as 1 / s
}

void iaf_psc_alpha_adapt_thresh_OU_nestml::init_buffers_()
{
#ifdef DEBUG
  std::cout << "iaf_psc_alpha_adapt_thresh_OU_nestml::init_buffers_()" << std::endl;
#endif
  // spike input buffers
...
void iaf_psc_alpha_adapt_thresh_OU_nestml::recompute_internal_variables(bool exclude_timestep) {
  const double __resolution = nest::Time::get_resolution().get_ms();  // do not remove, this is necessary for the resolution() function
  ...
  if (exclude_timestep) {    
      ...
      V_.D_noise = 2 * pow(P_.sigma_noise, 2) / P_.tau_syn_exc; // as pA2 / ms
      

      V_.A_noise = pow(((V_.D_noise * P_.tau_syn_exc / 2) * (1 - std::exp((-2) * __resolution / P_.tau_syn_exc))), 0.5); // as pA
      ...
      V_.__P__Theta__Theta = 1.0 * std::exp((-V_.__h) / P_.tau_Theta); // as real
...
void iaf_psc_alpha_adapt_thresh_OU_nestml::pre_run_hook() {
  B_.logger_.init();
...
void iaf_psc_alpha_adapt_thresh_OU_nestml::update(nest::Time const & origin,const long from, const long to)
{
  const double __resolution = nest::Time::get_resolution().get_ms();  // do not remove, this is necessary for the resolution() function
  ...
  S_.I_noise = P_.I_noise0 + (S_.I_noise - P_.I_noise0) * std::exp((-__resolution) / P_.tau_syn_exc) + V_.A_noise * ((0) + (1) * normal_dev_( nest::get_vp_specific_rng( get_thread() ) ));
  if (S_.r == 0)
  {  
    double V_m__tmp = (-P_.E_L) * V_.__P__V_m__V_m + P_.E_L + S_.I_kernel_exc__X__exc_spikes * V_.__P__V_m__I_kernel_exc__X__exc_spikes + S_.I_kernel_exc__X__exc_spikes__d * V_.__P__V_m__I_kernel_exc__X__exc_spikes__d + S_.I_kernel_inh__X__inh_spikes * V_.__P__V_m__I_kernel_inh__X__inh_spikes + S_.I_kernel_inh__X__inh_spikes__d * V_.__P__V_m__I_kernel_inh__X__inh_spikes__d + S_.V_m * V_.__P__V_m__V_m - P_.I_e * V_.__P__V_m__V_m * P_.tau_m / P_.C_m + P_.I_e * P_.tau_m / P_.C_m - S_.I_noise * V_.__P__V_m__V_m * P_.tau_m / P_.C_m + S_.I_noise * P_.tau_m / P_.C_m - B_.I_stim_grid_sum_ * V_.__P__V_m__V_m * P_.tau_m / P_.C_m + B_.I_stim_grid_sum_ * P_.tau_m / P_.C_m;
    double Theta__tmp = S_.Theta * V_.__P__Theta__Theta - P_.Theta_init * V_.__P__Theta__Theta + P_.Theta_init;
    double I_kernel_exc__X__exc_spikes__tmp = S_.I_kernel_exc__X__exc_spikes * V_.__P__I_kernel_exc__X__exc_spikes__I_kernel_exc__X__exc_spikes + S_.I_kernel_exc__X__exc_spikes__d * V_.__P__I_kernel_exc__X__exc_spikes__I_kernel_exc__X__exc_spikes__d;
    double I_kernel_exc__X__exc_spikes__d__tmp = S_.I_kernel_exc__X__exc_spikes * V_.__P__I_kernel_exc__X__exc_spikes__d__I_kernel_exc__X__exc_spikes + S_.I_kernel_exc__X__exc_spikes__d * V_.__P__I_kernel_exc__X__exc_spikes__d__I_kernel_exc__X__exc_spikes__d;
    double I_kernel_inh__X__inh_spikes__tmp = S_.I_kernel_inh__X__inh_spikes * V_.__P__I_kernel_inh__X__inh_spikes__I_kernel_inh__X__inh_spikes + S_.I_kernel_inh__X__inh_spikes__d * V_.__P__I_kernel_inh__X__inh_spikes__I_kernel_inh__X__inh_spikes__d;
    double I_kernel_inh__X__inh_spikes__d__tmp = S_.I_kernel_inh__X__inh_spikes * V_.__P__I_kernel_inh__X__inh_spikes__d__I_kernel_inh__X__inh_spikes + S_.I_kernel_inh__X__inh_spikes__d * V_.__P__I_kernel_inh__X__inh_spikes__d__I_kernel_inh__X__inh_spikes__d;
...
void iaf_psc_alpha_adapt_thresh_OU_nestml::handle(nest::DataLoggingRequest& e)
{
  B_.logger_.handle(e);
}


void iaf_psc_alpha_adapt_thresh_OU_nestml::handle(nest::SpikeEvent &e)
{
  ...
  else
  {
    ...
    if ( nestml_buffer_idx == iaf_psc_alpha_adapt_thresh_OU_nestml::PORT_NOT_AVAILABLE )
    {
      nestml_buffer_idx = std::get<0>(rport_to_nestml_buffer_idx[e.get_rport()]);
    }
    weight = -weight;
...
void iaf_psc_alpha_adapt_thresh_OU_nestml::handle(nest::CurrentEvent& e)
{
  assert(e.get_delay_steps() > 0);
