...
def update_files_from_drive(data_list: List[Dict], data_path: Path, logger=None) -> List[GoogleDriveFileUpdateResponse]:
    """
    ...
    if logger is not None:
        logger.info(Path(service_account_file).resolve())
    ...
    for file_info in file_info_list:
        ...
        drive_download_items(service_account_file, [file_metadata], output_folder)
        handle_downloaded_files_and_save_to_qdrant(file_info.collection_id)
