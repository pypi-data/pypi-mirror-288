        ...
        for (trigger, rate, id, name, type) in zip(trigger_list, rate_list, id_list, name_list, action_type):
            try:
                ...
                if hasattr(JobAction(), 'start_%s_job' % job_type):
                    ...
                    return messages.error(request, '【start error】 ' + job_name + ' job does not exist!!')
            except ConflictingIdError as e:
                logger.error(e)
                return messages.error(request, job_name + ' job startup conflict occurred！')
            except Exception as e:
                logger.error(e)
                return messages.error(request,
                                      job_name + ' job start error！<br/>Check the scheduling type or frequency')
        return messages.success(request, (job_name if len(id_list) == 1 else 'selected') + " job started successfully")
    ...
    def stop_job(self, request, queryset):
        ...
        try:
            ...
            return messages.success(
                request,
                (name_list[0].get('job_name') if len(name_list) == 1 else 'selected') + " job stopped successfully")
        except JobLookupError as e:
            logger.error(e)
            return messages.error(request, job_name + " job does not exists!！")
