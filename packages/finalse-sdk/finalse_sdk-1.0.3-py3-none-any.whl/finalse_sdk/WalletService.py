...
from finalse_sdk.Wallet import Wallet
import json


class WalletService:
    def __init__(self, auth: Auth):
        self._type = "WalletService" 
        self.auth = auth
    ...
    def fetch_page(self, page: Page):
        ...
        return Http.list_all("/" + Sdk.version() + "/wallets", qs, {}, lambda json: Wallet.from_remote_dict(json), self.auth)

    def create(self, form: dict):
        return Http.post("/" + Sdk.version() + "/wallets",  form, "", {}, Wallet.from_json, self.auth)

    def get(self, form: str):
        return Http.get("/" + Sdk.version() + "/wallets/" + form, "", {}, Wallet.from_json, self.auth)
    ...
    def list_all(self):
        return Http.list_all("/" + Sdk.version() + "/wallets", ListForm.empty().to_query_string(), {}, lambda json: Wallet.from_remote_dict(json), self.auth)

    def update(self, form: dict):
        return Http.patch("/" + Sdk.version() + "/wallets/" + form['id'],  form, "", {}, Wallet.from_json, self.auth)

    def to_string(self):
        return self.__repr__()

    def __repr__(self):
        return "WalletService(auth=" + repr(self.auth) + ")"