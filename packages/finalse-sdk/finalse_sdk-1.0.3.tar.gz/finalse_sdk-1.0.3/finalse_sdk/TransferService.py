...
from finalse_sdk.Transfer import Transfer
import json


class TransferService:
    def __init__(self, auth: Auth):
        self._type = "TransferService" 
        self.auth = auth
    ...
    def fetch_page(self, page: Page):
        ...
        return Http.list_all("/" + Sdk.version() + "/transfers", qs, {}, lambda json: Transfer.from_remote_dict(json), self.auth)

    def initiate(self, form: dict):
        return Http.post("/" + Sdk.version() + "/transfers/initiate",  form, "", {}, Transfer.from_json, self.auth)

    def get(self, form: str):
        return Http.get("/" + Sdk.version() + "/transfers/" + form, "", {}, Transfer.from_json, self.auth)
    ...
    def list_all(self):
        return Http.list_all("/" + Sdk.version() + "/transfers", ListForm.empty().to_query_string(), {}, lambda json: Transfer.from_remote_dict(json), self.auth)

    def update(self, form: dict):
        return Http.patch("/" + Sdk.version() + "/transfers/" + form['id'],  form, "", {}, Transfer.from_json, self.auth)

    def to_string(self):
        return self.__repr__()

    def __repr__(self):
        return "TransferService(auth=" + repr(self.auth) + ")"