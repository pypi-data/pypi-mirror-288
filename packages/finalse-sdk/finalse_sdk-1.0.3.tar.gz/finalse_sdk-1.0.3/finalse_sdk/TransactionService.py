...
from finalse_sdk.Transaction import Transaction
import json


class TransactionService:
    def __init__(self, auth: Auth):
        self._type = "TransactionService" 
        self.auth = auth
    ...
    def fetch_page(self, page: Page):
        ...
        return Http.list_all("/" + Sdk.version() + "/transactions", qs, {}, lambda json: Transaction.from_remote_dict(json), self.auth)

    def get(self, form: str):
        return Http.get("/" + Sdk.version() + "/transactions/" + form, "", {}, Transaction.from_json, self.auth)
    ...
    def list_all(self):
        return Http.list_all("/" + Sdk.version() + "/transactions", ListForm.empty().to_query_string(), {}, lambda json: Transaction.from_remote_dict(json), self.auth)

    def update(self, form: dict):
        return Http.patch("/" + Sdk.version() + "/transactions/" + form['id'],  form, "", {}, Transaction.from_json, self.auth)

    def to_string(self):
        return self.__repr__()

    def __repr__(self):
        return "TransactionService(auth=" + repr(self.auth) + ")"