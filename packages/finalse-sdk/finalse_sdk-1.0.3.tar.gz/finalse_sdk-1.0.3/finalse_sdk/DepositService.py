...
class DepositService:
    ...
    def fetch_page(self, page: Page):
        ...
        return Http.list_all("/" + Sdk.version() + "/deposits", qs, {}, lambda json: Deposit.from_remote_dict(json), self.auth)

    def initiate(self, form: dict):
        return Http.post("/" + Sdk.version() + "/deposits/initiate",  form, "", {}, Deposit.from_json, self.auth)

    def get(self, form: str):
        return Http.get("/" + Sdk.version() + "/deposits/" + form, "", {}, Deposit.from_json, self.auth)
    ...
    def list_all(self):
        return Http.list_all("/" + Sdk.version() + "/deposits", ListForm.empty().to_query_string(), {}, lambda json: Deposit.from_remote_dict(json), self.auth)

    def update(self, form: dict):
        return Http.patch("/" + Sdk.version() + "/deposits/" + form['id'],  form, "", {}, Deposit.from_json, self.auth)

    def to_string(self):
        return self.__repr__()

    def __repr__(self):
        return "DepositService(auth=" + repr(self.auth) + ")"