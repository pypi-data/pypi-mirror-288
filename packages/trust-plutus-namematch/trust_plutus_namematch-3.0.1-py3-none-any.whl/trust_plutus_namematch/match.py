...
class NameMatch:
    ...
    def icra_process(self, input_, string):
    ...
    def process_funds(self, i, icraSheet_list):
        temp_ = [self.icra_process(i, x) for x in icraSheet_list]
        icraList = [x for x in temp_ if x is not None]
        result = rf.process.extract(i, icraList, scorer=fuzz.ratio, limit=1, processor=self.string_proc)
        return result
    ...
    def run(self,in_path:str, out_path:str) -> bool:
        ...
        results = Parallel(n_jobs=-1,verbose=1)(delayed(self.process_funds)(i, icraSheet_list) for i in milesSheet_list)
        matchResults = [x[0][0] for x in results]
        list3 = [list(a) for a in zip(milesSheet_list, matchResults)]
        results = pd.DataFrame(list3,columns = ["Miles fund name","ICRA fund name"])
