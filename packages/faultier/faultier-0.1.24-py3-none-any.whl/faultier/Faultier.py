...
class Faultier:
    ...
    def find_serial_port(self):
        ...
        if system == "Windows":
            return self.find_serial_port_windows()
        elif system == "Darwin":  # macOS
            return self.find_serial_port_macos()
        elif system == "Linux":
            return self.find_serial_port_linux()
        else:
            raise Exception(f"Unsupported platform: {system}")
    
    def find_serial_port_windows(self):
        for port in serial.tools.list_ports.comports():
            if self.VID.lower() in port.hwid.lower() and self.PID.lower() in port.hwid.lower():
                return port.device
        return None

    def find_serial_port_macos(self):
    ...
    def find_serial_port_linux(self):
        for port in serial.tools.list_ports.comports():
            if self.VID.lower() in port.hwid.lower() and self.PID.lower() in port.hwid.lower():
                return port.device
        return None
