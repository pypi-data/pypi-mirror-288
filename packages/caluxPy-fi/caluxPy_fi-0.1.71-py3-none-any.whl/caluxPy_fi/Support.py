...
class Support ():
    ...
    def solver(v1, v2, v3, v4):
        solution = {}
        #logger, _fileHandler = Support.setup_logger()
        objetivo = v4
        ...
        #logger.info(f'Initial objective function value: {objective(x0)}')
        ...
        if abs(sol.fun - objetivo) <= tolerance and sol.success and sol.fun > 0:
            ...
            #logger.info(f'Resultados Solver: {t1}')
            solution = {'result': t1,
                        'test': test,
                        'residual': test - v4,
                        'message': 'Solution found within the desired tolerance range.'}
        else:
