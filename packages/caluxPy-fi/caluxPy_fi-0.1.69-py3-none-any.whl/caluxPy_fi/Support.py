...
class Support ():
    ...
    def __init__(self) -> None:
        ...
        _fileHandler = logging.FileHandler("{0}/{1}.log".format(desktop, 'solver'), mode = 'w', encoding = 'utf-8')
    ...
    def solver(self, v1, v2, v3, v4):
        ...
        def objective (x):
            ...
            #x4 = x[3] #ratio
            x1, x2, x3, x4 = x
            return (x1 * x2) / x3 # Objective Function: The function to be minimized.

        # Define the constraint function(s)
        def constraint1 (x):
            return ((x[0] * x[1]) / x[2]) - x[3] # Constraint Function: Ensures the calculated value matches the target value v4.
        
        # Define the initial guess for the variables
        x0 = [v1, v2, v3, v4] # Initial Guess: Starting point for the optimization.
        self._logger.info(f'Initial objective function value: {objective(x0)}')

        # Define the bounds for the decision variables Bounds: Specifies constraints for the variables.
        #b1 = (0, None) #variable
        #b2 = (v2, v2) #fixed
        #b3 = (v3, v3) #fixed
        #b4 = (v4, v4) #fixed
        ...
        cons = [{'type': 'eq', 'fun': constraint1}] # Constraints: Defines the equality constraint.
        ...
        if abs(sol.fun - objetivo) <= tolerance and sol.success and sol.fun > 0:
            ...
            self._logger.warning("No solution found within the desired tolerance range.")
            return 0
        
        # Removing the Logger Handler: Ensures proper closing of the file handler.
        self._logger.removeHandler(next(filter(lambda h: isinstance(h, logging.FileHandler), self._logger.handlers), None))
        return solution
