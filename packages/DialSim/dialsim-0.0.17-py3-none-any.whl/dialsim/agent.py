    ...
    def save_history(self, generator_instance:dict)->tuple:
        ...
        if self.history_type == "utts":
            ...
            if generator_instance["un"] == len(generator_instance["post_utterances"])-1:
                sum_prompt = open_file('./friends/prompt/chatgpt_summarize_prompt.txt').replace('<<<DIALOG>>>', generator_instance["history"])
                try:
                ...
                if not save_timeout_flag:
                    ...
                    if not search_timeout_flag:
                        if self.agent.ret_method == 'no_ret':
                            answer_prompt = open_file(f'./{self.script_name}/prompt/naive_llm_inference.txt').replace('<<<Date>>>', inst["date"]).replace('<<<Dialog_History>>>', ret_histories).replace('<<<Question>>>', self.question_part_prompt_sh).replace('<<<Chatbot>>>', self.agent_name_sh)
                        else:
                            answer_prompt = open_file(f'./{self.script_name}/prompt/RAG_qa_prompt.txt').replace('<<<Date>>>', inst["date"]).replace('<<<Dialog_History>>>', ret_histories).replace('<<<Question>>>', self.question_part_prompt_sh).replace('<<<Chatbot>>>', self.agent_name_sh)
