...
class NORSOK(Model):
    ...
    def _calculate_co2_fugacity(self, co2_fraction, pressure, temperature):
        """
        Calculate the fugacity of CO2.

        Parameters:
        co2_fraction (float): Fraction of CO2
        pressure (float): Pressure in the system
        temperature (float): Temperature in Celsius

        Returns:
        float: Fugacity of CO2 in [bar]
        """
        temperature_k = self._convert_celsius_to_kelvin(temperature)
        fugacity_coefficient = 10 ** (pressure * (0.0031 - 1.4 / temperature_k)) \
            if pressure <= 250 else 10 ** (250 * (0.0031 - 1.4 / temperature_k))
