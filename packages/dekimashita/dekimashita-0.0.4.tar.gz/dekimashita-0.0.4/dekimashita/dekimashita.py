...
class Dekimashita:
    ...
    @staticmethod
    def vpath(text: str, separator: str = '_') -> str:
        """
        Remove non-alphanumeric characters (except a-z, A-Z, 0-9, '/', '.', ':') from text.
        Convert all letters to lowercase. Replace spaces with a specified separator.
        Double separators are replaced with a single separator.
        ...
        # Regex to match allowed characters (alphanumeric, space, '/', '.', ':', and the separator)
        allowed_chars = f'[^a-zA-Z0-9\s/{re.escape(separator)}.:]'
        
        clear_text = re.sub(allowed_chars, '', text)
    ...
    @staticmethod
    def v3path(text: str, separator: str = '_') -> str:
        """
        Remove non-alphanumeric characters (except a-z, A-Z, 0-9, '/', '.', ':', '-') from text.
        Convert all letters to lowercase. Replace spaces with a specified separator.
        Double separators are replaced with a single separator.
