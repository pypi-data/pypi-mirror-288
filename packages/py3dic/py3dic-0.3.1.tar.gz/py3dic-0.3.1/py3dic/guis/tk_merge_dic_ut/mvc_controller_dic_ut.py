...
class tkapp_DIC_UT_merge_Controller:
    ...
    def start_merging(self):
        ...
        logging.info("completed for user to sync")
        #TODO there is a problem here with 
        # the merge window which I need to fix 
        # I.e. using run is probably not optimal.
        print(f"Final offset value: {self.model.merge_dicut.time_offset_s}")
        ...
        # create final merged and save# # save dic results to file
        df_fin = self.model.merge_dicut.resample_data(time_resolution_s=INTERPOLATION_TIME_RESOLUTION_S,
                                                      save_flag=True, 
                                                      fname_prefix=f"dicMerged_{analysis_timestamp}")
        df_fin.plot.scatter(x='e_xx', y= 'force_N') # TODO plot this in a window
