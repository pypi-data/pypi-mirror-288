...
class FileSystemIterableSource(IterableSource, MultiProcessingMixIn, ABC):
    ...
    Raises:
        ValueError: If `iterable` is False or `fast_load` is False.
    """
    ...
    def file_names_iter(
        ...
        for i, entry in enumerate(scantree(self.path.resolve())):
            ...
            logger.info(f"{num_skipped_files} skipped so far", extra={"sample": True})
            logger.info(f"{num_ignored_files} ignored so far", extra={"sample": True})
    ...
    def fast_load_reservoir_sampling(
        self,
        iterable: Union[Iterator[Path], Iterator[str]],
        sample_size: int,
        sampled_file_names: List[str],
    ) -> Tuple[List[Any], bool]:
        """Reservoir sampling for fast_load."""
        all_files_sampled = False
        results: List[Any] = []
        iterator = iter(iterable)
        try:
            # Initialise the reservoir with samples based on sample_size
            while len(results) < sample_size:
                file_name = next(iterator)
                if file_name not in sampled_file_names:
                    file_name = self._apply_datasource_specific_filters_to_file(
                        str(file_name)
                    )
                    if file_name:
                        results.append(file_name)
        except StopIteration:
            all_files_sampled = True
        random.shuffle(results)  # Randomize their positions
        # When the reservoir is full, and if not all files have been iterated over,
        # for each record generate a random variable r in [0,i) where i is the
        # position of the record in the stream.
        if not all_files_sampled:
            for i, file_name in enumerate(iterator, sample_size):
                r = random.randint(0, i)  # nosec
                # If r < sample_size replace the record at position r of
                # the reservoir, otherwise discard it.
                if r < sample_size:
                    file_name = self._apply_datasource_specific_filters_to_file(
                        str(file_name)
                    )
                    if file_name:
                        results[r] = (
                            file_name  # at a decreasing rate, replace random items
                        )
        return results, all_files_sampled
    ...
    def _fast_load_data(self, **kwargs: Any) -> None:
        ...
        """
        # This method is done iteratively now;
        # rather than constructing a list and then going over it
        # in a for-loop, we directly iterate over a
        # iterator until we hit the desired criteria.
        dfs: List[pd.DataFrame] = []
        all_files_sampled = False
        sampled_and_read_file_names: List[str] = (
            []
        )  # to keep track of files that were already sampled
        num_files_processed = 0
        while (
            len(dfs) < FAST_LOAD_MAXIMUM_NUMBER_OF_FILES_TO_LOAD
            and not all_files_sampled
        ):
            # We aim to load FAST_LOAD_MINIMUM_NUMBER_OF_FILES_TO_LOAD files,
            # so we need to update this number to not oversample
            num_files_to_load = FAST_LOAD_MAXIMUM_NUMBER_OF_FILES_TO_LOAD - len(dfs)
            sampled_file_names, all_files_sampled = self.fast_load_reservoir_sampling(
                self.file_names_iter(),
                num_files_to_load,
                sampled_and_read_file_names,
            )
            for file in sampled_file_names:
                if file not in sampled_and_read_file_names:
                    df = self._get_data(
                        file_names=[file], skip_non_tabular_data=True, **kwargs
                    )
                    # Keep track of files that were already sampled so we
                    # do not attempt to reload
                    sampled_and_read_file_names.append(file)
                    if not df.empty:
                        dfs.append(df)
                        num_files_processed += 1
        if all_files_sampled and len(dfs) > 0:
            logger.info(f"All files ({num_files_processed}) loaded.")
        elif all_files_sampled:
            logger.info(
                "No files compatible the the selected datasource found. "
