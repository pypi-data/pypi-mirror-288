...
GaussianWishart = type('GaussianWishart', (GauWisTP, ),
                       {'__new__': lambda cls, *m: [_temp := GauWisTP(*m),
                                                    _temp if getattr(_temp, '_is_valid')() else 0][-1]})
...
def _gauss_conj_mu_sigma(pri: GauWisTP, like: Union[LikeDis, LikeData]) -> GauWisTP:
    if isinstance(like, ndarray):  # design mat
        ...
        _w_inv = (like.T @ like + pri.beta * (pri.mean[..., np.newaxis] @ pri.mean[..., np.newaxis].T) -
                  _beta * (_mean[..., np.newaxis] @ _mean[..., np.newaxis].T) + np.linalg.inv(pri.w))
        ...
        _w_inv = (_xn[..., np.newaxis] @ _xn[..., np.newaxis].T +
                  ...
                  _beta * (_mean[..., np.newaxis] @ _mean[..., np.newaxis].T) + np.linalg.inv(pri.w))
    return GaussianWishart(_mean, _beta, _nu, np.linalg.inv(_w_inv))
...
def _gauss_pred_mu_sigma(pos: GauWisTP, like: Union[LikeDis, LikeData]) -> StuTP:
    ...
    return st.multivariate_t(mean, np.linalg.inv(_lambda), nu)


def _gauss_conj_mu(pri: MGauTP, like: Union[LikeDis, LikeData], precision: ndarray) -> MGauTP:
    _lambda = (like.shape[0] * precision + np.linalg.inv(pri.cov) if isinstance(like, ndarray)
               else [pri := _init_gauss(pri), precision + np.linalg.inv(pri.cov)][-1])  # treat MGauTP as single test
    _mean = (np.linalg.inv(_lambda) @ (precision @ (like.sum(axis=0) if isinstance(like, ndarray)
             else [_p := _init_gauss(like), _p.mean][-1]) + np.linalg.inv(pri.cov) @ pri.mean))
    return st.multivariate_normal(_mean, np.linalg.inv(_lambda))


def _gauss_pred_mu(pos: MGauTP, like: Union[LikeDis, LikeData], precision: ndarray) -> MGauTP:
    _lambda_inv, _ = np.linalg.inv(precision) + pos.cov, like
    _mean = pos.mean
...
def _gauss_conj_sigma(pri: WisTP, like: Union[LikeDis, LikeData], mu: ndarray) -> WisTP:
    if isinstance(like, ndarray):  # design mat
        _design = like - mu
        _w, _nu = np.linalg.inv(_design.T @ _design + np.linalg.inv(pri.scale)), like.shape[0] + pri.df
    else:  # treat MGauTP as single test
        _w, _nu = np.linalg.inv(np.linalg.inv(like.cov) + np.linalg.inv(pri.scale)), 1 + pri.df
...
def gaussian(**params):
    ...
    if isinstance(prior, MGauTP):
        res = Bayes(name='gaussian', kernel=kernel, prior=prior, likelihood_check=_check_gauss,
                    update_conjugate=partial(_gauss_conj_mu, precision=np.linalg.inv(kernel.cov)),
                    update_predictive=partial(_gauss_pred_mu, precision=np.linalg.inv(kernel.cov)))
