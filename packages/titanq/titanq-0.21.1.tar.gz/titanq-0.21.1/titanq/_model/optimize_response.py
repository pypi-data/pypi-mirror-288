...
class OptimizeResponse:
    ...
    def constraint_violations(self) -> Tuple[Optional[List[int]], Optional[List[List[int]]]]:
        ...
        return (
            ...
            self.computation_metrics("constraint_violation_indexes")
        )


    def constraint_violations_ratio(self) -> List[float]:
        """
        The ratio of violated constraints for each engine (``num_engines``).
        If no constraints were used in the computation, all ratios will be equal to 0.

        Returns
        -------
        The ratio of (constraints violations) / (total constraints).
        """
        constraint_weights_shape: Tuple[int, int] = self.original_input_params("constraint_weights_shape")
        total_constraint_violations: List[int] = self.computation_metrics("total_constraint_violations")

        return [count / constraint_weights_shape[0] for count in total_constraint_violations]
