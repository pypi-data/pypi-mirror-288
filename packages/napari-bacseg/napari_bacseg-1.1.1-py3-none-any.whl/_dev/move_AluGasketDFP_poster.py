...
files = glob(r"D:\ScanR Data\ConorDFP_Gasket_Poster\**\*ome.xml")

file_data = [read_xml([file]) for file in files]
upload_data = pd.concat([get_upload_data(dat) for dat in file_data])


# upload_data = upload_data[["file_name","well_position","well_position_name","well_position_name_transpose","fov_index", "antibiotic","abxconcentration", "folder","usermeta2","usermeta3", "usermeta4"]]

upload_data = upload_data.drop_duplicates(subset=['file_name','well_position_name_transpose', 'fov_index'], keep = "first")

# upload_data = upload_data.drop_duplicates()
...
# user_metadata = user_metadata[user_metadata["user_meta1"] != "Direct From Plate GASKET POSTER"]
