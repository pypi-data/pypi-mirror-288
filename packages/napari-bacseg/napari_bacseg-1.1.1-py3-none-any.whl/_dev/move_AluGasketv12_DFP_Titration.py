    ...
    try:
        ...
        for path in paths:
            ...
            with open(path) as fd:
                ...
                position_list = [img["@Name"].split("(")[-1].split(")")[0] for img in image_list]
                column_list = np.unique([pos[0] for pos in position_list]).tolist()
                row_list = np.unique([pos[1:] for pos in position_list]).tolist()
                
                horizonal_pos_transform = {}
                for col in column_list:
                    for row in row_list:
                        horizonal_pos_transform[str(col)+str(row)] = str(col) + str(len(row_list) + 1 - int(row))
                        
                
                vertical_pos_transform = {}
                for col in column_list:
                    for row in row_list:
                        col_transpose = chr(ord('@')+int(row))
                        row_transpose = ord(col.lower()) - 96
                        vertical_pos_transform[str(col).upper()+str(row)] = str(col_transpose) + str(row_transpose)
                ...
                for i in range(len(image_list)):
                    ...
                    well_name = img["@Name"]
                    well_shortname = img["@Name"].split("(")[-1].split(")")[0]
                    ...
                    for j in range(len(img["Pixels"]["TiffData"])):
                        ...
                        try:
                            ...
                            horizonal_position = horizonal_pos_transform[well_shortname]
                            vertical_position = vertical_pos_transform[well_shortname]
                        ...
                        files[file_name] = dict(file_name=file_name,
                                                ...
                                                well_position_name_vertical = vertical_position,
                                                well_position_name_horizonal = horizonal_position,
...
def get_upload_data(file_data):
    ...
    for file_name,data in file_data.items():
        ...
        well_position_name = data["well_position_name_horizonal"]
        folder = data["folder"]
        ...
        if column == "A":
            repeat = 1
            usermeta3 = f"BioRepeat1"
        elif column == "B":
            repeat = 2
            usermeta3 = f"BioRepeat2"
        elif column == "C":
            repeat = 3
            usermeta3 = f"BioRepeat3"
        elif column == "D":
            repeat = 4
            usermeta3 = f"BioRepeat4"
             
        usermeta2 = "plate1"

        if row == 1:
            antibiotic = None
            antibiotic_contration = None
        if row == 2:
        ...
        if row == 4:
            antibiotic = "Ciprofloxacin"
            antibiotic_contration = "20XEUCAST"
        if row == 5:
        ...
        treatment_time = folder.split("_")[3].replace("min","")
        ...
        if data["time_index"] == 0:
            ...
            file_data[file_name]["treatmenttime"] = treatment_time
            file_data[file_name]["repeat"] = repeat
            file_data[file_name]["usermeta1"] = "Direct From Plate 100X Titration"
            file_data[file_name]["usermeta2"] = usermeta2
            file_data[file_name]["usermeta3"] = usermeta3
            file_data[file_name]["usermeta4"] = ""
...
files = glob(r"D:\Titration\**\*ome.xml")
...
# upload_data = upload_data.sort_values(["folder", "well_index","position_index"])


# phenotype_data = upload_data[["strain","phenotype","antibiotic","abxconcentration","treatmenttime"]].drop_duplicates()
