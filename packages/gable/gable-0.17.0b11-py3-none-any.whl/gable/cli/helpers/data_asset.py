...
def gather_pyspark_asset_data(
    ...
    logger.debug("Raw python_path.stdout:", python_path.stdout)
    logger.debug("Stripped python_path.stdout:", python_path.stdout.strip())
    logger.debug("Current Working Directory:", os.getcwd())

    # Attempt to get the absolute path
    python_executable_path = os.path.abspath(python_path.stdout.strip())

    # Debug print statement for the absolute path
    logger.debug("Absolute python_executable_path:", python_executable_path)

    prepare_npm_environment(client)
    # Run SCA, get back the results
    sca_results = run_sca_pyspark(
        project_root=project_root,
        python_executable_path=python_executable_path,  # Get the absolute path if the python executable is from a virtual environment
