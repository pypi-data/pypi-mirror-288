...
def _delvewheel_patch_1_7_1():
    ...
    if sys.version_info[:2] >= (3, 8) and not is_conda_cpython or sys.version_info[:2] >= (3, 10):
        ...
        load_order_filepath = os.path.join(libs_dir, '.load-order-threedigrid_builder-1.16.0')
        if os.path.isfile(load_order_filepath):
            with open(os.path.join(libs_dir, '.load-order-threedigrid_builder-1.16.0')) as file:
