...
class Cube(ReprJsonable):
    ...
    @property
    def _base_table_identifier(self) -> TableIdentifier:
        assert self._atoti_client._graphql_client
        cube = check_named_object_defined(
            self._atoti_client._graphql_client.get_cube_base_table_name(
                cube_name=self.name,
            ).cube,
            "cube",
            self.name,
        )
        return TableIdentifier(cube.base_table.name)
