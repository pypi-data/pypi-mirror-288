...
class AzureDevopsAdapter(GitAdapter):
    ...
    def _project_name_from_repo(self, standardized_repo: StandardizedRepository) -> str:
        '''
        A repo URL looks something like:
        https://dev.azure.com/channeladvisor/REL/_git/azure_pipeline_test

        We need the project name (in that URL, it's "REL") to compose requests to other
        API endpoints. An Azure Dev Ops "project" does not exist in our Django ORM, and we
        only need it when querying for raw data. We can extract it from a repositories URL,
        using this helper function
        '''
        repo_url = standardized_repo.url  # self.repo_id_to_url[standardized_repo.id]
        try:
            ret = re.search(r'^https://.*/.*/(.*)/_git/.*', repo_url)
            return ret.groups()[0]
        except Exception:
            raise ValueError(f'Unexpected URL format: {repo_url}')
    ...
    def get_users(
        ...
        for i, api_user in enumerate(
            self.client.get_graph_users(org_name=standardized_org.login), start=1
        ):
            yield _standardize_graph_user(api_user)
...
def _standardize_team(api_team: Dict, api_team_members: List[Dict]) -> StandardizedTeam:
    members = [_standardize_graph_user(member) for member in api_team_members]
...
def _standardize_git_user_date(user: Dict) -> StandardizedUser:
...
def _standardize_commit(
    ...
    author = _standardize_git_user_date(api_commit['author'])
