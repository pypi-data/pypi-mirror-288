...
class ProjectStructureGenerator:
    ...
    @staticmethod
    def update_template_script(project_name):
        import fileinput
        import sys
        pyspark_template_path = os.path.join(os.getcwd(), project_name, 'scripts', f'{project_name}.py')

        if os.path.exists(pyspark_template_path):
            with fileinput.FileInput(pyspark_template_path, inplace=True) as file:
                for line in file:
                    # Replace '/workflows/pyspark_template/' with the actual path
                    line = line.replace('/workflows/pyspark_template/', f'/workflows/{project_name}/')
                    sys.stdout.write(line)
            logging.info(f"Updated pyspark_template.py with project-specific paths.")
        else:
    ...
    @staticmethod
    def update_template_config(configs, full_config_file_path, project_name):
