...
from infomark_cli.config.menu_text import RICH_TAGS
...
class Console:
    ...
    def print(self, *args, **kwargs):
        ...
        if kwargs and "text" in kwargs and "menu" in kwargs:
            if not self._settings.TEST_MODE:
                if self._settings.ENABLE_RICH_PANEL:
                    if self._settings.SHOW_VERSION:
                        version = self._settings.VERSION
                        version = f"[param]Infomark Platform CLI v{version}[/param]"
                    else:
                        version = (
                            "[param]Infomark Platform CLI[/param] "
                        )
                    self._console.print(
                        panel.Panel(
                            "\n" + kwargs["text"],
