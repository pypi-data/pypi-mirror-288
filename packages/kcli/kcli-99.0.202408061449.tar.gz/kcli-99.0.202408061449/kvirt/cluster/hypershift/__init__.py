...
def scale(config, plandir, cluster, overrides):
    ...
    if os.popen(f"oc get nodepool -n {namespace} {nodepool} -o name").read() == "":
        ...
        return {'result': 'failure', 'reason': msg}
    total_cmd = f'oc get nodepool -n {namespace} -o '
    total_cmd += 'jsonpath="{range .items[?(@.spec.clusterName==\\"%s\\")]}{.spec.replicas}{\'\\n\'}{end}"' % cluster
    total_cmd += " | awk '{sum += $1} END {print sum}'"
    total_replicas = int(os.popen(total_cmd).read())
