...
class Corps(Base):
    ...
    def pick_rnd_x_code(count: int) -> list:
        ...
        """
        return random.sample(Corps.list_all_codes(), count)
...
class C1034(Corps, ABC):
    ...
    @abstractmethod
    def find(self, row_title: str, allow_empty=False) -> dict:
        pass
...
class C104(C1034):
    ...
    def find(self, row_title: str, remove_yoy=False) -> dict:
        """
        :param row_title: 해당하는 항목을 반환한다.
        :param remove_yoy: 전분기대비, 전년대비를 삭제할지 말지
        :return: 중복된 항목은 합쳐서 딕셔너리로 반환하고 중복된 갯수를 정수로 반환
