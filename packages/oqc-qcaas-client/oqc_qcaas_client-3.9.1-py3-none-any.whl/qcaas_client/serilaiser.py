...
def json_dumps(*args, serializable_types=None, **kwargs):
    ...
    json_string = json_string.replace(
        "qcaas_client.compiler_config", "qat.purr.compiler.config"
    )
    return json_string
...
def json_dump(*args, serializable_types=None, **kwargs):
    kwargs.setdefault("cls", CustomJSONEncoder)
    json_string = json.dump(*args, serializable_types=serializable_types, **kwargs)
    json_string = json_string.replace(
        "qcaas_client.compiler_config", "qat.purr.compiler.config"
    )
    return json_string
            ...
            for old_path in old_paths:
                if old_path in obj_type:
                    obj_type = obj_type.replace(
                        old_path, "qcaas_client.compiler_config"
                    )
                    break
