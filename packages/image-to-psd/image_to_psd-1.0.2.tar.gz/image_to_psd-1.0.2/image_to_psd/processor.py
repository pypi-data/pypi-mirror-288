    ...
    try:
        with WandImage() as psd:
            for i, layer in enumerate(layers):
                ...
                img.save(img_path)
                print_yellow(f"Saved image {img_path}")

                if not os.path.exists(img_path):
                    print_red(f"Image {img_path} does not exist")
                    continue
                ...
                if save_individual_layers:
                    ...
                    print_cyan(f"Saved individual layer to {individual_layer_path}")

                os.remove(img_path)

            psd.format = 'psd'
            psd.save(filename=output_path)
            print_green(f"Saved PSD to {output_path}")
