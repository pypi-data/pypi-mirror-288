...
class TestFormTypes:
    ...
    def test_method_list(self, client: EndexFactsetGlobalFilings) -> None:
        form_type = client.filings.references.form_types.list()
        assert_matches_type(FormTypeListResponse, form_type, path=["response"])

    @parametrize
    def test_raw_response_list(self, client: EndexFactsetGlobalFilings) -> None:
        response = client.filings.references.form_types.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        form_type = response.parse()
        assert_matches_type(FormTypeListResponse, form_type, path=["response"])

    @parametrize
    def test_streaming_response_list(self, client: EndexFactsetGlobalFilings) -> None:
        with client.filings.references.form_types.with_streaming_response.list() as response:
...
class TestAsyncFormTypes:
    parametrize = pytest.mark.parametrize("async_client", [False, True], indirect=True, ids=["loose", "strict"])

    @parametrize
    async def test_method_list(self, async_client: AsyncEndexFactsetGlobalFilings) -> None:
        form_type = await async_client.filings.references.form_types.list()
        assert_matches_type(FormTypeListResponse, form_type, path=["response"])

    @parametrize
    async def test_raw_response_list(self, async_client: AsyncEndexFactsetGlobalFilings) -> None:
        response = await async_client.filings.references.form_types.with_raw_response.list()

        assert response.is_closed is True
        assert response.http_request.headers.get("X-Stainless-Lang") == "python"
        form_type = await response.parse()
        assert_matches_type(FormTypeListResponse, form_type, path=["response"])

    @parametrize
    async def test_streaming_response_list(self, async_client: AsyncEndexFactsetGlobalFilings) -> None:
        async with async_client.filings.references.form_types.with_streaming_response.list() as response:
            assert not response.is_closed
            assert response.http_request.headers.get("X-Stainless-Lang") == "python"
