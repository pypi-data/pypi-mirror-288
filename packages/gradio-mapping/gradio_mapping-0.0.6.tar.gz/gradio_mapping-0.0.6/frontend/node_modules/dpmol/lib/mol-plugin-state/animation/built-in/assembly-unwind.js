...
exports.AnimateAssemblyUnwind = model_1.PluginStateAnimation.create({
    ...
    setup: function (params, _, plugin) {
        var state = plugin.state.data;
        var root = !params.target || params.target === 'all'
            ? mol_state_1.StateTransform.RootRef
            : params.target;
        var reprs = state.select(mol_state_1.StateSelection.Generators.ofType(objects_1.PluginStateObject.Molecule.Structure.Representation3D, root));
        ...
        for (var _i = 0, reprs_1 = reprs; _i < reprs_1.length; _i++) {
            var r = reprs_1[_i];
            var unwinds = state.select(mol_state_1.StateSelection.Generators.ofTransformer(transforms_1.StateTransforms.Representation
                .UnwindStructureAssemblyRepresentation3D, r.transform.ref));
            if (unwinds.length > 0)
                continue;
            changed = true;
            update
                ...
                .UnwindStructureAssemblyRepresentation3D, { t: 0 }, { tags: 'animate-assembly-unwind' });
        }
        if (!changed)
            return;
    ...
    teardown: function (_, __, plugin) {
        var state = plugin.state.data;
        var reprs = state.select(mol_state_1.StateSelection.Generators.ofType(objects_1.PluginStateObject.Molecule.Structure.Representation3DState).withTag('animate-assembly-unwind'));
        if (reprs.length === 0)
            return;
        var update = state.build();
            ...
            return (0, tslib_1.__generator)(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        root = !ctx.params.target || ctx.params.target === 'all'
                            ? mol_state_1.StateTransform.RootRef
                            : ctx.params.target;
                        ...
                        return [4 /*yield*/, commands_1.PluginCommands.State.Update(ctx.plugin, {
                                state: state,
                                tree: update,
