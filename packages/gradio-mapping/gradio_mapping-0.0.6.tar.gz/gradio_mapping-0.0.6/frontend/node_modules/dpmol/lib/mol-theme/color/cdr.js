...
exports.CDRColorThemeProvider = exports.CDRColorTheme = exports.elementSymbolColor = exports.getElementSymbolColorThemeParams = exports.ElementSymbolColorThemeParams = exports.ElementSymbolColors = void 0;
...
function CDRColorTheme(ctx, props) {
    ...
    function color(location) {
        if (structure_1.StructureElement.Location.is(location)) {
            if (structure_1.Unit.isAtomic(location.unit)) {
                var cdr = location.unit.model.atomicHierarchy.atoms.cdr;
                var auth_asym_id = location.unit.model.atomicHierarchy.chains.auth_asym_id;
                var val = cdr.value(location.element);
                var chainId = auth_asym_id.value(location.unit.chainIndex[location.element]);
                if (val !== 0) {
                    if (val === 3 && chainId.toLocaleUpperCase() === 'H')
                        return color_1.Color.fromRgb(252, 180, 91);
                    return color_1.Color.fromRgb(153, 154, 235);
                }
                if (chainId.toLocaleUpperCase() === 'L') {
                    return color_1.Color.fromRgb(145, 225, 192);
                }
                if (chainId.toLocaleUpperCase() === 'H') {
                    return color_1.Color.fromRgb(148, 223, 247);
                }
                return (0, color_1.Color)(0xFFFFFF);
            }
        }
        else if (structure_1.Bond.isLocation(location)) {
            if (structure_1.Unit.isAtomic(location.aUnit)) {
                var cdr = location.aUnit.model.atomicHierarchy.atoms.cdr;
                var auth_asym_id = location.aUnit.model.atomicHierarchy.chains.auth_asym_id;
                var val = cdr.value(location.aUnit.elements[location.aIndex]);
                var chainId = auth_asym_id.value(location.aUnit.chainIndex[location.aIndex]);
                if (val !== 0) {
                    if (val === 3 && chainId.toLocaleUpperCase() === 'H')
                        return color_1.Color.fromRgb(252, 180, 91);
                    return color_1.Color.fromRgb(153, 154, 235);
...
exports.CDRColorTheme = CDRColorTheme;
exports.CDRColorThemeProvider = {
    name: 'cdr',
    label: 'CDR',
