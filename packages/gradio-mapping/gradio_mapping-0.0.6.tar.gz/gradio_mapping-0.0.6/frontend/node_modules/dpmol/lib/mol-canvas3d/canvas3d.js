...
exports.Canvas3DParams = {
    camera: param_definition_1.ParamDefinition.Group({
        mode: param_definition_1.ParamDefinition.Select('orthographic', param_definition_1.ParamDefinition.arrayToOptions(['perspective', 'orthographic']), { label: 'Camera' }),
    ...
    cameraFog: param_definition_1.ParamDefinition.MappedStatic('on', {
        on: param_definition_1.ParamDefinition.Group({
            intensity: param_definition_1.ParamDefinition.Numeric(50, { min: 1, max: 100, step: 1 })
        }),
        off: param_definition_1.ParamDefinition.Group({})
...
(function (Canvas3DContext) {
    ...
    function fromCanvas(canvas, attribs) {
        ...
        var a = (0, tslib_1.__assign)((0, tslib_1.__assign)({}, Canvas3DContext.DefaultAttribs), attribs);
        ...
        var input = input_observer_1.InputObserver.fromElement(canvas, {
            pixelScale: pixelScale,
            preventGestures: true
        });
        var webgl = (0, context_1.createContext)(gl, { pixelScale: pixelScale });
        var passes = new passes_1.Passes(webgl, attribs);
        ...
        return {
            ...
            handlewWebglContextRestored: handlewWebglContextRestored
        };
    }
...
var requestAnimationFrame = typeof window !== 'undefined'
    ...
    : function (f) {
        return setImmediate(function () { return f(Date.now()); });
    };
...
(function (Canvas3D) {
    function create(_a, props) {
        ...
        var p = (0, tslib_1.__assign)((0, tslib_1.__assign)({}, exports.DefaultCanvas3DParams), props);
        ...
        var height = 128;
        updateViewport();
        var scene = scene_1.Scene.create(webgl, passes.draw.wboitEnabled
            ? render_item_1.GraphicsRenderVariantsWboit
            : render_item_1.GraphicsRenderVariantsBlended);
        ...
        function render(force) {
            ...
            if (x > gl.drawingBufferWidth ||
                x + width < 0 ||
                y > gl.drawingBufferHeight ||
                y + height < 0)
                return false;
            var markingUpdated = resolveMarking();
            var didRender = false;
        ...
        function getPosition(x, y) {
            var cam = camera;
            return webgl.isContextLost ? undefined : pickHelper.getPosition(x, y, cam);
        }
        ...
        function resolveCameraReset() {
            if (!cameraResetRequested || controls.isLockCamera)
                return;
            ...
            if (autoAdjustControls.name === 'on') {
                var minDistance = autoAdjustControls.params.minDistanceFactor * radius +
            ...
            if (radius > 0) {
                var duration = nextCameraResetDuration === undefined
                    ? p.cameraResetDurationMs
                    : nextCameraResetDuration;
                var focus_1 = camera.getFocus(center, radius);
                var next = typeof nextCameraResetSnapshot === 'function'
                    ? nextCameraResetSnapshot(scene, camera)
                    : nextCameraResetSnapshot;
                var snapshot = next ? (0, tslib_1.__assign)((0, tslib_1.__assign)({}, focus_1), next) : focus_1;
                camera.setState((0, tslib_1.__assign)((0, tslib_1.__assign)({}, snapshot), { radiusMax: scene.boundingSphere.radius }), duration);
            }
            nextCameraResetDuration = void 0;
            nextCameraResetSnapshot = void 0;
            cameraResetRequested = false;
        ...
        function shouldResetCamera() {
            ...
            for (var _i = 0, _a = scene.renderables; _i < _a.length; _i++) {
                ...
                if ((cameraDist > cameraSphere.radius ||
                    cameraDist > b.radius ||
                    b.radius > camera.state.radiusMax) &&
                    !geometry_1.Sphere3D.includes(oldBoundingSphereVisible, b))
                    return true;
        ...
        function getProps() {
            var radius = scene.boundingSphere.radius > 0
                ? 100 -
                    Math.round((camera.transition.target.radius / scene.boundingSphere.radius) *
                        100)
                : 0;
            return {
                camera: {
                    mode: camera.state.mode,
                    helper: (0, tslib_1.__assign)({}, helper.camera.props),
                    stereo: (0, tslib_1.__assign)({}, p.camera.stereo),
                ...
                viewport: p.viewport,
                postprocessing: (0, tslib_1.__assign)({}, p.postprocessing),
                marking: (0, tslib_1.__assign)({}, p.marking),
                multiSample: (0, tslib_1.__assign)({}, p.multiSample),
                renderer: (0, tslib_1.__assign)({}, renderer.props),
                trackball: (0, tslib_1.__assign)({}, controls.props),
                interaction: (0, tslib_1.__assign)({}, interactionHelper.props),
                debug: (0, tslib_1.__assign)({}, helper.debug.props),
                handle: (0, tslib_1.__assign)({}, helper.handle.props)
            };
        }
        ...
        return {
            ...
            setProps: function (properties, doNotRequestDraw) {
                ...
                if (props.cameraClipping !== undefined) {
                    if (props.cameraClipping.far !== undefined &&
                        props.cameraClipping.far !== camera.state.clipFar) {
                        cameraState.clipFar = props.cameraClipping.far;
                    }
                    if (props.cameraClipping.radius !== undefined) {
                        var radius = (scene.boundingSphere.radius / 100) *
                ...
                if (props.viewport !== undefined) {
                    var doNotUpdate = p.viewport === props.viewport ||
                        (p.viewport.name === props.viewport.name &&
                            (0, mol_util_1.shallowEqual)(p.viewport.params, props.viewport.params));
                    if (!doNotUpdate) {
            ...
            getImagePass: function (props) {
                if (props === void 0) { props = {}; }
                return new image_1.ImagePass(webgl, renderer, scene, camera, helper, passes.draw.wboitEnabled, props);
            },
            ...
            if (p.viewport.name === 'canvas') {
                ...
                y =
                    gl.drawingBufferHeight -
                        height -
                        p.viewport.params.y * webgl.pixelRatio;
                ...
                y = Math.round(gl.drawingBufferHeight -
                    height -
                    p.viewport.params.y * gl.drawingBufferHeight);
