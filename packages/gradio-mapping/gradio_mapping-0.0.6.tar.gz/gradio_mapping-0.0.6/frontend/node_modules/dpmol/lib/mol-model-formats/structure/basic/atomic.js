...
function findHierarchyOffsets(atom_site) {
    ...
    for (var i = (start + 1); i < end; i++) {
        var newChain = !label_entity_id.areValuesEqual(i - 1, i) ||
...
function createHierarchyData(atom_site, sourceIndex, offsets) {
    var atoms = db_1.Table.ofColumns(atomic_1.AtomsSchema, {
        type_symbol: db_1.Column.ofArray({
            array: db_1.Column.mapToArray(atom_site.type_symbol, types_1.ElementSymbol),
            schema: db_1.Column.Schema.Aliased(db_1.Column.Schema.str)
        }),
        ...
        em_confidence: atom_site.em_confidence,
    });
...
function getConformation(atom_site) {
    return {
        ...
        occupancy: atom_site.occupancy.isDefined
            ? atom_site.occupancy
            : db_1.Column.ofConst(1, atom_site._rowCount, db_1.Column.Schema.float),
        B_iso_or_equiv: atom_site.B_iso_or_equiv,
...
function createChainOperatorMappingAndSubstituteNames(hierarchy, format) {
    ...
    hierarchy.chains.label_asym_id = db_1.Column.ofArray({
        array: mappedLabel,
        valueKind: hierarchy.chains.label_asym_id.valueKind,
        schema: hierarchy.chains.label_asym_id.schema
    });
    hierarchy.chains.auth_asym_id = db_1.Column.ofArray({
        array: mappedAuth,
