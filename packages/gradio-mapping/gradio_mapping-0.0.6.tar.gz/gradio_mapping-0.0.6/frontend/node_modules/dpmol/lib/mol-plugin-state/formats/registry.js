...
var DataFormatRegistry = /** @class */ (function () {
    function DataFormatRegistry() {
        ...
        for (var _b = 0, BuiltInCoordinatesFormats_1 = coordinates_1.BuiltInCoordinatesFormats; _b < BuiltInCoordinatesFormats_1.length; _b++) {
            var _c = BuiltInCoordinatesFormats_1[_b], id = _c[0], p = _c[1];
            this.add(id, p);
        }
        for (var _d = 0, BuiltInTrajectoryFormats_1 = trajectory_1.BuiltInTrajectoryFormats; _d < BuiltInTrajectoryFormats_1.length; _d++) {
            var _e = BuiltInTrajectoryFormats_1[_d], id = _e[0], p = _e[1];
            this.add(id, p);
        }
    }
    ...
    DataFormatRegistry.prototype.auto = function (info, dataStateObject) {
        for (var i = 0, il = this.list.length; i < il; ++i) {
            ...
            if (provider.binaryExtensions &&
                provider.binaryExtensions.indexOf(info.ext) >= 0)
                hasExt = true;
            else if (provider.stringExtensions &&
                provider.stringExtensions.indexOf(info.ext) >= 0)
                hasExt = true;
            if (hasExt &&
                (!provider.isApplicable ||
                    provider.isApplicable(info, dataStateObject.data)))
                return provider;
        }
        return;
    };
