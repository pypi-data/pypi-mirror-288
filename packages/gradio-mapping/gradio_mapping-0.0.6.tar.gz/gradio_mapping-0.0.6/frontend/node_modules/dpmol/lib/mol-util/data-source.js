        ...
        data.onprogress = function (e) {
            ...
            try {
                if (e.lengthComputable) {
                    ...
                    ctx.update({
                        message: "".concat(action, " ").concat((e.loaded / 1024 / 1024).toFixed(2), " MB"),
            ...
            switch (_a.label) {
                case 0:
                    ...
                    compression = type === 'zip'
                        ? DataCompressionMethod.None
                        : getCompression(file.name);
                    if (type === 'binary' ||
                        type === 'zip' ||
                        compression !== DataCompressionMethod.None) {
            ...
            switch (_a.label) {
                ...
                case 1:
                    ...
                    for (_i = Math.min(len, maxConcurrency); currentSrc < _i; currentSrc++) {
                        ...
                        promises.push(wrapPromise(currentSrc, current.id, assetManager
                            .resolve(assets_1.Asset.getUrlAsset(assetManager, current.url), current.isBinary ? 'binary' : 'string')
                            .runAsChild(ctx)));
