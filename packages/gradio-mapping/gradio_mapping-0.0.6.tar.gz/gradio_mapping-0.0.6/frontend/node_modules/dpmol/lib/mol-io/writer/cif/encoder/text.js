...
var TextEncoder = /** @class */ (function () {
    ...
    TextEncoder.prototype.startDataBlock = function (header) {
        ...
        mol_util_1.StringBuilder.write(this.builder, "data_".concat((header || '').replace(/[ \n\t]/g, '').toUpperCase(), "\n#\n"));
    };
...
function getFloatPrecisions(categoryName, fields, formatter) {
    ...
    for (var _a = 0, fields_1 = fields; _a < fields_1.length; _a++) {
        ...
        if (format && typeof format.digitCount !== 'undefined')
            ...
            ret[ret.length] =
                f.type === 2 /* Float */ ? Math.pow(10, (0, util_1.getFieldDigitCount)(f)) : 0;
    }
    return ret;
}
function writeCifSingleRecord(category, instance, source, builder, filter, formatter) {
    ...
    for (var _f = 0; _f < fields.length; _f++) {
        ...
        if (!filter.includeField(category.name, f.name))
            continue;
        mol_util_1.StringBuilder.writePadRight(builder, "_".concat(category.name, ".").concat(f.name), width);
...
function writeCifLoop(category, instance, source, builder, filter, formatter) {
    ...
    var fields = filter === encoder_1.Category.DefaultFilter
        ? fieldSource
        : fieldSource.filter(function (f) { return filter.includeField(category.name, f.name); });
...
function writeLine(builder, val) {
    mol_util_1.StringBuilder.write(builder, val);
    mol_util_1.StringBuilder.newline(builder);
}
function writeInteger(builder, val) {
    mol_util_1.StringBuilder.writeInteger(builder, val);
    mol_util_1.StringBuilder.whitespace1(builder);
}
function writeFloat(builder, val, precisionMultiplier) {
    mol_util_1.StringBuilder.writeFloat(builder, val, precisionMultiplier);
    mol_util_1.StringBuilder.whitespace1(builder);
}
function writeNotPresent(builder) {
    mol_util_1.StringBuilder.writeSafe(builder, '. ');
}
...
function writeChecked(builder, val) {
    ...
    if (escape) {
        mol_util_1.StringBuilder.writeSafe(builder, escapeKind ? '"' : "'");
        mol_util_1.StringBuilder.writeSafe(builder, val);
        mol_util_1.StringBuilder.writeSafe(builder, escapeKind ? '" ' : "' ");
    }
    else {
        mol_util_1.StringBuilder.writeSafe(builder, val);
