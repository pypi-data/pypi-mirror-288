...
function otherLabel(count, location, granularity, hidePrefix, reverse, condensed) {
    return "".concat(elementLabel(location, {
        granularity: granularity,
        hidePrefix: hidePrefix,
        reverse: reverse,
        condensed: condensed
    }), " <small>[+ ").concat(countLabel(count - 1, "other ".concat((0, string_1.capitalize)(granularity))), "]</small>");
}
...
function getResidueCount(unit) {
    ...
    return (residueAtomSegments.index[elementEnd] -
        residueAtomSegments.index[elementStart] +
        1);
}
function structureElementStatsLabel(stats, options) {
    if (options === void 0) { options = {}; }
    var o = (0, tslib_1.__assign)((0, tslib_1.__assign)({}, exports.DefaultLabelOptions), options);
...
function _structureElementStatsLabel(stats, countsOnly, hidePrefix, condensed, reverse) {
    ...
    if (!countsOnly &&
        ...
        var granularity = structure_1.Unit.isAtomic(unit) && getResidueCount(unit) === 1
            ? 'residue'
            : structure_1.Unit.Traits.is(unit.traits, 1 /* MultiChain */)
                ? 'residue'
                : 'chain';
        ...
        if (conformationCount > 0) {
            label.push(conformationCount === 1
...
function bondLabel(bond, options) {
    ...
    return bundleLabel({
        loci: [
            structure_1.StructureElement.Loci(bond.aStructure, [
                { unit: bond.aUnit, indices: int_1.OrderedSet.ofSingleton(bond.aIndex) }
            ]),
            structure_1.StructureElement.Loci(bond.bStructure, [
...
function _bundleLabel(bundle, options) {
    ...
    if (isSingleElements) {
        ...
        if (condensed) {
            return labels
                .map(function (l) { return l[0].replace(/\[.*\]/g, '').trim(); })
                .filter(function (l) { return !!l; })
                .join(' \u2014 ');
        }
        var offset = 0;
        ...
        if (offset > 0) {
            ...
            return labels
                .map(function (l) { return l.filter(function (l) { return !!l; }).join(' | '); })
                .filter(function (l) { return !!l; })
                .join('</br>');
        }
    }
...
function elementLabel(location, options, forLoci) {
    var _a, _b;
    if (options === void 0) { options = {}; }
    var o = (0, tslib_1.__assign)((0, tslib_1.__assign)({}, exports.DefaultLabelOptions), options);
    ...
    // CHANGED BY XUHANG
    var label = (o.condensed
        ? (_b = (_a = _label[0]) === null || _a === void 0 ? void 0 : _a.replace(/\[.*\]/g, '').trim()) !== null && _b !== void 0 ? _b : ''
        : _label.filter(function (l) { return !!l; }).join(' | ')) +
        (forLoci
            ? plddtLabel(structure_1.StructureProperties.atom.B_iso_or_equiv(location))
            : '');
    return o.htmlStyling ? label : (0, string_1.stripTags)(label);
}
...
/* uniFold功能中plddt的展示 */
// ADDED BY XUHANG
function plddtLabel(plddt) {
    if (plddt >= 90) {
        return "<div><small>Plddt ".concat(plddt, " (Very high)</small></div>");
    }
    else if (plddt < 90 && plddt >= 70) {
        return "<div><small>Plddt ".concat(plddt, " (Confident)</small></div>");
    }
    else if (plddt < 70 && plddt >= 50) {
        return "<div><small>Plddt ".concat(plddt, " (Low)</small></div>");
    }
    else if (plddt < 50) {
        return "<div><small>Plddt ".concat(plddt, " (Very low)</small></div>");
    }
}
...
function _atomicElementLabel(location, granularity, hideOccupancy) {
    ...
    var has_label_seq_id = location.unit.model.atomicHierarchy.residues.label_seq_id.valueKind(rI) ===
        0 /* Present */;
    ...
    var id = structure_1.StructureProperties.atom.id(location);
    ...
    var compId = granularity === 'residue' && microHetCompIds.length > 1
        ? "(".concat(microHetCompIds.join('|'), ")")
        : comp_id;
    var label = [];
    switch (granularity) {
        case 'element':
            label.push("<b>".concat(atom_id, " ").concat(id, "</b>").concat(alt_id ? "%".concat(alt_id) : ''));
        case 'conformation':
            if (granularity === 'conformation' && alt_id) {
                label.push("<small>Conformation</small> <b>".concat(alt_id, "</b>"));
            }
        case 'residue':
            var seq_id = label_seq_id === auth_seq_id && has_label_seq_id
                ? label_seq_id
                : auth_seq_id;
            label.push("<b>".concat(compId, " ").concat(seq_id, "</b>").concat(seq_id !== label_seq_id
                ? " <small>[auth</small> <b>".concat(label_seq_id, "</b><small>]</small>")
                : '', "<b>").concat(ins_code ? ins_code : '', "</b>"));
        case 'chain':
            if (label_asym_id === auth_asym_id) {
                ...
                if (granularity === 'chain' &&
                    structure_1.Unit.Traits.is(location.unit.traits, 1 /* MultiChain */)) {
...
function distanceLabel(pair, options) {
    if (options === void 0) { options = {}; }
    var o = (0, tslib_1.__assign)((0, tslib_1.__assign)((0, tslib_1.__assign)({}, exports.DefaultLabelOptions), { measureOnly: false, unitLabel: '\u212B' }), options);
    ...
    return o.condensed
        ? "".concat(distance, " | ").concat(label)
        : "Distance ".concat(distance, "</br>").concat(label);
}
exports.distanceLabel = distanceLabel;
function angleLabel(triple, options) {
    if (options === void 0) { options = {}; }
    var o = (0, tslib_1.__assign)((0, tslib_1.__assign)((0, tslib_1.__assign)({}, exports.DefaultLabelOptions), { measureOnly: false }), options);
...
function dihedralLabel(quad, options) {
    ...
    return o.condensed
        ? "".concat(dihedral, " | ").concat(label)
        : "Dihedral ".concat(dihedral, "</br>").concat(label);
}
