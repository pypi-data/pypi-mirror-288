...
function ofRange(min, max) {
    return max < min ? tuple_1.IntTuple.create(min, min) : tuple_1.IntTuple.create(min, max + 1);
}
exports.ofRange = ofRange;
function ofBounds(start, end) {
    return end <= start ? tuple_1.IntTuple.create(start, start) : tuple_1.IntTuple.create(start, end);
}
exports.ofBounds = ofBounds;
function ofLength(length) {
    return length < 0 ? tuple_1.IntTuple.create(0, 0) : tuple_1.IntTuple.create(0, length);
}
...
function max(i) {
    return tuple_1.IntTuple.snd(i) - 1;
}
exports.max = max;
...
function has(int, v) {
    return tuple_1.IntTuple.fst(int) <= v && v < tuple_1.IntTuple.snd(int);
}
exports.has = has;
/** Returns the index of `x` in `set` or -1 if not found. */
function indexOf(int, x) {
    var m = (0, exports.start)(int);
    return x >= m && x < (0, exports.end)(int) ? x - m : -1;
}
exports.indexOf = indexOf;
function getAt(int, i) {
    return tuple_1.IntTuple.fst(int) + i;
}
exports.getAt = getAt;
