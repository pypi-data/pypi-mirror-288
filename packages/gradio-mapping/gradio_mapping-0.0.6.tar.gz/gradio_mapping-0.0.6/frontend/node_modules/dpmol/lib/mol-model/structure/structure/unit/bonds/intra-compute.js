...
function findIndexPairBonds(unit) {
    ...
    for (var _aI = 0; _aI < atomCount; _aI++) {
        ...
        for (var i = offset[srcA], il = offset[srcA + 1]; i < il; ++i) {
            ...
            var add = false;
            // 文件中有Bond数据的格式应当完全按照文件内容走 By Xuh
            if (unit.model.sourceData.kind === 'mol' || unit.model.sourceData.kind === 'sdf' || unit.model.sourceData.kind === 'mol2') {
                add = true;
            }
            else if (d >= 0) {
...
function computeIntraUnitBonds(unit, props) {
    var p = (0, tslib_1.__assign)((0, tslib_1.__assign)({}, common_1.DefaultBondComputationProps), props);
