...
function refineInteractions(structure, interactions) {
    ...
    var hydrogenAcceptorMap = new Set();
    var hydrogenInterRingMap = new Map();
    var hydrogenIntraRingMap = new Map();
    for (var i = 0, il = contacts.edgeCount; i < il; ++i) {
        ...
        if (e.props.type === 5) {
            ...
            var info = typeA === 5 /* HydrogenAcceptor */ ? infoA : infoB;
            var elements = [];
            for (var j = info.offsets[info.feature], l = info.offsets[info.feature + 1]; j < l; j++) {
                elements.push(info.members[j]);
            }
            if (elements.length > 1) {
                hydrogenInterRingMap.set(i, elements);
            }
            if (elements.length === 1) {
                hydrogenAcceptorMap.add(elements[0]);
            }
        }
        if (unitsContacts.size) {
    ...
    while (true) {
        ...
        for (var i = 0, il = contacts_1.edgeCount * 2; i < il; ++i) {
            ...
            var type = contacts_1.edgeProps.type[i];
            if (type === 5) {
                var typeA = infoA.types[infoA.feature];
                var info = typeA === 5 /* HydrogenAcceptor */ ? infoA : infoB;
                var elements = [];
                ...
                if (elements.length > 1) {
                    hydrogenIntraRingMap.set(i, { elements: elements, value: value });
                }
                if (elements.length === 1) {
                    hydrogenAcceptorMap.add(elements[0]);
                }
    ...
    hydrogenInterRingMap.forEach(function (elements, index) {
        if (elements.filter(function (item) { return hydrogenAcceptorMap.has(item); }).length > 0) {
            contacts.edges[index].props.flag = common_1.InteractionFlag.Filtered;
        }
    });
    hydrogenIntraRingMap.forEach(function (_a, index) {
        var elements = _a.elements, value = _a.value;
        if (elements.filter(function (item) { return hydrogenAcceptorMap.has(item); }).length > 0) {
            unitsContacts.get(value).edgeProps.flag[index] = common_1.InteractionFlag.Filtered;
        }
    });
}
...
function hydrophobicRefiner(structure, interactions) {
    ...
    var setInterFiltered = function (i) {
        return (contacts.edges[i].props.flag = common_1.InteractionFlag.Filtered);
    };
    var residueIntraMap;
    var setIntraFiltered;
    return {
        isApplicable: function (type) {
            return type === common_1.InteractionType.Hydrophobic;
        },
        ...
        startUnit: function (unit, contacts, features) {
            ...
            setIntraFiltered = function (i) {
                return (contacts.edgeProps.flag[i] = common_1.InteractionFlag.Filtered);
            };
        },
...
function weakHydrogenBondsRefiner(structure, interactions) {
    ...
    var hasHydrogenBond = function (infoA, infoB) {
        var acc = infoA.types[infoA.feature] === 9 /* WeakHydrogenDonor */
            ? infoB
            : infoA;
        // check intra
        ...
        for (var i = 0, il = interIndices.length; i < il; ++i) {
            if (contacts.edges[interIndices[i]].props.type ===
                common_1.InteractionType.HydrogenBond)
                return true;
        }
        return false;
    };
    return {
        isApplicable: function (type) {
            return type === common_1.InteractionType.WeakHydrogenBond;
        },
        handleInterContact: function (index, infoA, infoB) {
...
function piStackingRefiner(structure, interactions) {
    ...
    return {
        isApplicable: function (type) {
            return type === common_1.InteractionType.Hydrophobic || type === common_1.InteractionType.CationPi;
        },
        handleInterContact: function (index, infoA, infoB) {
