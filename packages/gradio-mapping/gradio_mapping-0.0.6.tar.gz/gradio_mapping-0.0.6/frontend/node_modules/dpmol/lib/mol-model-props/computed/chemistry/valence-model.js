...
function calculateHydrogensCharge(structure, unit, index, props) {
    ...
    var assignCharge = props.assignCharge === 'always' ||
        (props.assignCharge === 'auto' && charge === 0);
    var assignH = !['sdf', 'mol'].includes(unit.model.sourceData.kind) && (props.assignH === 'always' ||
        (props.assignH === 'auto' && hydrogenCount === 0));
    var degree = (0, util_1.bondCount)(structure, unit, index);
    var valence = explicitValence(structure, unit, index); // 3
    ...
    switch (element) {
            ...
            if (assignCharge) {
                if (!assignH) {
                    ...
                    while (tmpChargeBondItA.hasNext) {
                        ...
                        if (elementB === "S" /* S */ || (0, types_1.isMetal)(elementB) || types_2.BondType.is(b.type, 32 /* Computed */)) {
                            charge = 0;
                            break;
                        }
                        else {
        ...
        case "S" /* S */:
            if (assignCharge) {
                if (!assignH) {
                    if (valence <= 3 &&
                        (0, util_1.bondToElementCount)(structure, unit, index, "O" /* O */) === 0) {
...
function calcUnitValenceModel(structure, unit, props) {
    ...
    // 把这里hasParent的逻辑去掉了，防止两个类似的蛋白文件同时在mergeStructure节点中时价态的计算值错误的问题 By Xuh
    var hasParent = false;
    var mapping;
    if (hasParent) {
...
exports.ValenceModelParams = {
    assignCharge: param_definition_1.ParamDefinition.Select('auto', [
        ['always', 'always'],
        ['auto', 'auto'],
        ['never', 'never']
    ]),
    assignH: param_definition_1.ParamDefinition.Select('auto', [
        ['always', 'always'],
        ['auto', 'auto'],
        ['never', 'never']
    ])
