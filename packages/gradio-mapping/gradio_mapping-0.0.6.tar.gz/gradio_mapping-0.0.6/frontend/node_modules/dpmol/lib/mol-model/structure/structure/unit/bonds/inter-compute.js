...
function getDistance(unitA, indexA, unitB, indexB) {
    ...
    return linear_algebra_1.Vec3.distance(tmpDistVecA, tmpDistVecB);
}
...
function findPairBonds(unitA, unitB, props, builder) {
    ...
    var indexPairs = !props.forceCompute &&
        unitA.model === unitB.model &&
        index_pair_1.IndexPairBonds.Provider.get(unitA.model);
    var sourceIndex = unitA.model.atomicHierarchy.atomSourceIndex;
    var invertedIndex = (indexPairs
        ? model_1.Model.getInvertedAtomSourceIndex(unitB.model)
        : { invertedIndex: void 0 }).invertedIndex;
    ...
    for (var _aI = 0; _aI < atomCount; _aI++) {
        var aI = atomsA[_aI];
        linear_algebra_1.Vec3.set(_imageA, xA[aI], yA[aI], zA[aI]);
        if (isNotIdentity)
            linear_algebra_1.Vec3.transformMat4(_imageA, _imageA, imageTransform);
        if (linear_algebra_1.Vec3.squaredDistance(_imageA, bCenter) > testDistanceSq)
            continue;
        ...
        var structConnEntries = props.forceCompute
            ? void 0
        ...
        for (var ni = 0; ni < count; ni++) {
            ...
            if (dist <= pairingThreshold) {
                ...
                builder.add(_aI, _bI, {
                    ...
                    flag: (isMetal
                        ? 2 /* MetallicCoordination */
                        : 1 /* Covalent */) | 32 /* Computed */
                });
            }
        }
    }
...
function findBonds(structure, props) {
    ...
    var hasIndexPairBonds = structure.models.some(function (m) {
        return index_pair_1.IndexPairBonds.Provider.get(m);
    });
    ...
    structure_1.Structure.eachUnitPair(structure, function (unitA, unitB) {
        ...
        validUnit: function (unit) { return unit_1.Unit.isAtomic(unit); },
...
function computeInterUnitBonds(structure, props) {
    var p = (0, tslib_1.__assign)((0, tslib_1.__assign)({}, DefaultInterBondComputationProps), props);
    return findBonds(structure, (0, tslib_1.__assign)((0, tslib_1.__assign)({}, p), { validUnitPair: (props && props.validUnitPair) ||
            (function (s, a, b) {
                var mtA = a.model.atomicHierarchy.derived.residue.moleculeType;
                var mtB = b.model.atomicHierarchy.derived.residue.moleculeType;
                var notWater = (!unit_1.Unit.isAtomic(a) ||
                    mtA[a.residueIndex[a.elements[0]]] !== 2 /* Water */) &&
                    (!unit_1.Unit.isAtomic(b) ||
                        mtB[b.residueIndex[b.elements[0]]] !== 2 /* Water */);
                var sameModel = a.model === b.model;
                var notIonA = (!unit_1.Unit.isAtomic(a) || mtA[a.residueIndex[a.elements[0]]] !== 3 /* Ion */) || (sameModel && hasStructConnRecord(a));
                var notIonB = (!unit_1.Unit.isAtomic(b) || mtB[b.residueIndex[b.elements[0]]] !== 3 /* Ion */) || (sameModel && hasStructConnRecord(b));
                var notIon = notIonA && notIonB;
                return (structure_1.Structure.validUnitPair(s, a, b) &&
                    (notWater || !p.ignoreWater) &&
                    (notIon || !p.ignoreIon));
            }) }));
}
