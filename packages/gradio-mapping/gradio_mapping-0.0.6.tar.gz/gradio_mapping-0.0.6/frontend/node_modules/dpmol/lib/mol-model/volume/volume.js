...
(function (Volume) {
    ...
    function createIsoValueParam(defaultValue, stats) {
        ...
        return param_definition_1.ParamDefinition.Conditioned(def, {
            'absolute': param_definition_1.ParamDefinition.Converted(function (v) { return Volume.IsoValue.toAbsolute(v, grid_1.Grid.One.stats).absoluteValue; }, function (v) { return Volume.IsoValue.absolute(v); }, param_definition_1.ParamDefinition.Numeric(mean, { min: min, max: max, step: (0, number_1.toPrecision)(sigma / 100, 2) })),
            'relative': param_definition_1.ParamDefinition.Converted(function (v) { return Volume.IsoValue.toRelative(v, grid_1.Grid.One.stats).relativeValue; }, function (v) { return Volume.IsoValue.relative(v); }, param_definition_1.ParamDefinition.Numeric(Math.min(1, relMax), { min: relMin, max: relMax, step: (0, number_1.toPrecision)(Math.round(((max - min) / sigma)) / 100, 2) }))
    ...
    (function (Cell) {
        ...
        var tmpBoundaryPos = (0, linear_algebra_1.Vec3)();
        function getBoundingSphere(volume, indices, boundingSphere) {
            ...
            for (var i = 0, _i = int_1.OrderedSet.size(indices); i < _i; i++) {
                var o = int_1.OrderedSet.getAt(indices, i);
