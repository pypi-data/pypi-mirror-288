            ...
            source: param_definition_1.ParamDefinition.MappedStatic('pdb-xray', {
                'pdb-xray': param_definition_1.ParamDefinition.Group({
                    provider: param_definition_1.ParamDefinition.Group({
                        ...
                        server: param_definition_1.ParamDefinition.Select('rcsb', [
                            ['pdbe', 'PDBe'],
                            ['rcsb', 'RCSB PDB']
                        ])
                    }, { pivot: 'id' }),
                    type: param_definition_1.ParamDefinition.Select('2fofc', [
                        ['2fofc', '2Fo-Fc'],
                        ['fofc', 'Fo-Fc']
                    ])
                }, { isFlat: true }),
                'pdb-xray-ds': param_definition_1.ParamDefinition.Group({
                    provider: param_definition_1.ParamDefinition.Group({
                        id: param_definition_1.ParamDefinition.Text('1tqn', { label: 'Id' }),
                        server: param_definition_1.ParamDefinition.Select('pdbe', [
                            ['pdbe', 'PDBe'],
                            ['rcsb', 'RCSB PDB']
                        ])
                    }, { pivot: 'id' }),
            ...
            switch (_c.label) {
                case 0:
                    ...
                    switch (src.name) {
                        ...
                        case 'pdb-xray':
                            downloadParams =
                                src.params.provider.server === 'pdbe'
                                    ? {
                ...
                case 1:
                    ...
                    switch (src.name) {
                        case 'url':
                            downloadParams = src.params;
                            provider =
                                src.params.format === 'auto'
                                    ? plugin.dataFormats.auto((0, file_info_1.getFileInfo)(assets_1.Asset.getUrl(downloadParams.url)), (_a = data.cell) === null || _a === void 0 ? void 0 : _a.obj)
...
exports.AssignColorVolume = mol_state_1.StateAction.build({
    ...
    params: function (a, plugin) {
        var cells = plugin.state.data.select(mol_state_1.StateSelection.Generators.root
            .subtree()
            .ofType(objects_1.PluginStateObject.Volume.Data)
            .filter(function (cell) { var _a; return !!cell.obj && !((_a = cell.obj) === null || _a === void 0 ? void 0 : _a.data.colorVolume) && cell.obj !== a; }));
        if (cells.length === 0)
            return { ref: param_definition_1.ParamDefinition.Text('', { isHidden: true, label: 'Volume' }) };
        return {
            ref: param_definition_1.ParamDefinition.Select(cells[0].transform.ref, cells.map(function (c) { return [c.transform.ref, c.obj.label]; }), { label: 'Volume' })
        };
    }
})(function (_a, plugin) {
    ...
    return plugin
        .build()
        .to(ref)
        .apply(transforms_1.StateTransforms.Volume.AssignColorVolume, { ref: params.ref }, { dependsOn: [params.ref] })
        .commit();
});
