...
function validPair(structure, infoA, infoB) {
    ...
    if (infoA.unit.residueIndex[infoA.unit.elements[indexA]] ===
...
function checkLineOfSight(structure, infoA, infoB, distFactor) {
    ...
    for (var r = 0; r < count; ++r) {
        ...
        if (invalidAltLoc(unit, i, infoA.unit, indexA) ||
            invalidAltLoc(unit, i, infoB.unit, indexB))
            continue;
        // allow member atoms
        // 允许相互作用力两边的原子
        if ((infoA.unit === unit && isMember(i, infoA)) ||
        ...
        if (linear_algebra_1.Vec3.squaredDistance(tmpVec, tmpVecA) < 1 ||
            linear_algebra_1.Vec3.squaredDistance(tmpVec, tmpVecB) < 1)
            continue;
        return false;
    }
    return true;
 ...
 */
function addUnitContacts(structure, unit, features, builder, testers, props, filter) {
    for (var _i = 0, testers_1 = testers; _i < testers_1.length; _i++) {
        var tester = testers_1[_i];
        _addUnitContacts(structure, unit, features, builder, tester, props, filter);
    }
}
exports.addUnitContacts = addUnitContacts;
function _addUnitContacts(structure, unit, features, builder, tester, props, filter) {
    ...
    for (var t = 0, tl = int_1.OrderedSet.size(subsetIndices); t < tl; ++t) {
        ...
        for (var r = 0; r < count; ++r) {
            ...
            if (j <= i)
                continue;
            infoB.feature = j;
            var elementIdA = infoA.unit.elements[infoA.members[infoA.offsets[infoA.feature]]];
            var elementIdB = infoB.unit.elements[infoB.members[infoB.offsets[infoB.feature]]];
            if (filter && !filter(infoA.unit, elementIdA, infoB.unit, elementIdB)) {
                continue;
            }
            if (!validPair(structure, infoA, infoB))
                continue;
            var type = tester.getType(structure, infoA, infoB, squaredDistances[r]);
            // By Xuh: 临时去掉checkLineOfSight的限制
 ...
 */
function addStructureContacts(structure, unitA, featuresA, unitB, featuresB, builder, testers, props, filter) {
    ...
    for (var i = 0; i < countA; ++i) {
        ...
        for (var r = 0; r < count; ++r) {
            var j = indices[r];
            infoB.feature = j;
            if ((filter && !filter(infoA.unit, infoA.unit.elements[infoA.members[infoA.offsets[i]]], infoB.unit, infoB.unit.elements[infoB.members[infoB.offsets[j]]])) || !validPair(structure, infoA, infoB))
                continue;
