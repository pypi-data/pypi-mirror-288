...
(function (Language) {
    ...
    (function (ASTNode) {
        ...
        function list(bracket, nodes) {
            return { kind: 'list', bracket: bracket, nodes: nodes };
        }
        ASTNode.list = list;
    ...
    var Expr = monadic_parser_1.MonadicParser.lazy(function () {
        return monadic_parser_1.MonadicParser.alt(Str, List, Symb, Comment).trim(ws);
    });
    var Str = monadic_parser_1.MonadicParser.takeWhile(function (c) { return c !== '`'; })
        .trim('`')
        .map(ASTNode.str);
        ...
        switch (expr.kind) {
            ...
            case 'symbol': {
                ...
                if (value.length > 1) {
                    ...
                    switch (fst) {
                        case '.':
                            return builder_1.MolScriptBuilder.atomName(value.substr(1));
                        case '_':
                            return builder_1.MolScriptBuilder.struct.type.elementSymbol([value.substr(1)]);
                    }
                }
                if (value === 'true')
                    return true;
            ...
            case 'list': {
                switch (expr.bracket) {
                    case '[':
                        return builder_1.MolScriptBuilder.core.type.list(withoutComments(expr.nodes).map(visitExpr));
                    case '{':
                        return builder_1.MolScriptBuilder.core.type.set(withoutComments(expr.nodes).map(visitExpr));
                    case '(': {
                    ...
                    default:
                        (0, type_helpers_1.assertUnreachable)(expr.bracket);
                }
            }
            default:
                (0, type_helpers_1.assertUnreachable)(expr);
        }
    }
