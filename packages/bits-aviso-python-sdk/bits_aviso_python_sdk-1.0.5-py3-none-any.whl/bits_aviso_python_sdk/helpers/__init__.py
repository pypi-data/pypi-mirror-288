...
def normalize_data_for_bigquery(data):
    ...
    data = replace_empty_strings_in_keys(data)
    data = replace_at_sign_in_keys(data)
    data = replace_periods_in_keys(data)
    data = replace_spaces_in_keys(data)
    data = replace_slashes_in_keys(data)

    # convert to newline delimited json
    bq_data = parse_to_nldjson(data, upload_date=True)
...
def replace_empty_strings_in_keys(data):
    """Recursively replaces empty strings in the keys of the given data dict or list of dicts with underscores.

    Args:
        data (dict, list[dict]): The data to convert.

    Returns:
        dict, list[dict]: The data with empty strings replaced by underscores in the keys.
    ...
    if isinstance(data, dict):
        new_data = {}  # new dictionary to store the data with empty strings replaced by underscores
        logging.info("Converting empty strings to underscores in dictionary keys...")
        for key, value in data.items():
            new_key = key if key else "_"
            if isinstance(value, (dict, list)):
                new_data[new_key] = replace_empty_strings_in_keys(value)
            else:
        ...
        new_data = []
        logging.info("Converting empty strings to underscores in list of dictionary keys...")
        for item in data:
            if isinstance(item, (dict, list)):
                new_data.append(replace_empty_strings_in_keys(item))
            else:
