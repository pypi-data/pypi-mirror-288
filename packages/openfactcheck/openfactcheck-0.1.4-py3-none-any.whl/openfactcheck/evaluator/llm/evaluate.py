...
from openfactcheck.evaluator.llm import data as data_dir

# Import LLM Evaluation Dataset
default_dataset_path = str(pkg_resources.files(data_dir) / "dataset.jsonl")
...
class LLMEvaluator(SnowballingEvaluator, SelfAwareEvaluator, FreshQAEvaluator, FreeTextEvaluator):
    ...
    def read_input(self):
        ...
        # Read the questions and check the number of columns
        self.logger.info(f"Reading the dataset from {self.dataset_path}...")
        df_dataset = pd.read_json(self.dataset_path, lines=True)
