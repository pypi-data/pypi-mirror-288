...
def run_cash_backtest_prices(
    prices: pl.DataFrame, threshold_move: float, sl: float, exit_time: tuple[int, int]
) -> tuple[pl.DataFrame, pl.DataFrame]:

    prices = prices.filter(
        pl.col("timestamp").count().over(pl.col("timestamp").dt.date()) <= 375
    )
    all_trades = (
        prices.with_columns(date=pl.col("timestamp").dt.date())
        .group_by("date")
        .map_groups(
            lambda df: process_day(df, threshold_move, sl, exit_time),
        )
    )
...
def fetch_and_run_single_stock(
    stock, backtester, threshold_move, sl, exit_time, from_date=None, to_date=None
):
    ...
    try:
        trades, summary = run_cash_backtest_prices(
            prices, threshold_move, sl, exit_time
        )
