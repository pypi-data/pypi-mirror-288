...
from email.utils import parsedate_to_datetime as parse_imfts
...
class Response:
	...
	def get_field(self, name: str) -> bytes:
		"""
		Return a byte string of comma separated message field values with the given name

		Due to them not being representable as a comma separated list, Set-Cookie fields
		cannot be requested with this method, use `Response.get_cookie()` instead.
		"""
		name = name.lower()
		if name == "set-cookie":
			raise ValueError("Use Response.get_cookie() to get a cookie value")
		return b", ".join(field[1] for field in self.headers if field[0] == name)

	def get_int_field(self, name: str, default: int|None = None) -> int|None:
		"""
		Return an integer value from the first matching field, or the default if not found

		If a matching field is found but its value cannot be interpreted as a decimal
	...
	def get_datetime_field(self, name: str, default: DateTime|None = None) -> DateTime|None:
		"""
		Return a date-time value from the first matching field, or the default if not found

		If a matching field is found but its value cannot be interpreted as a date-time
		string, raises `ValueError`.
		"""
		name = name.lower()
		for field_name, field_value in self.headers:
			if field_name == name:
				return parse_imfts(field_value.decode("utf-8"))
		return default

	def get_cookies(self) -> Iterator[bytes]:
		"""
		Return an iterator that yields the values of any Set-Cookie fields

		It is left to the caller to parse the contents of the Set-Cookie fields.
		"""
		return (field[1] for field in self.headers if field[0] == "set-cookie")
