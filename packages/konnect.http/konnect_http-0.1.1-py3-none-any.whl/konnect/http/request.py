...
class CurlRequest:
	...
	_debug = logging.getLogger(__package__).debug
	...
	def completed(self) -> bytes:
		...
		This is part of the `konnect.curl.Request` interface.
		"""
		assert self._phase == Phase.READ_BODY, self._phase
		data, self._data = self._data, b""
		return data
	...
	async def _start_request(self) -> BodySendStream|Response:
		...
		self._debug("Preparing request %r", self)
	...
	async def get_response(self) -> Response:
		...
		self._debug("Returned response %r to %r", resp, self)
		return resp
