...
def calculate_optimal_chunk(dimension: int) -> int:
    """given the dimension, returns the optimal chunk_size

    Args:
        dimension (int): the number of dimensions over which
...
def permutate(n: int, r: int) -> int:
    ...
    Returns:
        int: n permutate r (using sterlings approximation if
            when appropriate)
    """
    try:
        return int(math.factorial(n) / math.factorial(n - r))
    except OverflowError:
        return int(sterlings_approximation(n) / sterlings_approximation(n - r))
...
def combinate(n: int, r: int) -> int:
    ...
    Returns:
        int: n combinate r (using sterlings approximation when
            appropriate)
    """
    try:
        ...
        return int(
            sterlings_approximation(n)
            / (sterlings_approximation(n - r) * sterlings_approximation(r))
        )
...
def discrete_distribution_prob(exp: pd.Series, obs: pd.Series) -> float:
    ...
    individual_seq_prob = (
        prob_freq_df["Probability"] ** prob_freq_df["Frequency"]
    ).product()
...
def generate_joint_pdf(exp: pd.Series, num_obs: int) -> Callable:
    ...
    exponent_functions = [
        generate_normal_exponent(
            mean=(p * num_obs), std_dev=math.sqrt(p * num_obs * (1 - p))
        )
        for p in percent_exp
    ]
    denominator_expr = [
        math.sqrt(p * num_obs * (1 - p) * 2 * math.pi) for p in percent_exp
    ]
...
def hyperplane_integration(
    f: Callable,
    hyperplane: list,
    max_val: float = None,
    chunk_size: int = "auto",
    num_samples: int = "auto",
    random_state: int = 42,
    pbar: Callable = None,
) -> float:
    ...
    Args:
        ...
        pbar (tqdm): a progress bar to update with every chunk (defaulted to None)
    ...
    if num_samples == "auto":
        num_samples = calculate_num_samples(
            integration_dim=integration_dim, hyperplane=hyperplane
        )
        ...
        for i in range(current_chunk_size):
            ...
            for j in range(integration_dim):
                max_val_for_dim = (
                    remaining_sum
                    if j == 0
                    else remaining_sum - np.sum(hyperplane_points[i, :j])
                )
                samples[i, j] = samples[i, j] * max_val_for_dim
                hyperplane_points[i, j] = samples[i, j]

            x = hyperplane_points[i, :-1]
            hyperplane_points[i, -1] = (
                hyperplane.coef - np.dot(hyperplane.normal[:-1], x)
            ) / hyperplane.normal[-1]
...
def E(
    ...
    if not approximate:
        ...
        # obs = scale_binomial(exp=exp, obs=obs, discrete=True)    # SCALING RESEARCH TO FOLLOW
        ...
        # obs = scale_binomial(exp=exp, obs=obs, discrete=False)    # SCALING RESEARCH TO FOLLOW
        ...
        if num_samples == "auto":
            num_samples = calculate_num_samples(
                integration_dim=(len(obs) - 1), hyperplane=hyperplane
            )
        print(
            f"Using {num_samples*2} total samples in size {chunk_size} chunks ({num_samples*2/chunk_size} total chunks)"
        )
        ...
        thresheld_space = hyperplane_integration(
            f=joint_pdf,
        ...
        error_space = hyperplane_integration(
            f=joint_pdf,
            hyperplane=hyperplane,
            chunk_size=chunk_size,
            num_samples=num_samples,
        ...
        e_val = thresheld_space / error_space

    return round(e_val, 5)
