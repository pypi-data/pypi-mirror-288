...
class BasePlugin(metaclass=ABCMeta):
    ...
    @classmethod
    def _get_video_meta(cls, content: Union[bytes, io.IOBase]) -> dict | None:
        """returns video stream info of a video file using ffprobe command line interface"""
        args = ["ffprobe", "-v", "quiet", "-i", "pipe:", "-show_streams", "-show_format", "-print_format", "json"]

        if isinstance(content, bytes):
            # p = subprocess.Popen(args, stdin=io.BytesIO(content), stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            # TODO: support bytes ( e.g. youtube plugin gives bytes)
            return None
        elif isinstance(content, io.IOBase):
            p = subprocess.Popen(args, stdin=content, stdout=subprocess.PIPE, stderr=subprocess.PIPE)  # type: ignore

        out, err = p.communicate()
        if p.returncode != 0:
            logger.error(f"Failed call ffprobe: {out=} {err=}")
            return None
    ...
    @classmethod
    def parse_video_tag(cls, content: Union[bytes, io.IOBase]) -> Union[BaseTag, None]:
    ...
    @classmethod
    def parse_video_datetime(cls, content: Union[bytes, io.IOBase]) -> Union[int, None]:
        info = BasePlugin._get_video_meta(content)
