...
impl LineFrame {
    ...
    pub fn write_msgpack(
        ...
        write_str_pair(&mut buf, "thread", Some(&self.thread));
        write_int_pair(&mut buf, "thread_native_id", self.thread_native_id);
...
fn write_main_frames(buf: &mut Vec<u8>, main_frames: Vec<SerializedFrame>) {
    rmp::encode::write_str(buf, "frames_of_interest").expect("Writing to memory, not I/O");
    write_raw_frames(buf, main_frames);
}
...
/// a list of frames.
fn write_thread_frames(buf: &mut Vec<u8>, frames: HashMap<usize, Vec<SerializedFrame>>) {
...
fn write_meta(buf: &mut Vec<u8>, version: &str, source: &str) {
    ...
    write_bool_pair(buf, "use_frame_boundaries", true);
}
...
pub fn build_trace(
    py: Python,
    main_frames: Vec<SerializedFrame>,
    thread_frames: HashMap<usize, Vec<SerializedFrame>>,
    trace_id: &str,
    source: &str,
    ...
    write_thread_frames(&mut buf, thread_frames);
    write_main_frames(&mut buf, main_frames);
    write_int_pair(&mut buf, "main_thread_id", main_thread_id);
    write_meta(&mut buf, &version, source);
...
pub fn write_frame(
    ...
    rmp::encode::write_map_len(buf, 12).expect("Writing to memory, not I/O");
    ...
    write_str_pair(buf, "thread", Some(&thread_name));
    write_int_pair(buf, "thread_native_id", native_id);
