...
ava_dll.AVS_GetName.argtypes = [ctypes.c_int]
ava_dll.AVS_GetName.restype = ctypes.c_char_p
ava_dll.AVS_GetWavelengths.argtypes = [ctypes.c_int, ctypes.POINTER(ctypes.c_double)]
ava_dll.AVS_GetWavelengths.restype = ctypes.c_int
...
class DAQ_1DViewer_AvaSpec(DAQ_Viewer_base):
    ...
    def ini_detector(self, controller=None):
        if self.settings['controller_status'] == "Slave":
            ...
            for ind_spectro in range(N):
                try:
                    name = ava_dll.AVS_GetName(ind_spectro).decode('utf-8')
    ...
    def stop(self):
        # Assuming AvaSpec DLL does not provide a stop method.
        pass
