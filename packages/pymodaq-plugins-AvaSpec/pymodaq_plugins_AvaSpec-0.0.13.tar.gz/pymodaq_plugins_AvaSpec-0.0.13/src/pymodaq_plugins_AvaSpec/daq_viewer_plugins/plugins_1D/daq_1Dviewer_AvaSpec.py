...
class DAQ_1DViewer_AvaSpec(DAQ_Viewer_base):
    ...
    def grab_data(self, Naverage=1, **kwargs):
        ...
        for ind_spectro in range(len(self.spectro_names)):
            print(f"Processing spectrometer {ind_spectro} ({self.spectro_names[ind_spectro]})...")
            ...
            if grab_param:
                print(f"  Grab enabled for {self.spectro_names[ind_spectro]}")
                print(f"  Exposure time set to {exposure_time} ms")

                try:
                    print(f"  Preparing measurement with {Naverage} averages...")
                    self.controller.prepare_measure(
                        self.controller.MeasConfigType(m_IntegrationTime=exposure_time, m_NrAverages=Naverage))
                    ...
                    _, data = self.controller.get_data()
                    ...
                    print(f"  Error during data acquisition for spectrometer {ind_spectro}: {e}")
                    continue

            else:
                print(f"  Grab not enabled for {self.spectro_names[ind_spectro]}")
