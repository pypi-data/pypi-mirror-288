...
def get_spectrometers_list(dll_path):
    ...
    try:
        ...
        num_devices = c_int()
        result = dll.AVS_GetNrOfDevices(byref(num_devices))
        if result != 0:
            print(f"Error getting number of devices: {result}")
            return []

        print(f"Number of devices found: {num_devices.value}")

        spectrometers = []

        for i in range(num_devices.value):
            # Get the serial number of each device
            serial_buffer = create_string_buffer(128)
            result = dll.AVS_GetHandleFromSerial(i, serial_buffer, 128)
            if result != 0:
                print(f"Error getting handle for device {i}: {result}")
                continue
            serial = serial_buffer.value.decode('utf-8').strip()
            spectrometers.append(serial)
            print(f"Spectrometer {i}: {serial}")
