    ...
    def grab_data(self, Naverage=1, **kwargs):
        try:
            ...
            for ind_spectro in range(len(self.spectro_names)):
                if self.settings.child('spectrometers', 'spectro0', 'grab').value():
                    # Préparation de la mesure avec l'exposition et le nombre de moyennes
                    cfg = self.controller.MeasConfigType()
                    cfg.m_IntegrationTime = self.settings.child('spectrometers', 'spectro0', 'exposure_time').value()
                    cfg.m_NrAverages = Naverage
                    print(f"Preparing measure with integration time: {cfg.m_IntegrationTime} and averages: {cfg.m_NrAverages}")
                    ret = self.controller.prepare_measure(cfg)
                    if ret != 0:
                        print(f"Error in prepare_measure: {ret}")
                        continue
                    ret = self.controller.measure(1)
                    if ret != 0:
                        print(f"Error in measure: {ret}")
                        continue
                    while not self.controller.poll_scan():
                        time.sleep(0.01)
                    result, data = self.controller.get_data()
                    if result == 0:
                        print(f"Data acquisition successful: {data[:10]}...")  # Afficher les 10 premières valeurs pour le débogage
                        data_array = np.array(data)
                        dte.append(DataFromPlugins(name=self.spectro_names[ind_spectro], data=[data_array], dim='Data1D'))
                        QtWidgets.QApplication.processEvents()
                    else:
                        print(f"Error in data acquisition: {result}")
            self.dte_signal.emit(dte)
        except Exception as e:
            print(f"Exception during data acquisition: {e}")
