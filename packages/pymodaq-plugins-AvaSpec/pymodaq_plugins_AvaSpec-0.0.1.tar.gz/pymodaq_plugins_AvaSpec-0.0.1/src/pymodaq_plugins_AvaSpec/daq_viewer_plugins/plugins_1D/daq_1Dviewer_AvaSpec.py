...
class DAQ_1DViewer_AvaSpec(DAQ_Viewer_base):
    ...
    def commit_settings(self, param):
        ...
        elif param.name() == 'avaspec_dll_path':
            self.initialize_controller(param.value())
        ...
        try:
            ...
            for ind_spectro in range(len(self.spectro_names)):
                if self.settings.child('spectrometers', 'spectro0', 'grab').value():
                    self.controller.prepare_measure(self.controller.MeasConfigType(m_IntegrationTime=self.settings.child('spectrometers', 'spectro0', 'exposure_time').value(), m_NrAverages=Naverage))
