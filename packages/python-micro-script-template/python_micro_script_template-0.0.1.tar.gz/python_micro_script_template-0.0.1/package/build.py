...
def main():
    ...
    package_dir = get_package_dir_arg_or_relative_default()

    print(f"Package dir: {package_dir}")

    if (not package_dir):
        raise Exception('Empty package dir is not allowed --> exitting')

    print_exec_cmd(f'rm {package_dir}/dist/*')
    print_exec_cmd(f'python3 {package_dir}/setup.py sdist bdist_wheel')
