...
class OracleTools:
    ...
    @staticmethod
    def _desc(ora_conn, username, table):
        """得到表结构，包括索引、约束和默认值情况"""
        ...
        for col_ in cols_:
            ...
            if col_[3] != 'None':
                data_default = col_[3].replace('''/* GOLDENGATE_DDL_REPLICATION */''', '').strip().strip('\n')
            else:
                data_default = None
                
            mess_map['columns'].append({'name':colstr, 'type':typestr, 'nullable':nullable, 'default_value':data_default})
        ...
        if len(rows) > 0:
            ...
            for rs in rss:
                ...
                mess_map['indexes'].append({'name':name, 'columns':col, 'type':idx_type, 'owner':idx_owner})
    ...
    def desc(ora_conn, username, table, simple_mode=True):
        ...
        for rs in rss:
            ...
            if c_type == 'U':
                ...
                rs = ora_conn.run(sql).get_rows(True)[0]
                constraint_type = ' fk references ' + r_owner.lower() + '.' + rs[0].lower() + '(' + rs[1].lower() + ')'
            else:
                constraint_type = ''
            if simple_mode:
