...
from .request import NeuralTrustClient
...
class NeuralTrustMetadata(pydantic.BaseModel):
    batch_size: int
        ...
        def execute_task_with_backoff(batch: List[Any]):
            try:
                ...
                if isinstance(e, APIError) and 400 <= int(e.status) < 500:
                    self._log.warn(
                        f"Received {e.status} error by NeuralTrust server, not retrying: {e.message}"
                    )
                    return
...
class TaskManager(object):
    _log = logging.getLogger("neuraltrust")
