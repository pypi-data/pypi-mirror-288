...
SENSI_CONFIG_OPTIONAL_COLUMNS = ["Description"]
...
def sensi_config_is_valid(sensi_config):
    ...
    optional_headers = set(SENSI_CONFIG_OPTIONAL_COLUMNS)
    ...
    unexpected_headers = set(sensi_config_header) - required_headers - optional_headers
    if unexpected_headers:
        logger.error(
            "Sensi config has unexpected headers. "
            f"Unexpected headers: {unexpected_headers}"
        )
        raise SensiIOError(
            "Sensitivity configuration file has unexpected headers. "
            f"Unexpected headers: {unexpected_headers}"
        )
    ...
    expected_columns_num = len(SENSI_CONFIG_HEADER) + (1 if "Description" in sensi_config_header else 0)
