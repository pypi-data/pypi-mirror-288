...
class AI:
    def __init__(self,role="helpful assistant",limit_words=30,mode=0,level=1,tone=1,pointer_template=""):
    ...
    def __generate_messages(self,msg,is_multipart=False):
        if not is_multipart:
            self.__messages = []
        if self.role==1:
            self.__messages.append({'role': 'system', 'content': f'You are a helpful assistant.'})
        elif self.role==2:
            self.__messages.append({'role': 'system', 'content': f'You are a English teacher.'})
        elif self.role==3:
            self.__messages.append({'role': 'system', 'content': f'You are a Math teacher.'})
        elif self.role==4:
            self.__messages.append({'role': 'system', 'content': f'You are a scientist.'})
        elif self.role==5:
            self.__messages.append({'role': 'system', 'content': f'You are a good friend.'})
        elif self.role==6:
            self.__messages.append({'role': 'system', 'content': f'You are a robot.'})
        elif self.role==7:
            self.__messages.append({'role': 'system', 'content': f'You are a programming teacher.'})
        else:
            self.__messages.append({'role': 'system', 'content': f'You are a helpful assistant.'})
