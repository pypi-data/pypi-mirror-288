...
from voxelment.core.utils import format_args

class Transform(Apply):
    """Root class for single augmentations
    """
    def __init__(self, is_trainable=False, p=0.5):
        """
            :args:
                is_trainable: bool
        ...
        """
        super(Transform, self).__init__(is_trainable, p)

    def forward(self, *args, force_apply=False, **data):
        """
            :args:
                data: dict
        ...
        """
        if args:
            raise KeyError('You have to pass data to augmentations as named arguments, for example: aug(exg=exg)')
