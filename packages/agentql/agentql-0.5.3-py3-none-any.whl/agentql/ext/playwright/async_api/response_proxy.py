...
from agentql.ext.playwright._response_proxy_parent import BaseAQLResponseProxy
from agentql.ext.playwright.async_api.playwright_driver_async import locate_interactive_element

log = logging.getLogger("agentql")


class AQLResponseProxy(BaseAQLResponseProxy):
    """
    ...
    def _resolve_item(self, item, query_tree_node) -> Union[Locator, "AQLResponseProxy", None]:
        ...
        if isinstance(query_tree_node, IdNode) or isinstance(query_tree_node, IdListNode):
            interactive_element = locate_interactive_element(self._page, item)
    ...
    async def _to_data_id_node(self, response_data: dict) -> Union[dict, str, None]:
        ...
        if not name or not name.strip():
            web_element = locate_interactive_element(self._page, response_data)
            if not web_element:
