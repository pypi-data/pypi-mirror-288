...
class JenkinsCommonApiService(JenkinsApiService):
    ...
    def get_jenkins_info(self) -> Dict:
        ...
        try:
            response = self.jenkins_client.make_request(CommonEndpoints.INFO, method='GET')
            ...
            self.presenter.error(f"Unexpected error retrieving Jenkins instance information: {e}")
            raise
    ...
    def get_crumb(self) -> Dict:
        ...
        try:
            response = self.jenkins_client.make_request(CommonEndpoints.CRUMB_ISSUER, method='GET')
    ...
    def who_am_i(self, depth: int = 1) -> Dict:
        ...
        try:
            response = self.jenkins_client.make_request(CommonEndpoints.WHOAMI_URL, method='GET', depth=depth)
            self.presenter.info("Successfully retrieved current user information.")
            return response
        except JenkinsError as e:
            self.presenter.error(f"Jenkins error retrieving current user information: {e}")
            raise
        except Exception as e:
            self.presenter.error(f"Unexpected error retrieving current user information: {e}")
            raise