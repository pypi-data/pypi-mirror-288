...
class JenkinsJobApiService(JenkinsApiService):
    ...
    def get_all_jobs(self, tree: str) -> Dict:
        ...
        try:
            response = self.jenkins_client.make_request(JobEndpoints.JOBS_QUERY, method='GET', tree=tree)
    ...
    def get_job_info(self, folder_url: str, short_name: str, depth: int = 1) -> Dict:
        ...
        dict
            The job information.
        """
        self.presenter.info(f"Retrieving job info for {short_name} in {folder_url} with depth {depth}")
        try:
            response = self.jenkins_client.make_request(JobEndpoints.JOB_INFO, method='GET', folder_url=folder_url, short_name=short_name, depth=depth)
    ...
    def build_job(self, folder_url: str, short_name: str) -> Dict:
        ...
        try:
            response = self.jenkins_client.make_request(JobEndpoints.BUILD_JOB, method='POST', folder_url=folder_url, short_name=short_name)
    ...
    def build_with_params(self, folder_url: str, short_name: str, params: Dict) -> Dict:
        ...
        """
        self.presenter.info(f"Triggering build with parameters for job {short_name} in {folder_url} with params {params}")
        try:
            request_query = DeployParameters(params).to_url_query()
            response = self.jenkins_client.make_request(JobEndpoints.BUILD_WITH_PARAMETERS, method='POST', query=request_query, folder_url=folder_url, short_name=short_name)
    ...
    def get_all_builds(self, folder_url: str, short_name: str, tree: str) -> Dict:
        ...
        """
        self.presenter.info(f"Retrieving all builds for job {short_name} in {folder_url} with tree structure {tree}")
        try:
            response = self.jenkins_client.make_request(JobEndpoints.ALL_BUILDS, method='GET', folder_url=folder_url, short_name=short_name, tree=tree)
