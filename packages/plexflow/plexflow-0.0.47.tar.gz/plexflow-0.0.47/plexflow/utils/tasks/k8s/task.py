...
class PlexflowK8sOperator(KubernetesPodOperator):
    def __init__(self, *args, **kwargs):
        ...
        self.default_volumes = [
            k8s.V1Volume(
                name='dags-volume',
                persistent_volume_claim=k8s.V1PersistentVolumeClaimVolumeSource(claim_name='ssd-airflow-dags-pvc')
            ),
            k8s.V1Volume(
                name='logs-volume',
    ...
    def _generate_labels(self, context):
        ...
        labels = {
            ...
            **{f"ARG_{i}": str(arg) for i, arg in enumerate(self.extra_args)},
            **{f"KW_ARG_{i}": json.dumps({"key": key, "value": value}) for i, (key, value) in enumerate(self.extra_kwargs.items())},
        }
        return labels
