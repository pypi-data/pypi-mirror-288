...
template<size_t D>
using IntersectedTriangulation = std::pair<Triangulation<D>, TriangleCellAssociation<D>>;

MLHP_EXPORT
IntersectedTriangulation<3> intersectTriangulationWithMesh( const AbsMesh<3>& mesh,
                                                            const Triangulation<3>& triangulation,
                                                            const KdTree<3>& tree );
...
struct CellAssociatedTriangulation
{
    ...
    operator Triangulation<D>& ( ) { return triangulation; }
    operator const Triangulation<D>& ( ) const { return triangulation; }
};
