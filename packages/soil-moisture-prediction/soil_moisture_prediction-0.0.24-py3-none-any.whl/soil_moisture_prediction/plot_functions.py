...
def plot_selection(rfo_model, plot_input=True, plot_results=True):
    ...
    if rfo_model.input_data.are_all_predictors_constant():
    ...
    for time_step in predictor_time_steps:
        ...
        if what_to_plot.pred_correlation and plot_results:
            prediction_correlation_matrix(rfo_model.input_data, time_step, output_dir)
...
def plot_predictors(
    ...
    n_cols = np.ceil(len(input_data.predictors) / 2).astype(int)
    xaxis, yaxis = input_data.soil_moisture_data.geometry.get_axis()

    if CONSTANT_TIME_STEP == time_step:
        x = np.concatenate(list(input_data.soil_moisture_data.x.values())) / 1000
        y = np.concatenate(list(input_data.soil_moisture_data.y.values())) / 1000
    else:
        x = input_data.soil_moisture_data.x[time_step] / 1000
        y = input_data.soil_moisture_data.y[time_step] / 1000
    ...
    nan_mask = input_data.get_nan_mask(time_step)

    for pred_name, pred_data in input_data.predictors.items():
...
def prediction_correlation_matrix(
    input_data, time_step, output_dir, return_base64_img=False
):
    ...
    plt.rcParams.update({"font.size": 5})
    correlation_matrix = input_data.compute_correlation_matrix(time_step)
...
def plot_measurements(
    soil_moisture_data, time_step, output_dir=None, return_base64_img=False
):
    """
...
def plot_rfo_model_no_dispersion(
    ...
    im = plt.pcolormesh(
        xaxis,
        yaxis,
        rfo_model.RF_prediction[0, time_index, :, :],
        vmin=0.1,
        vmax=0.45,
        cmap="Spectral",
    )
    ...
    fig_file_path = os.path.join(output_dir, "RF_prediction_" + time_step + ".svg")
    plt.savefig(fig_file_path, dpi=300)
    plt.close()
...
def plot_monte_carlo_iteration(
    ...
    fig_file_path = os.path.join(
        output_dir,
        "RF_prediction_" + time_step + "_iteration_" + iteration_index + ".svg",
    )
    plt.savefig(fig_file_path, dpi=300)
    plt.close()
