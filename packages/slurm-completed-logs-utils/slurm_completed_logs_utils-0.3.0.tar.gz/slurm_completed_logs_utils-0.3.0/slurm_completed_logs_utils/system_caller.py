...
def _derive_std_file(cmd: str, outdir: str, std_type: str) -> str:
    ...
    Returns:
        str: The path to the file where STDOUT or STDERR will be written to.
    """
    if std_type.lower() not in ["stdout", "stderr"]:
        raise Exception(f"std_type '{std_type}' is not valid")

    primary = cmd.split(" ")[0]

    basename = os.path.basename(primary)

    std_file = os.path.join(outdir, f"{basename}.{std_type.lower()}")
