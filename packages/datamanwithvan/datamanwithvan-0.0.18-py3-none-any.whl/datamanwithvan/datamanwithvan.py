...
# Section 5 : Create an instance of the DatamanwithvanMessages class
...
file_handler = logging.FileHandler(dmwvConfigObj.app_file_log)
...
def datamanwithvan_entry():
    ...
    if args.config:
        dmwvConfigObj._load_config_from_file(args.config)
    # End of Section 14
    ...
    if args.subcommand:
        if args.run is not None and args.subcommand == "job":
            ReplJobObj = replicationjob.ReplicationJob(args.run,
                                                       dmwvConfigObj.dmConf,
                                                       args.verbose)

            ReplJobObj.runjob(args.run)
        else:
            pass
    # End of Section 8
