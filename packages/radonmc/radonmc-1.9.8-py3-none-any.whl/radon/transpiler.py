...
class Transpiler:
    def __init__(self, statements: List[Statement], macros: List[Statement], pack_namespace: str = "mypack",
        ...
        load_file = [
            ...
            'data modify storage temp _fn_args set value {}',
            'data modify storage temp _fn_args_macro set value {}',
        ...
        if isinstance(statement, DefineFunctionStatement):
            ...
            for index, arg in enumerate(arguments):
                if arg.store_via == "macro":
                    ...
                    macro = f"$({arg.store.location.split(' ')[-1].split('.')[-1]})"
                    if t.type == "string":
                        macro = f'"{macro}"'
            ...
            arguments.append(
                FunctionArgument(
                    ...
                    _type_to_cpl(
                        ...
                        score_loc="",
                        nbt_loc=f"storage temp _fn_args.{arg_name}",
                        force_nbt=True
                    ),
                    store_via=store_via
                )
    ...
    def run_function_with_cpl(
        ...
        for index, arg in enumerate(fn_args):
            ...
            if arg.store_via == "stack":
                ...
                val.cache(ctx, nbt_loc=f"storage temp _fn_args_macro.{arg.store.location.split(' ')[-1]}", force="nbt")
        ...
        if found_fn.type == "radon":
            ctx.file.append(f"function {self.pack_namespace}:{found_fn.file_name}" + (
                f" with storage temp _fn_args_macro" if has_any_macro_argument else ""
            ))
            for index, arg in enumerate(fn_args):
                if arg.store_via == "stack":
