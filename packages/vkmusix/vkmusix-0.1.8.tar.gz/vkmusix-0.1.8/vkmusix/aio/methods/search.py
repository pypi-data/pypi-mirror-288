...
class Search:
    async def search(self, query: str, limit: int = 10, offset: int = 0) -> Union[List[Union[Track, dict]], Track, dict, None]:
        """
        Ищет аудиотреки по запросу.

        Пример использования:\n
        result = await client.search(query="prombl — zapreti", limit=1)\n
        print(result)
    ...
    async def searchAll(self, query: str, limit: int = 10, offset: int = 0) -> Union[dict[Union[List[Union[Artist, Album, Track, Playlist, dict]], Artist, Album, Track, Playlist, dict]], None]:
        """
        Ищет артистов, альбомы, аудиотреки, плейлисты по запросу.»

        Пример использования:\n
        result = await client.searchAll(query="prombl", limit=1)\n
        print(result)
