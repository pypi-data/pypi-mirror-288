...
class Expression(str):
    ...
    def __init__(self,expression:str):
        ...
        elif self.expression[1] in ['+','-','*','/'] or self.expression[0] in ['(','[','{']:self.__type = 'affix expression'
    ...
    def count(expression):
        ...
        for i in expression:
            if str(i) in '1234567890':stack.push(i)
                ...
                a = int(stack.pop());b = int(stack.pop());stack.push(b-a)
            elif i == '*':stack.push(int(stack.pop())*int(stack.pop()))
            elif i == '/':a = int(stack.pop());b = int(stack.pop());stack.push(b / a)
            else:raise ExpressionError('Operators and numbers must in "+","-","*","/","1","2","3","4","5","6","7","8","9","0".')
        return stack.pop()
