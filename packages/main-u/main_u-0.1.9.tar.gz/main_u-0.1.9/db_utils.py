...
"""
for example:
    env_db = MysqlUtils(database='env')    # 使用环境变量

    base = MysqlUtils(database='base', host='localhost', user='root', password='123456')
    print(base)
    print(get_db_data(base, 'select * from user'))

    >>>
    [{'id': 1, 'name': 'hejia', 'created_at': datetime.datetime(2024, 7, 29, 16, 12, 56, 11808),'updated_at': datetime.datetime(2024, 7, 29, 16, 12, 56, 11808), 'deleted_at': None, 'is_deleted': 0, 'is_superuser': 0, 'password': 'abc123456', 'phone': '1352625236'}, 
    {'id': 2, 'name': 'hejia24', 'created_at': datetime.datetime(2024, 7, 29, 16, 15, 24, 501265),'updated_at': datetime.datetime(2024, 7, 29, 16, 15, 24, 501265), 'deleted_at': None, 'is_deleted': 0, 'is_superuser': 0, 'password': 'abc123456', 'phone': None}]
"""
...
class DBUtils(metaclass=ABCMeta):
    def __init__(self, database:str, is_ssh: bool=False, curson_type: bool=True, user_dotenv: bool=True, **kwargs):
        ...
        if user_dotenv:
            ...
            if kwargs:
                db_attr = ["host", "port", "user", "password", "ssh_host", "ssh_port", "ssh_user", "ssh_password"]
                __setattr = [setattr(self, arg, kwargs[arg]) for arg in kwargs if arg in db_attr]
