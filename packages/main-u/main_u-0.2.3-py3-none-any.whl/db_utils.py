...
for example:
    ...
    db = MysqlUtils(database='base')    # 使用环境配置文件
    print(get_db_data(db, 'select * from user'))

    >>>
    [{'id': 1, 'name': 'admin'}, {'id': 2, 'name': 'admin'}]

    base = MysqlUtils(database='base', use_dotenv=False, host='localhost', user='root', password='123456', port=3306) # 不使用环境配置文件
