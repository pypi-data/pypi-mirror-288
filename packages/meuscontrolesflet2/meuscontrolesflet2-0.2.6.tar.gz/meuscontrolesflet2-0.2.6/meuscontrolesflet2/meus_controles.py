...
class Display(ft.Container):
    def __init__(self,
        ...
        if self.opitions is None:
            self.opitions = [ft.PopupMenuItem(i, on_click = self.Clicou) for i in range(30,250,1)]
        else:
            self.opitions = [ft.PopupMenuItem(i, on_click = self.Clicou) for i in opitions]
        ...
        self.content = ft.PopupMenuButton(
            content=ft.Column([ft.Text(self._value, color = 'white', weight='BOLD', size=self.text_size)], alignment='center', horizontal_alignment='center'),
            items=self.opitions,
