...
# GoogleExplorer
################################################################
class GoogleShoppingExplorer(Explorer):
    def __init__(
        self,
        serpapi_api_key: str,
        serpapi_rpm: int = 10,
    ...
    async def search(
        ...
        try:
            logger.info("Executing shopping search for task %s", name)
            pages = await self.google.shopping(q=query, location=location)

            # page handler (update target link)
            hit = 0
            log = f"No results for this query. ({query})"
            if pages:
                ...
                logger.info(
                    "Shopping search for task %s returned %d pages with a total of %d hits", name, len(pages), hit
                )

            return TaskResult(name=name, after=None, before=punch_date(), success=True, hit=hit, log=log)
        finally:
