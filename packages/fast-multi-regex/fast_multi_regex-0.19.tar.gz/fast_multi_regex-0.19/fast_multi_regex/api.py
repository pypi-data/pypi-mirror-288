...
async def get_metrics():
    ...
    if 'fmrs_matchers_count' not in global_metric_wrapper:
        global_metric_wrapper['fmrs_matchers_count'] = Gauge('fmrs_matchers_count', '匹配器/正则库的数量', ['process_index']).labels(process_index)
    global_metric_wrapper['fmrs_matchers_count'].set(len(global_matchers))
    ...
    for metric, value in total_matchers_metrics.items():
        name = f'fmrs_matchers_{metric}_sum'
