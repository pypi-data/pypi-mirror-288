...
def get_track(distance: int, track_mode: TRACK_MODE_TYPE = 'bezier_curve') -> List[List]:
    track = []
...
def slide(ele_or_loc: str, distance: int, cp: ChromiumPage,
          track_mode: TRACK_MODE_TYPE = 'bezier_curve', timeout: Optional[float] = None) -> None:
    ac = Actions(cp)
    ac.move_to(ele_or_loc).hold()
    track = get_track(distance, track_mode=track_mode)
    for i in track:
        ac.move(*i)
    ac.release()
