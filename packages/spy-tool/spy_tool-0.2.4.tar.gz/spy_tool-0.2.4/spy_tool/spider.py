...
class CpHandler(ABC, CpMixin):
    logger = logger

    def _handle_return_url(self, return_url: Optional[str] = None) -> None:
        if return_url:
            if self.cp.url != return_url:
                self.cp.get(return_url)

    @abstractmethod
    def action(self, return_url: Optional[str] = None, timeout: float = .5):
        pass

    def run(self, return_url: Optional[str] = None, timeout: float = .5):
        while True:
            try:
                self.action(return_url, timeout)
                break
