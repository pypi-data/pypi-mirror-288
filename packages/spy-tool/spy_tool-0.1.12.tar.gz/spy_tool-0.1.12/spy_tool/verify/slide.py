...
def get_distance(background_image_base64: str, target_image_base64: str, background_image_render_width: int,
...
def points_to_track(points: List[List]) -> List[List]:
    track = []
    cur_x, cur_y = 0, 0
    for point in points:
        x, y = point
        track.append([x - cur_x, y - cur_y, random.uniform(.02, .04)])
        cur_x = x
        cur_y = y
    return track


def get_track_by_bezier_curve(slide_x,
    ...
    """
    bc = BezierCurve()
    res = bc.trackArray([0, 0], [slide_x, 0], numberList, le=le, deviation=deviation, bias=bias, type=type, cbb=cbb,
                        yhh=yhh)
...
def get_track_by_ghost_cursor(distance: int) -> List[List]:
    js_code = '''function sdk(from, to) {
...
def get_track(distance: int, mode: Literal['bezier_curve', 'ghost_cursor'] = 'bezier_curve') -> List[List]:
    track = []
    if mode == 'bezier_curve':
        track = get_track_by_bezier_curve(distance)
    elif mode == 'ghost_cursor':
        track = get_track_by_ghost_cursor(distance)
    return track


def slide(ele_or_loc: str, distance: int, cp: ChromiumPage, timeout: Optional[float] = None) -> None:
    ac = Actions(cp)
