            ...
            if filename.exists() and filename.is_file():
                ...
                with Session(ENGINE) as session:
                    for num,row in enumerate(df.itertuples()):
                        ...
                        if len(check) > 0:
                            if num % 200 == 0:
                                msg=f"{Fore.light_red}{num+1} [{Fore.light_magenta}ET:{now-import_start}{Fore.light_red}] not added{Style.reset} -> [{check[0].Term}..{check[-1].Term}] - [{check[0].Definition}..{check[-1].Definition}]"
                                if len(msg) < os.get_terminal_size().columns:
                                    print(msg)
                                else:
                                    print(msg[:os.get_terminal_size().columns])
                            continue
                        ...
                        if num % 200 == 0:
                            ...
                            session.flush()
                            session.refresh(gt)
                            msg=f"{Fore.light_green}{num+1} [{Fore.cyan}ET:{now-import_start}{Fore.light_green}] added{Style.reset} -> {gt}"
                            if len(msg) < os.get_terminal_size().columns:
                                print(msg)
                            else:
                                print(msg[:os.get_terminal_size().columns])
