    ...
    def import_hsv(self):
        try:
            ...
            if filename.exists() and filename.is_file():
                ...
                with Session(ENGINE) as session:
                    for num,row in enumerate(df.itertuples()):
                        check=session.query(Glossary).filter(Glossary.Term==row.Term).all()
                        if len(check) > 0:
                            check2=session.query(Glossary).filter(Glossary.Term==row.Term,Glossary.Definition==row.Definition).first()
                            if check2:
                                print(f"{Fore.light_red}{num+1} not added{Style.reset} -> {check2.Term} - {check2.Definition}")
                                continue
