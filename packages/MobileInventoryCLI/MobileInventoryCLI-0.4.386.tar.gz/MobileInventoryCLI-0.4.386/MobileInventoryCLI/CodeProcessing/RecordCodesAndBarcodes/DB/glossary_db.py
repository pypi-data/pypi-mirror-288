                    ...
                    for num,row in enumerate(df.itertuples()):
                        ...
                        if len(check) > 0:
                            if num % flush_size == 0:
                                ...
                                modCount=len(Fore.light_red+Fore.light_steel_blue+Fore.light_red+Fore.light_magenta+Fore.light_red+Style.reset)
                                if len(msg) < os.get_terminal_size().columns:
                                    print(msg)
                                else:
                                    print(msg[:os.get_terminal_size().columns+modCount])
                            continue
                        ...
                        if num % flush_size == 0:
                            ...
                            session.flush()
                            session.refresh(gt)
                            msg=f"{Fore.light_green}{num+1}/{Fore.light_steel_blue}{size}{Fore.light_green} {round(((num+1)/size)*100,2)}% [{Fore.cyan}ET:{now-import_start}{Fore.light_green}] added{Style.reset} -> {gt}"
                            modCount=len(Fore.light_green+Fore.light_steel_blue+Fore.light_green+Fore.cyan+Fore.light_green+Style.reset)
                            if len(msg) < os.get_terminal_size().columns:
                                print(msg)
                            else:
                                print(msg[:os.get_terminal_size().columns+modCount])
