...
def preprocess(data, fil='bandpass', S_L=True, bandpass=[2,8],freq=100, sl=[0.5,1],beta=0,kernel=(3,3)):
    ...
    if S_L:
        data_f = data_SL(data_f, int(sl[0]*freq), int(sl[1]*freq))
    return data_f

    
def auto_split(data,hwin=512,wwin=512,overlap=0.5,freq =200, return_centers = False): 
    """
    ...
    centers = np.zeros((h_num, w_num,2))
    for i in range(h_num):
        for j in range(w_num):
            ...
            #centers.append([int(i*hwin*(1-overlap))+hwin//2,int(j*wwin*(1-overlap))+wwin//2])
            centers[i,j] = [int(i*hwin*(1-overlap))+hwin//2,int(j*wwin*(1-overlap))+wwin//2]
    if return_centers:
        centers[:,:,1] = centers[:,:,1] / freq
        return data_split,centers
    else:
        return data_split
