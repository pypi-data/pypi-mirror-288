...
class detector():
    ...
    def detect(
            ...
            return_centers = False # 是否返回检测到的事件的中心，默认为False
            ):
        """
        ...
        datas,centers = auto_split(data,hwin=hwin,wwin=wwin,overlap=overlap,freq=freq, return_centers = True)
        a,b,_,_ = datas.shape
        ...
        if figure:
            ...
            for i in range(a):
                for j in range(b):
                    ...
                    plt.title(f'{output_class[i,j]}')
                    plt.axis('off')
            plt.tight_layout()
            plt.show()
        return output_class,centers
