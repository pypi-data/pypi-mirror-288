...
class Query(object):
    ...
    def __init__(self, client_id, client_secret, proxies={}, verify=True, timeout=None):
    ...
    def get_data(self, dataset_id, start=None, limit=None, except_on_truncate=False):
        ...
        # 2. HEAD
        is_created = False
        for index in range(self._timeout) if self._timeout is not None else itertools.count():
            response = self._requests(
                method='head',
            ...
            if except_on_truncate is True and (
                response.headers.get('X-Truncated', 'false') == 'true'
                or
                response.headers.get('x-truncated', 'false') == 'true'
            ):
