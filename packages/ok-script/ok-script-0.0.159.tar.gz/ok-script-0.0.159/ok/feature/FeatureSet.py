...
def adjust_coordinates(x, y, w, h, screen_width, screen_height, image_width, image_height):
    ...
    if scale_x > scale_y:
        y = round(y * scale)
        x = scale_by_anchor(x, image_width, screen_width, scale)
    elif scale_x < scale_y:
        x = round(x * scale)
        y = scale_by_anchor(y, image_height, screen_height, scale)
    else:
...
def scale_by_anchor(x, image_width, width, scale):
    if x > image_width * 0.5:
        if x > image_width * 0.75:
            x = width - round((image_width - x) * scale)
        else:
            x = round(width * 0.5 + (x - image_width * 0.5) * scale)
    else:
        if x > image_width * 0.25:
            x = round(width * 0.5 - (image_width * 0.5 - x) * scale)
        else:
            x = round(image_width * scale)
    return x
