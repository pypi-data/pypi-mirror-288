def file_io_execute(command):
    ...
    try:
        if "write" in words and "to file" in words:
            ...
            # More flexible content extraction, doesn't rely on "that"
            if "content is" in words:
                content_index = words.index("is") + 1
            else:
                content_index = words.index("to") + 2  # Start after "to file <filename>"
