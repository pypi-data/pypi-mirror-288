...
def reverse(s: Outcome):
    return Outcome.LOSS if s == Outcome.WIN else Outcome.DRAW if s == Outcome.DRAW else Outcome.WIN
...
class Player:
    ...
    def update(self, players=[], s=[]):
        ...
        Δ = v * sum(g(p.φ) * (s_j.value - E(self.μ, p.μ, p.φ)) for p, s_j in zip(players, s, strict=True))
        A = a = ln(self.σ**2)
        ...
        self.μ += self.φ**2 * sum(g(p.φ) * (s_j.value - E(self.μ, p.μ, p.φ)) for p, s_j in zip(players, s, strict=True))
        self.r, self.RD = SCALE * self.μ + DEFAULT_R, SCALE * self.φ
    
    def __repr__(self):
        return f'Player(r={self.r}, RD={self.RD}, σ={self.σ})'

def update(player1: Player, player2: Player, s: Outcome):
    tmp = copy(player1)
    player1.update([player2], [s])
    player2.update([tmp], [reverse(s)])