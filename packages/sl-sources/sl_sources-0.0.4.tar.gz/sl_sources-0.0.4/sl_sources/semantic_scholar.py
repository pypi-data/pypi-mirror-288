...
async def arxiv_to_pdf(arxiv_id, path, session):
    ...
    async with session.get(url, allow_redirects=True) as r:
        ...
        with open(path, "wb") as f:
            ...
            # if content is 0 bytes, try again
            if not content:
                raise Exception(f"No paper with arxiv id {arxiv_id}")
            f.write(content)


async def download_pdf(url, path, session):
    # download
    async with session.get(url, allow_redirects=True) as r:
        if r.status != 200:
...
async def download_from_semantic_scholar(papers, pdir=os.curdir, verbose=False):
    if not os.path.exists(pdir):
        os.mkdir(pdir)

    async def download_paper(paper):
        path = os.path.join(pdir, f'{paper["paperId"]}.pdf')
        if check_pdf(path, verbose=verbose):
            with open(path, "rb") as file:
                pdf = PdfReader(file)
                full_text = "\n".join(page.extract_text() for page in pdf.pages)
                return {"full_text": full_text, "url": paper["url"]}, paper

        sources = [
            ("ArXiv", arxiv_to_pdf),
            ("PubMedCentral", pmc_to_pdf),
            ("PubMed", pubmed_to_pdf),
            ("openAccessPdf", download_pdf),
            ("DOI", doi_to_pdf),
        ]

        for source_name, download_func in sources:
            try:
                if source_name in paper["externalIds"] or (
                    source_name == "openAccessPdf" and paper["openAccessPdf"]
                ):
                    async with ThrottledClientSession(
                        rate_limit=15 / 60, headers=get_header()
                    ) as session:
                        if source_name == "openAccessPdf":
                            await download_func(
                                paper["openAccessPdf"]["url"], path, session
                            )
                        else:
                            await download_func(
                                paper["externalIds"][source_name], path, session
                            )
                    ...
                    if check_pdf(path, verbose=verbose):
                        with open(path, "rb") as file:
                            ...
                            if not full_text.strip():  # Check if the text is empty or whitespace
                                print(f"Empty text from PDF, trying scrape for {paper['url']}")
                                full_text = await scrape(paper["url"])
                            return {"full_text": full_text, "url": paper["url"]}, paper
            except Exception as e:
                if verbose:
                    print(f"\t{source_name} failed: {e}")

        # If no PDF is found or the extracted text is empty, try scraping the URL
        try:
            print(f"No PDF found, trying scrape for {paper['url']}")
            full_text = await scrape(paper["url"])
            return {"full_text": full_text, "url": paper["url"]}, paper
        except Exception as e:
            if verbose:
                print(f"Error scraping {paper['url']}: {e}")
            return {"full_text": paper.get("abstract", ""), "url": paper["url"]}, paper

    results = await asyncio.gather(*[download_paper(paper) for paper in papers])
    return {
        result["full_text"]: paper for result, paper in results if result is not None
    }

def run_search_papers(query, num_results=10, semantic_scholar_api_key=None):
    loop = asyncio.get_event_loop()
    return loop.run_until_complete(
        search_papers(query, num_results, semantic_scholar_api_key)
    )


def run_download_from_semantic_scholar(papers, pdir=os.curdir, verbose=False):
    loop = asyncio.get_event_loop()
    return loop.run_until_complete(
        download_from_semantic_scholar(papers, pdir, verbose)
    )
...
if __name__ == "__main__":
    ...
    # Download papers
    downloaded_papers = run_download_from_semantic_scholar(search_results, verbose=True)
    print(f"Downloaded papers: {downloaded_papers}")

    assert len(search_results) == num_results
    for result in search_results:
        assert all(key in result for key in ["paperId", "citation", "url", "year"])

    print("search_papers and download_from_semantic_scholar tests passed!")
