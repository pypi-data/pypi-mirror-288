...
async def main():
    ...
    async with Frame() as f:
        ...
        await f.display.show_text("Say something now...", align=Alignment.MIDDLE_CENTER)
        audio_data = await f.microphone.record_audio()
        ...
        async def on_wake_async():
            ...
            print("taking photo after waking")
            await f.camera.save_photo("frame-test-photo-2.jpg")
            print("photo saved")
        
        def on_wake():
            asyncio.get_event_loop().run_until_complete(on_wake_async())
