...
from src.frame_sdk.display import Alignment, PaletteColors
from src.frame_sdk.camera import AutofocusType, Quality
...
async def main():
    ...
    async with Frame() as f:
        ...
        length = await f.microphone.save_audio_file("test_audio.wav")
        print(f"Recorded {length:01.1f} seconds: \"./test_audio.wav\"")
        ...
        await f.files.write_file("greeting.txt", "Hello world".encode())
        ...
        await f.camera.save_photo("frame-test-photo-3.jpg", autofocus_seconds=3, quality=Quality.HIGH, autofocus_type=AutofocusType.CENTER_WEIGHTED)
        ...
        length = f.microphone.save_audio_file("test_audio.wav")
        ...
        await f.display.show_text(f"Battery {await f.get_battery_level()}%", align=Alignment.MIDDLE_CENTER)
        await f.run_lua("frame.display.text('battery: ' .. frame.battery_level() .. '%', 10, 10);frame.display.show()")
        ...
        await f.display.draw_rect_filled(640-16-batteryWidth, 40-8, batteryWidth+16, batteryHeight+16, 8, PaletteColors.WHITE, PaletteColors.VOID)
