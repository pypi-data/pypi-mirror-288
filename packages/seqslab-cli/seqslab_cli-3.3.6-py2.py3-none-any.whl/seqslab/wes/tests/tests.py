...
class CreateJobsTest(BasicTest):
    ...
    def test_add_reads_runsheet(self):
        ...
        with open(
            f"{working_dir}request-runsheet/{fixture_run_name}_AR-request.json", "r"
        ) as f:
            query_json = json.load(f)["workflow_params"]["inputs"]
        with open(
            f"{working_dir}request-runsheet/{fixture_execs_fixture_json}", "r"
        ) as f:
            ans_json = json.load(f)["inputs"]
        TestCase().assertDictEqual(query_json, ans_json)
