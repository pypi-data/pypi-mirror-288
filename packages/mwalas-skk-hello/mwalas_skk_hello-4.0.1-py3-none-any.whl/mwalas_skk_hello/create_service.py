...
def create_service(user, group):
    ...
    home_dir = Path.home()
    app_home = home_dir / f"version_control_py/wersja_1_application.txt/local"
    venv_python = app_home / "venv/bin/python"
    exec_start = app_home / "hello.py"
    log_file = app_home / "hello.log"

    # Path to the service file in the same directory as the script
    service_template_path = script_dir / "mwalas-skk-hello.service"
    ...
    shutil.copy(service_template_path, dest_file)
    ...
    content = content.replace("{{EXEC_START}}", str(exec_start))
