...
from .exceptions import UnknownChar
import re
...
class AbstractTokenizer(metaclass=AbstractSingleton):
	...
	def _order_by_length(seq: Tuple[str]) -> Tuple[str]:
	...
	def tokenize(self, string: str) -> Tuple[str]:
		...
		for match in re.findall(self._tokens_regex, string):
			...
			if match[0] != None:			#then there is a character or a cluster of them recognized as a phoneme:
