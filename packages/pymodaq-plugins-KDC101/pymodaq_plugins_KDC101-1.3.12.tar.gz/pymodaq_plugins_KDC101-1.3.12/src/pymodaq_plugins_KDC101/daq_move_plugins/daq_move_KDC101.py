...
class Kinesis:
    ...
    def __init__(self):
        self._device = None
        print("Kinesis instance created")

    def connect(self, serial: str):
        if self._device:
            print(f"Connecting to device with serial: {serial}")
    ...
    def name(self) -> str:
        if self._device:
            device_name = self._device.GetDeviceInfo().Name
    ...
    def backlash(self):
        if self._device:
            backlash_value = Decimal.ToDouble(self._device.GetBacklash())
            print(f"Current backlash: {backlash_value}")
            return backlash_value
        return 0.0

    @backlash.setter
    def backlash(self, backlash: float):
        if self._device:
            print(f"Setting backlash to: {backlash}")
