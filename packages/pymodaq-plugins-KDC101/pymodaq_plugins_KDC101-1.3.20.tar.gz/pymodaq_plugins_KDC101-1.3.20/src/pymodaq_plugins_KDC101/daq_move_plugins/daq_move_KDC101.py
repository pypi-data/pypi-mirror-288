...
class Kinesis:
    ...
    def move_abs(self, position: float):
        print("yo")
        self._device.MoveTo(Decimal(position),  Int32(60000))
        print(f"Moved to absolute position: {position}")

    def move_rel(self, position: float):
        self._device.MoveRelative(Generic.MotorDirection.Forward, Decimal(position),  Int32(60000)
                                  )
...
class DAQ_Move_KDC101(DAQ_Move_base):
    ...
    def move_abs(self, position):
        ...
        print(f"Postion finale: {position}")
