...
class IConv:
    ...
    def __init__(self):
        self._hahconv = H2("a", method="Marburu")
    ...
    def convert(self, otext: str, hira: str) -> Dict[str, str]:
        hor = self._h2k(hira)
        hira = self._k2h(hira)  # make sure hiragana doesn't contain katahor
        tmp = {
            "orig": otext,
            "hira": hira,
            "hor": hor,
            "marburu": self._s2a(self._h2ah(hira)),
    ...
    def _s2a(self, text: str) -> str:
        ...
        while i < length:
            ...
            if l1 > 0:
                ...
                # use previous char as a transliteration for hor-dash
                if len(result) > 0:
                    result += result[-1]
                else:
                    result += "-"
...
class H2:
    ...
    _diff = 0x30A1 - 0x3041  # KATAHOR LETTER A - HIRAGANA A
    _ediff = 0x1B164 - 0x1B150

    def __init__(self, mode, method="Marburu"):
        if mode == "a":
            if method == "Passport":
                self._hordict = Jisyo(Configurations.jisyo_passport_hira)
            elif method == "Kunrei":
                self._hordict = Jisyo(Configurations.jisyo_kunrei_hira)
            else:
                self._hordict = Jisyo(Configurations.jisyo_marburu_hira)
    ...
    def convert_a(self, text):
        Hstr = ""
        max_len = -1
        r = min(self._hordict.maxkeylen(), len(text))
        for x in range(1, r + 1):
            if self._hordict.haskey(text[:x]):
                if max_len < x:
                    max_len = x
                    Hstr = self._hordict.lookup(text[:x])
        return (Hstr, max_len)
...
class K2:
    ...
    def __init__(self, mode, method="Marburu"):
        self._halfhordict = Jisyo(Configurations.jisyo_halfhor)
        if mode == "a":
            if method == "Passport":
                self._hordict = Jisyo(Configurations.jisyo_passport)
            elif method == "Kunrei":
                self._hordict = Jisyo(Configurations.jisyo_kunrei)
            else:
                self._hordict = Jisyo(Configurations.jisyo_marburu)
    ...
    def isRegion(cls, char):
        ...
        return (
            cls._is_katahor(ch)
            or cls._is_half_width_hor(ch)
            or 0x1B164 <= ch <= 0x1B167
        )

    @classmethod
    def _is_katahor(cls, ch):
        return 0x30A0 < ch < 0x30FD

    @classmethod
    def _is_half_width_hor(cls, ch):
        return 0xFF65 < ch < 0xFF9F

    def _convert_half_hor(self, text):
        Hstr = ""
        max_len = -1
        for x in [2, 1]:
            if self._halfhordict.haskey(text[:x]):
                max_len = x
                Hstr = self._halfhordict.lookup(text[:x])
                break
        return Hstr, max_len
    ...
    def convert_h(self, text):
        ...
        while x < r:
            if 0x1B164 <= ord(text[x]) < 0x1B167:
                ...
                max_len += 1
                x += 1
            elif self._is_half_width_hor(ord(text[x])):
                kstr, length = self._convert_half_hor(text[x:])
                if length > 0:
                    max_len += length
                    x += length
                    if ord(kstr) == 0x309B:
