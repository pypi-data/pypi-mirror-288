...
class Mssql_Dst(object):
    ...
    def import_into_dst(self,source_data,dst_table,dst_title=None,source_title=None,batch=1000):
        ...
        if dst_title is None:
            sql_str=f"""select c.column_id,c.name,ct.name as type_name,c.max_length,c.is_identity,c.is_nullable from sys.all_columns c with(nolock)
                        ...
                        where t.name='{dst_table}'
                        and c.is_identity =0
                        order by column_id"""
            dst_title_tmp=dst_db.get_datalist(sql_str)
            dst_title=[]
            if bool(source_data):
                for title_dict in dst_title_tmp:
                    if title_dict["name"] in source_data[0]:
                        dst_title.append(title_dict["name"])

        if type(dst_title)==list:
