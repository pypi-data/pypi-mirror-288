...
class AuthSession(metaclass=Singleton):
    ...
    def __init__(self, gateway_url: Optional[str] = None) -> None:
        self.cookies = {}
        self.gateway_url = gateway_url if gateway_url is not None else _get_server_url()
    ...
    def _try_authenticate_by_credentials(self) -> bool:
        ...
        response = httpx.post(
            posixpath.join(self.gateway_url, "oauth", "login"), data={"username": login, "password": password}
        )
