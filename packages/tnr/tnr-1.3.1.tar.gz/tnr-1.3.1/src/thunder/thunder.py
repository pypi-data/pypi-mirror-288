...
def start():
    ...
    with yaspin(text="Setting up Thunder Compute instance", color="blue") as spinner:
        ...
        if not os.path.exists(os.path.expanduser("~/.thunder")):
            os.makedirs(os.path.expanduser("~/.thunder"))

        if "pem_key" in response.json():
            if os.path.exists(os.path.expanduser("~/.thunder/id_rsa")):
                os.chmod(os.path.expanduser("~/.thunder/id_rsa"), 0o600)

            with open(os.path.expanduser("~/.thunder/id_rsa"), "w") as f:
                f.write(response.json()["pem_key"])
            os.chmod(os.path.expanduser("~/.thunder/id_rsa"), 0o400)

        ip = response.json()["public_ip"]
        spinner.ok("✅")

    with yaspin(
        text="Connecting to Thunder Compute instance (this can take a few minutes)",
        color="blue",
    ) as spinner:
        ...
        ssh.connect(
            ip, username="ubuntu", key_filename=os.path.expanduser("~/.thunder/id_rsa")
        )
        spinner.ok("✅")
    ...
    with yaspin(
        ...
        if os.path.exists(os.path.expanduser("~/.thunder/dev")):
            scp.put(os.path.expanduser("~/.thunder/dev"), remote_path="~/.thunder/dev")
    ...
    subprocess.run(
        [
            f"ssh ubuntu@{ip} -o StrictHostKeyChecking=accept-new -i ~/.thunder/id_rsa -t 'pip install --upgrade tnr --quiet && cd ~/{os.path.basename(os.getcwd())} && exec /home/ubuntu/.local/bin/tnr run /bin/bash'"
        ],
        shell=True,
    )
