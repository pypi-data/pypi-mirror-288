...
def position_type_check(position_name, data):
    if not isinstance(data.get('timestamp'), str):
        logger.warning(f"{position_name}: timestamp must be a string")
    if not isinstance(data.get('next_trading_time'), str):
        logger.warning(f"{position_name}: next_trading_time must be a string")
    if not isinstance(data.get('position'), (float, int)):
        logger.warning(f"{position_name}: position must be a float or int")
    if not isinstance(data.get('allocation'), (float, int)):
        logger.warning(f"{position_name}: allocation must be a float or int")
    if not isinstance(data.get('weight'), (float, int)):
        logger.warning(f"{position_name}: weight must be a float or int")
    if not isinstance(data.get('entry_at'), str):
        logger.warning(f"{position_name}: entry_at must be a string")
    if not isinstance(data.get('exit_at'), str):
        logger.warning(f"{position_name}: exit_at must be a string")
    if not isinstance(data.get('market'), str):
        logger.warning(f"{position_name}: market must be a string")
    if not isinstance(data.get('stop'), list):
        logger.warning(f"{position_name}: stop must be a list")
    if data.get('stop_loss') is not None and not isinstance(data.get('stop_loss'), (float, int)):
        logger.warning(f"{position_name}: stop_loss must be a float or int")
    if data.get('take_profit') is not None and not isinstance(data.get('take_profit'), (float, int)):
        logger.warning(f"{position_name}: take_profit must be a float or int")
    if data.get('trail_stop') is not None and not isinstance(data.get('trail_stop'), (float, int)):
        logger.warning(f"{position_name}: trail_stop must be a float or int")
