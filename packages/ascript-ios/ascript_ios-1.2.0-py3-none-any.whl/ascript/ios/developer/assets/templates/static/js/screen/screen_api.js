...
var gpapi = {
    ...
    file: {
        ...
        path: {
            workspace: "sdcard/airscript/",
            model: "sdcard/airscript/model/",
            screen: "sdcard/airscript/screen/",
...
function api_strack_load() {
    ...
    if (gp_file) {
        ...
        var temp_gpdir = strackDir +device_id+"/"+temp_gpname
...
function api_strack_file(file){
    var path = gpapi.file.path.concat([gpapi.strack.info.path, file])
    var url =  `${baseUrl}/api/file/getpicture?path=${path}`;
...
function api_file_url(file_path) {
    return `${baseUrl}/api/file/getpicture?path=${file_path}`;
}
...
function api_module_getlist(listener) {
    $.get(baseUrl + "/api/model/getlist ", {}, function (data, textStatus) {
