...
@cache
def worker_version_id(worker_run_id: UUID) -> str | None:
    return WorkerRun.get(id=worker_run_id).worker_version
...
def get_or_create_worker_version(worker_version: dict) -> WorkerVersion:
    worker_version, _ = WorkerVersion.get_or_create(
        ...
        defaults={
            ...
            **(
                {
                    "repository_url": worker_version["revision"]["commit_url"].replace(
                        f"/commit/{worker_version['revision']['hash']}",
                        "",
                    ),
                    "revision": worker_version["revision"]["hash"],
                }
                if worker_version["revision"]
                else {}
            ),
        },
    )
...
def create_missing_metadata(
    ...
    existing_metadata = list(
        Metadata.select().where(
            ...
            Metadata.worker_version == worker_version_id(worker_run_id),
    ...
    Metadata.insert_many(
        {
            "id": str(uuid.uuid4()),
            "element": element_id,
            "worker_version": worker_version_id(worker_run_id),
