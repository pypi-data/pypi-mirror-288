...
def large_mp3_file(data_dir, small_mp3_file):
    print(f"Running: ffmpeg -f concat -safe 0 -i <(for i in {{1..200}} ; do echo 'file {small_mp3_file}'; done) {data_dir}/test_long.mp3")
    subprocess.call(["bash", "-c", f"ffmpeg -y -f concat -safe 0 -i <(for i in {{1..200}} ; do echo \"file {small_mp3_file}\"; done) -c copy {data_dir}/test_long.mp3"])
...
def test_24bitwav(small_24bit_wav_file):
    with AudioSample(small_24bit_wav_file, unit_sec=True) as au:
        nau = au.as_numpy()

def test_very_bad_mp3(very_bad_mp3_file):
    au = AudioSample(very_bad_mp3_file, unit_sec=True)
    nau = au[:].as_numpy()

def test_very_very_short_mp3(very_very_short_mp3_file):
    _ = AudioSample(very_very_short_mp3_file, unit_sec=True).as_numpy()

def test_dual_stream_file(dual_stream_mp4_file):
    with AudioSample(dual_stream_mp4_file, unit_sec=True) as au:
        assert au.streams == 2
    with AudioSample(dual_stream_mp4_file, unit_sec=True, stream_idx=0) as au:
        nau1 = au[0:3].as_numpy()
    with AudioSample(dual_stream_mp4_file, unit_sec=True, stream_idx=0) as au:
        nau1_10 = au[10.5:][:3].as_numpy()
    with AudioSample(dual_stream_mp4_file, unit_sec=True, stream_idx=1) as au:
...
def test_mem_file(small_mp3_file, small_wav_file):
    memr = io.BytesIO()
    memw = io.BytesIO()
    memr.write(open(small_mp3_file, 'rb').read())
    memr.seek(0,0)
    with AudioSample(memr, unit_sec=True) as au:
...
def test_convert_file(data_dir, short_mp3_file, small_wav_file, small_opus_file):
    ...
    with AudioSample(small_mp3_file, unit_sec=True) as au:
    ...
    with AudioSample(small_wav_file, unit_sec=True) as au:
    ...
    with AudioSample(small_opus_file, unit_sec=True) as au:
...
def test_video_mov(data_dir, video_file_with_audio_in_stream_index_1):
    with AudioSample(video_file_with_audio_in_stream_index_1, unit_sec=True) as au:
...
def test_as_tensor(data_dir, small_mp3_file):
    au = AudioSample(open(small_mp3_file, "rb"), unit_sec=True)
    tau = au.as_tensor()
...
def test_short_samples(data_dir, small_mp3_file):
    ...
    with AudioSample(small_mp3_file, unit_sec=True, force_channels=1, thread_safe=True) as au:
...
def test_various_file_types(small_m4a_file, small_ogg_file, small_wav_file, small_wav_file_with_mp4_ext, small_flac_file):
    ...
    with AudioSample(small_ogg_file, unit_sec=True) as au:
        ogg_nau = au[10:11].as_numpy()
    with AudioSample(small_m4a_file, unit_sec=True) as au:
        m4a_nau = au[10:11].as_numpy()
    with AudioSample(small_flac_file, unit_sec=True) as au:
        flac_nau = au[10:11].as_numpy()
    with AudioSample(small_wav_file_with_mp4_ext, unit_sec=True) as au:
...
def test_long_samples(large_mp3_file, data_dir):
    t1 = time.time()
    with AudioSample(large_mp3_file, unit_sec=True) as au:
    ...
    t1 = time.time()
    with AudioSample(large_mp3_file, unit_sec=True, force_channels=1) as au:
        au[500:][:1].write_sample_to_file(f"{data_dir}/test10-1-s1.mp3", no_encode=True)
    #should take less than 100ms
    assert time.time() < t1+0.1
    t1 = time.time()
    with AudioSample(large_mp3_file, unit_sec=True, force_channels=1, thread_safe=True) as au:
...
@pytest.mark.slow
def test_long_samples_slow(large_mp3_file, small_mp3_file, data_dir):

    with AudioSample(small_mp3_file, unit_sec=True, thread_safe=True) as au:
...
def test_bad_meta_data_read(bad_meta_data_mp3):
    with AudioSample(bad_meta_data_mp3, unit_sec=True) as au:
        nau = au.as_numpy()
    
def test_non_zero_start_time(non_zero_start_time_mp4):
    with AudioSample(non_zero_start_time_mp4, unit_sec=True) as au:
        nau = au.as_numpy()
