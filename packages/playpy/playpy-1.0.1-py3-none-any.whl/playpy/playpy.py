...
print(
f'''
{plpu.attrs.BOLD}PlayPy {version} (Python {version_text}, Pygame {pg.ver}){plpu.attrs.RESET}
{plpu.colors.LIGHT_YELLOW}{plpu.attrs.BOLD}Welcome to PlayPy!{plpu.colors.RESET} If you would like to share your games and ideas, join our community on Discord!
...
def init():
    ...
    if initialized:
        raise InitError('PlayPy has already been initialized')
    pg.init()
    if not audio_free:
        try:
            pg.mixer.init()
    ...
    initialized = True
    file_error_text = 'To use PlayPy, you must add the following directories to your project:\n\n'
...
def set_image_path(path: str):
    ...
    if initialized:
        raise InitError('Cannot change image path after PlayPy has been initialized')
    imagePath = path

def set_sound_path(path: str):
    global soundPath, initialized
    if initialized:
        raise InitError('Cannot change sound path after PlayPy has been initialized')
    soundPath = path

def set_font_path(path: str):
    global fontPath, initialized
    if initialized:
        raise InitError('Cannot change font path after PlayPy has been initialized')
    fontPath = path

def set_audio_free(free: bool):
    global audio_free, initialized
    if initialized:
        raise InitError('Cannot change audio free status after PlayPy has been initialized')
    audio_free = free
