...
# 判断rth标志位
def get_rth_flag(t: Optional[time] = None) -> RthFlag:
    if t == None:
        t = datetimes.now(US_EASTERN_TZ).time()

    flag: RthFlag
    if t >= RthTime.US_EQUITY_PRE_RTH_START.value and t < RthTime.US_EQUITY_PRE_RTH_END.value:
        flag = RthFlag.PRE_RTH
    elif t >= RthTime.US_EQUITY_RTH_START.value and t < RthTime.US_EQUITY_RTH_END.value:
        flag = RthFlag.RTH
    elif t >= RthTime.US_EQUITY_POST_RTH_START.value and t < RthTime.US_EQUITY_POST_RTH_END.value:
        flag = RthFlag.POST_RTH
    elif t >= RthTime.US_EQUITY_NIGHT_START.value and t <= time(23, 59, 59, 999) or t >= time(0, 0, 0, 0) and t < RthTime.US_EQUITY_NIGHT_END.value:
        flag = RthFlag.NIGHT
    return flag