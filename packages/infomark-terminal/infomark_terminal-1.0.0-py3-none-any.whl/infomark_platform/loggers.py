...
from infomark_terminal.base_helpers import Infomark_posthog
from infomark_terminal.core.log.generation.directories import get_log_dir
from infomark_terminal.core.log.generation.formatter_with_exceptions import (
    FormatterWithExceptions,
)
from infomark_terminal.core.log.generation.path_tracking_file_handler import (
    PathTrackingFileHandler,
)
from infomark_terminal.core.log.generation.settings import (
    AppSettings,
    AWSSettings,
    LogSettings,
    Settings,
)
from infomark_terminal.core.log.generation.settings_logger import get_startup
from infomark_terminal.core.log.generation.user_logger import (
    NO_USER_PLACEHOLDER,
    get_current_user,
    get_user_uuid,
)
from infomark_terminal.core.session.current_system import (
    get_current_system,
    set_system_variable,
)
...
def get_app_id() -> str:
    ...
    try:
        ...
        if e.errno == 30:
            ...
            console.print(
                "Note for macOS users: copy `Infomark Terminal` folder outside the DMG."
            )
        else:
            raise e
...
def get_branch() -> str:
    def get_branch_commit_hash(branch: str) -> str:
        response = request(
            url=f"https://api.github.com/repos/Infomark-finance/Infomark-terminal/branches/{branch}"
        )
...
class PosthogHandler(logging.Handler):
    ...
    def __init__(self, settings: Settings):
        ...
        self.disabled = Infomark_posthog.feature_enabled(
            "disable_analytics",
    ...
    def send(self, record: logging.LogRecord):
        ...
        if (
            ...
            Infomark_posthog.alias(get_user_uuid(), app_settings.identifier)

        Infomark_posthog.capture(
