# This is for helpers that do NOT import any infomark Modules
import logging
import os
...
# change this from "openbb" configurations to "infomark" configurations
infomark_posthog = Posthog(
...
def load_env_files():
    ...
    infomark_terminal modules are reloaded to refresh config files with new env,
...
def clear_infomark_env_vars(exceptions: Optional[List[str]] = None):
    """Clear infomark environment variables."""
    for v in os.environ:
        if v.startswith("infomark") and (not exceptions or v not in exceptions):
            os.environ.pop(v)