...
def parse_Infomark_script(  # noqa: PLR0911,PLR0912
    raw_lines: List[str],
    script_inputs: Optional[List[str]] = None,
) -> Tuple[str, str]:
    """
    Parse .Infomark script

    Parameters
    ----------
    raw_lines : List[str]
        Lines from .Infomark script
    ...
    str
        Processed string from .Infomark script that can be run by the Infomark Terminal
    """
    ...
    # Now we want to replace the ROUTINE_VARS to where applicable throughout the .Infomark script
    ...
    # recognized by the Infomark Terminal
    if final_lines:
