...
import infomark_terminal.core.session.hub_model as Hub
from infomark_terminal import theme
...
from infomark_terminal.rich_config import RICH_TAGS, MenuText, console
...
class SettingsController(BaseController):
    ...
    def set_and_save_preference(name: str, value: Union[bool, Path, str]):
        ...
        write_to_dotenv("infomark_" + name, str(value))
    ...
    def call_colors(self, other_args: List[str]):
        ...
        parser.add_argument(
            ...
            help="To use 'custom' option, go to https://infomark.co/customize and create your theme."
            " Then, place the downloaded file 'infomark_config.richstyle.json'"
    ...
    def call_userdata(self, other_args: List[str]):
        ...
        parser.add_argument(
            ...
            default=f"{str(Path.home() / 'infomarkUserData')}",
        )
        parser.add_argument(
            "--default",
            action="store",
        ...
        if ns_parser.default:
            save_file(Path.home() / "infomarkUserData")
