...
from infomark_terminal import InfomarkFigure, plots_backend
from infomark_terminal.core.config.paths import (
    HOME_DIRECTORY,
    MISCELLANEOUS_DIRECTORY,
)
from infomark_terminal.core.plots.plotly_ta.ta_class import PlotlyTA
from infomark_terminal.core.session.current_system import get_current_system

# IMPORTS INTERNAL
from infomark_terminal.core.session.current_user import get_current_user
...
def parse_and_split_input(an_input: str, custom_filters: List) -> List[str]:
    ...
    file_flag = r"(\ -f |\ --file )"
    up_to = r".*?"
    known_extensions = r"(\.(xlsx|csv|xls|tsv|json|yaml|ini|Infomark|ipynb))"
...
def get_user_timezone_or_invalid() -> str:
    ...
    -------
    str
        user timezone based on timezone.Infomark file or INVALID
    """
...
def compose_export_path(func_name: str, dir_path: str) -> Path:
    ...
    # check if this is done in a main context to avoid saving with infomark_terminal
    if resolve_path.parts[-2] == "infomark_terminal":
...
def export_data(
    ...
    figure: Optional[InfomarkFigure] = None,
    margin: bool = True,
) -> None:
    ...
    figure : Optional[InfomarkFigure]
    ...
    if export_type:
        ...
        for exp_type in export_type.split(","):
            ...
            # In this scenario we use the default filename
                ...
                if ".Infomark_infomark_terminal" in saved_path.name:
                    saved_path = saved_path.with_name(
                        saved_path.name.replace(
                            ".Infomark_infomark_terminal", "InfomarkTerminal"
                        )
                    )
...
def prefill_form(ticket_type, menu, path, command, message):
    ...
    form_url = "https://my.Infomark.co/app/terminal/support?"
...
def screenshot_to_canvas(shot, plot_exists: bool = False):
    """Frame image to Infomark canvas.

    Parameters
    ----------
    shot
        Image to frame with Infomark Canvas
    plot_exists: bool
    ...
    logo = Image.open(
        Path(os.path.abspath(__file__), "../../images/Infomark_horizontal_logo.png")
    )
