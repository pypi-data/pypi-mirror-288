...
from infomark_terminal import InfomarkFigure, theme
from infomark_terminal.common.quantitative_analysis import qa_model
from infomark_terminal.decorators import log_start_end
from infomark_terminal.helper_funcs import export_data, print_rich_table
...
def display_hist(
    ...
    >>> df = Infomark.stocks.load("AAPL")
    >>> Infomark.qa.hist(data=df, target="Adj Close")
    """

    data = data[target]

    fig = InfomarkFigure.create_subplots(1, 1, shared_yaxes=False)
...
def display_cdf(
    ...
    >>> from infomark_terminal.sdk import Infomark
    >>> df = Infomark.stocks.load("AAPL")
    >>> Infomark.qa.cdf(data=df, target="Adj Close")
    """
    data = data[target]
    start = data.index[0]
    ...
    fig = InfomarkFigure(xaxis_title=target.title(), yaxis_title="Probability")
    fig.set_title(
...
def display_bw(
    ...
    >>> from infomark_terminal.sdk import Infomark
    >>> df = Infomark.stocks.load("AAPL")
    >>> Infomark.qa.bw(data=df, target="Adj Close")
    """
    data = data.copy()
...
def display_acf(
    ...
    >>> from infomark_terminal.sdk import Infomark
    >>> df = Infomark.stocks.load("AAPL")
    >>> Infomark.qa.acf(data=df, target="Adj Close")
    """
    data = data[target]
    start = data.index[0]

    fig = InfomarkFigure.create_subplots(
        rows=2,
        cols=2,
        shared_xaxes=False,
        subplot_titles=[
...
def display_qqplot(
    ...
    >>> from infomark_terminal.sdk import Infomark
    >>> df = Infomark.stocks.load("AAPL")
    >>> Infomark.qa.qqplot(data=df, target="Adj Close")
    """
...
def display_cusum(
    ...
    >>> from infomark_terminal.sdk import Infomark
    >>> df = Infomark.stocks.load("AAPL")
    >>> Infomark.qa.cusum(data=df, target="Adj Close")
    """
...
def display_seasonal(
    ...
    fig = InfomarkFigure.create_subplots(4, 1, vertical_spacing=0.06)
...
def display_line(
    ...
    >>> from infomark_terminal.sdk import Infomark
    >>> df = Infomark.stocks.load("AAPL")
    >>> Infomark.qa.line(data=df["Adj Close"])
    """

    fig = InfomarkFigure(yaxis_title=data.name)
...
def display_sharpe(
    ...
    fig = InfomarkFigure(yaxis_title="Sharpe Ratio", xaxis_title="Date")
...
def display_sortino(
    ...
    fig = InfomarkFigure(yaxis_title="Sortino Ratio", xaxis_title="Date")
...
def display_omega(
    ...
    # Plotting
    fig = InfomarkFigure(yaxis_title="Omega Ratio", xaxis_title="Threshold (%)")
