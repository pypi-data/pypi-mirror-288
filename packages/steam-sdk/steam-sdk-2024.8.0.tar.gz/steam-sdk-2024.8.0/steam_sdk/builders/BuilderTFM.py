...
class BuilderTFM:
    ...
    def calculate_BS(self):
        ...
        fm = self.B_nom_center / self.ledet_options.Iref
        R = self.BS.r_BS
        R_eq = self.BS.r_BS*1.08  ##Not the actual radius but an equivalent one, Correction factor of 1.08 valid for LHC main dipole !!!
        l_mag = self.General.magnet_length
        ...
        for aperture in apertures:
            ...
            tb_2 = self.getAttribute('BS', f't_Ap{aperture}_1')
            ...
            tau_DC1_dyn = [tau_DC1_dyn[:idx_s1], [tau_DC1_dyn[idx_s1]] * (len(f) - idx_s1)]
            ...
            P_AC_1 = skinDepth_1 ** 2 / (2 * rho_Cu_Inner) * (fm * 2 * np.pi * f) ** 2 * np.pi * (skinDepth_1) * (R - tb_2 - tb_S)
            ...
            P_AC_2 = skinDepth_2 ** 2 / (2 * rho_Cu_Outer) * (fm * 2 * np.pi * f) ** 2 * np.pi * (skinDepth_2) * (R - tb_S)
            ...
            P_AC_3 = skinDepth_3 ** 3 / (2 * rho_SS) * (fm * 2 * np.pi * f) ** 2 * np.pi * R
            ...
            self.setAttribute(self.BS, f'P_{aperture}', P_tot)
            self.setAttribute(self.BS, f'I_{aperture}', I_tot)
            self.setAttribute(self.BS, f'tau_{aperture}', tau_DC1_sta)
            self.setAttribute(self.BS, f'L_{aperture}', L_ap)
            self.setAttribute(self.BS, f'R_{aperture}', R_ap)
            self.setAttribute(self.BS, f'M_{aperture}', M_ap)
...
def calculate_tau(P_tot: np.ndarray, frequency: np.ndarray, effect: str) -> int:
    ...
    if frequencies_tau[np.argmin(dPt2)]<100:
        min_tol = 1e-6
    # elif frequencies_tau[np.argmin(dPt2)]>1000:
    #     min_tol = 1e-6
    else:
        min_tol = 1e-7
