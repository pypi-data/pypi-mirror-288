...
from autopilot_tools.parameters_interface import ParametersInterface
...
class Vehicle:
    ...
    def reset_params_to_default(self):
        logger.debug("Reset parameters to default")
    ...
    def reboot(self):
        logger.debug("Reboot the autopilot")
    ...
    def run_mission(self, path: str = None, timeout: int = 100) -> MissionResult:
        return self.mission.run(path, timeout)

    def _connect_serial(self):
        logger.debug("Trying to connect to a serial port.")
    ...
    def get_autopilot_type_by_serial_devices(serial_devices: list) -> tuple:
        ...
        for serial_device in serial_devices:
            ...
            if -1 != serial_device.find(AutopilotTypes.PX4):
                ...
                break
        logger.debug(f"Autopilot {device_path} has been bound")
