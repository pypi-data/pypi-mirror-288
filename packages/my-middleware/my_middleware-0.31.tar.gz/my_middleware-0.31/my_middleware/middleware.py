...
def flask_middleware(get_request_data, respond_with_error, next_handler):
    middleware = GeneralMiddleware(get_request_data, respond_with_error, next_handler)
    
    def wrapper():
        request_data = request.get_json()  # Example: retrieve request data for Flask
        return middleware(request_data)
    
    return wrapper

def django_middleware(get_request_data, respond_with_error, next_handler):
    middleware = GeneralMiddleware(get_request_data, respond_with_error, next_handler)
    
    def wrapper(request):
        request_data = request.body  # Example: retrieve request data for Django
        if not middleware.custom_check(request_data):
            return JsonResponse({"error": "API check failed"}, status=403)
        return middleware(next_handler, request)
