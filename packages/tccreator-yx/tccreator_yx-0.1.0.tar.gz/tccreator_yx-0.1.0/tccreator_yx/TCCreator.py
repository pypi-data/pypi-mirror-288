                    ...
                    if x[f_key[8]]=="Validation" :
                        ...
                        if itemtype=="Date" or itemtype=="Time":
                             ...
                             if re.search("isValid",x[f_key[-3]]):
                               f.at[x.name,f_key[15]]=time_str_converted
                               f.at[x.name,f_key[20]]=itemformat.strip("-").strip(":").replace("DD","UNK").replace("MMM","UNK").replace("MM","UNK").replace("HH","UNK").replace("mm","UNK").replace("ss","UNK").replace("yyyy","2024")
                               #先replaceMMMM再replaceMM.如果先MM再MMM，遇到MMM后会先将其中的MM替换成UNK，导致最终结果出现UNKM
                             elif re.search("isUNK",x[f_key[-3]]):
                               f.at[x.name,f_key[15]]=itemformat.strip("-").strip(":").replace("DD","UNK").replace("MMM","UNK").replace("MM","UNK").replace("HH","UNK").replace("mm","UNK").replace("ss","UNK").replace("yyyy","2024")
                               f.at[x.name,f_key[20]]=itemformat.strip("-").strip(":").replace("DD","Blank").replace("MMM","Blank").replace("MM","Blank").replace("HH","Blank").replace("mm","Blank").replace("ss","Blank").replace("yyyy","2024")
