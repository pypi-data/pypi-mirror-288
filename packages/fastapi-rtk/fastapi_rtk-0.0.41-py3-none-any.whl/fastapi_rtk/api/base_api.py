...
class BaseApi:
    ...
    """
    Flag to check if the router is initialized.
    ...
    def integrate_router(self, app: APIRouter | None = None, log: bool = True):
        """
        Integrates the router with the FastAPI application. This method is called automatically by the toolkit.

        Args:
            app (APIRouter | None, optional): The FastAPI application. If given, the router is included in the application. Defaults to None.
            log (bool, optional): Whether to log `Registering route {self.router.prefix}{path} {methods}`. Defaults to True.
        """
        if not self._router_initialized:
            self._router_initialized = True
            priority_routes = []
            exclude_routes = [
                f"{x}_headless" for x in getattr(self, "exclude_routes", [])
            ]
            ...
            for attr_name in dir(self):
                ...
                if hasattr(attr, "_permission_name"):
                    ...
                    permission_name_with_prefix = (
                        f"{PERMISSION_PREFIX}{permission_name}"
                    )
