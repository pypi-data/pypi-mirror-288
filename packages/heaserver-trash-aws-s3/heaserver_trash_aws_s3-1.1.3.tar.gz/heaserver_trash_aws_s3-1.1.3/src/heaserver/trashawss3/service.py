...
from heaobject.root import ViewerPermissionContext
...
import logging
from typing import AsyncGenerator, AsyncIterator, Union, Awaitable
        ...
        async with S3ClientContext(request, volume_id) as s3:
            ...
            try:
                ...
                for bucket in resp_.get('Buckets', []):
                    ...
                    asyncgens.append(asyncgen(volume_id, bucket_id, request.headers.get(SUB)))
    ...
    for key, versions in itertools.groupby(non_null_versions, itemgetter('Key')):
        ...
        if version_:
            ...
            if recursive:
                result.append(item)
    if not recursive:
        result.extend(truncated_key_dict.values())
    return result
...
async def _get_deleted_items_private(s3: S3Client, volume_id: str, bucket_id: str, prefix: str | None = None,
    ...
    logger = logging.getLogger(__name__)
    loop_ = asyncio.get_running_loop()
    results: list[AWSS3FolderFileTrashItem] = []
    async def producer() -> AsyncGenerator[asyncio.Future[list[AWSS3FolderFileTrashItem]], None]:
        async for response_ in _get_version_objects(s3, bucket_id, prefix if prefix else '', loop_):
            ...
            yield loop_.run_in_executor(None, _process_resp, volume_id, bucket_id, sub_user, version, recursive, response_)
            logger.debug('Done processing version object list %s', response_)

    async def consumer(item: asyncio.Future[list[AWSS3FolderFileTrashItem]]) -> Awaitable[None]:
        logger.debug('Awaiting results')
        await item
        results.extend(item.result())
        logger.debug('Done awaiting results')
    logger.debug('Queuing')
    await queued_processing(producer, consumer)
    logger.debug('Done queueing')
