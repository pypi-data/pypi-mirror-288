...
def execute_queries(config, schemas_to_compare, db_type,project_id):
    results = []
    for section, query_file in config.items():
        ...
        with open(f"{QUERIES_FOLDER}/{query_file}", "r"):
            ...
            results.append(execute_query(query, db_type,project_id))
    return results

def execute_query(query, db_type,project_id):
    if db_type == "bigquery":
