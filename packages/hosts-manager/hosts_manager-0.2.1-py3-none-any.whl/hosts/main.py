...
class HostsFile:
    ...
    def __init__(self, hosts_file_path: str = HOST_FILE):
        ...
        if not self._check_if_markers_exist():
    ...
    def list(self, identifier: str = None, in_only_section: bool = False, sort_by: str = "ip") -> bool:
        """
        List entries from the hosts file. If in_only_section is True, only entries between the markers are returned.
...
def main():
    ...
    if args.command == "add":
        ...
        if not hosts_manager.list(identifier=args.identifier, in_only_section=args.section, sort_by=args.sort_by):
