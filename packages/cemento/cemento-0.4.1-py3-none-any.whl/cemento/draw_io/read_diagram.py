...
class ReadDiagram(Diagram):
    ...
    def __init__(self, file_path, do_check_errors=True, diagram_ref=None, inverted_rank_arrows=False, parse_terms=False):
    ...
    def _parse_info(self, term, term_parser_titles=None):
        ...
        entries = dict()
        term_breaks = [term_text.find(f"{key_word}{sep}") for key_word in term_parser_titles.values()] # first find keyword start index
        term_breaks = sorted([term for term in term_breaks if term >= 0]) # sort and remove non-matches
        ...
        for i in range(term_break_size):
            ...
            key, value = substring.split(sep, 1) # only use the first occurence from left to right
    ...
    def _parse_entity_info(self):
        ...
        for row_idx, row in rels.iterrows():
            ...
            parent_term, child_term = None, None
            ...
            for col_idx, term in enumerate(terms):
                ...
                if f"{term_parser_titles['name_title']}:" in term:
                    ...
                    term_name = entries[self.get_diagram_ref().get_term_parser_title('name_title')]
                ...
                entries['is_term'] = (cols[col_idx] == 'parent' or cols[col_idx] == 'child')
    ...
    def _check_errors(self, elements, rels, terms, extra_rels=None):
        ...
        for rel_id in rels:
            ...
            if "source" in rel_data and "target" in rel_data and rel_data["source"] == rel_data["target"]:
    ...
    def print_errors(self, sep_width=25, print_id_width=25):
        ...
        print(sep_width * '=')
        print(f"Errors for {file_name}.{file_ext}\nLocated in -> {file_path}", end=2*'\n')
        if len(errors) > 0:
...
def clean_term(term):
    soup = BeautifulSoup(term, 'html.parser')
    term_text = soup.get_text(separator='', strip=True).replace(' ', '')
    return term_text
