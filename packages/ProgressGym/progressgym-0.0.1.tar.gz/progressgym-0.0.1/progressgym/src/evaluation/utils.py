...
def regenerate_inputs() -> Data:
    ...
    result = Data('evaluation', data_type = 'sft', data_path = './src/evaluation/assets/input_alpaca.json')
...
def semantic_matching(item, four = False):
    ...
    response_template = os.path.join('src', 'evaluation', 'assets', 'data', 'response_templates')
    ...
    with open(os.path.join(response_template, q_type.split('_')[0]+'.json'), 'r') as f:
    ...
    for entry in output_data:
        ...
        if not s_id in output.keys():
            if q_type.startswith('4c') or q_type.startswith('repeat2'):
                output[s_id] = {'4c_fav':[0,0,0,0,0,0],'repeat2_fav':[0,0,0,0,0,0]}
            else:
        ...
        four = lambda x: True if x.startswith('4c') or x.startswith('repeat2') else False
...
def generate_alpaca(source: str, dir: str):
    if source == "mc" or source == "foundation":
        ...
        context_and_action = csv_to_dict_list(context, ['scenario_id', 'context', 'action1', 'action2'])
        ...
        for key, boi in context_and_action.items():
            boi_ab = {"scenario_id":boi["scenario_id"], "question_type":"ab", "context":boi['context'], "action1":boi['action1'], "action2":boi['action2']}
            boi_compare = {"scenario_id":boi["scenario_id"], "question_type":"compare", "context":boi['context'], "action1":boi['action1'], "action2":boi['action2']}
            boi_repeat = {"scenario_id":boi["scenario_id"], "question_type":"repeat", "context":boi['context'], "action1":boi['action1'], "action2":boi['action2']}
            boi_ab["instruction"] = instruction[0].strip()
            boi_ab["input"] = question[0].format(boi["context"], boi["action1"], boi["action2"]).strip()
            boi_compare["instruction"] = instruction[1].strip()
            boi_compare["input"] = question[1].format(boi["context"], boi["action1"], boi["action2"]).strip()
            boi_repeat["instruction"] = instruction[2].strip()
            boi_repeat["input"] = question[2].format(boi["context"], boi["action1"], boi["action2"]).strip()
            for _ in range(repeat):
                output_list_dic.extend([boi_ab, boi_compare, boi_repeat])
        try: 
            with open(os.path.join('src', 'evaluation', 'assets', 'input_alpaca.json'), 'r') as f:
                temp = json.load(f)
        ...
        with open(os.path.join('src', 'evaluation', 'assets', 'input_alpaca.json'), 'w') as f:
               json.dump(temp, f)
        print("done", source)
    elif source == "views":
        """
        ...
        """
        context = os.path.join(dir,'prototype.csv')
        context_and_action = csv_to_dict_list(context, ['scenario_id', 'context', 'action1', 'action2', 'action3', 'action4'])        
        ...
        output_list_dic = []
        for key, boi in context_and_action.items():
            boi_ab_f = {"scenario_id":boi["scenario_id"], "question_type":"4c_fav", "context":boi['context'], "action1":boi['action1'], "action2":boi['action2'], "action3":boi['action3'], "action4":boi['action4']}
            boi_rp_f = {"scenario_id":boi["scenario_id"], "question_type":"repeat2_fav", "context":boi['context'],"action1":boi['action1'], "action2":boi['action2'], "action3":boi['action3'], "action4":boi['action4']}
            boi_ab_f["instruction"] = instruction[0].strip()
            boi_ab_f["input"] = question[0].format(boi["context"], boi["action1"], boi["action2"], boi["action3"], boi["action4"]).strip()
            boi_rp_f["instruction"] = instruction[2].strip()
            boi_rp_f["input"] = question[2].format(boi["context"], boi["action1"], boi["action2"], boi["action3"], boi["action4"]).strip()
...
def collect_dim(output_from_collect):
    ...
    """
    look_up_dict = []
    look_up_dict.append(csv_to_dict('src/evaluation/raw_dataset/moralchoice/prototype.csv', ['scenario_id', 'generation_rule']))
    look_up_dict.append(csv_to_dict('src/evaluation/raw_dataset/foundation/prototype.csv', ['scenario_id', 'generation_theme']))
    look_up_dict.append(csv_to_dict('src/evaluation/raw_dataset/views/prototype.csv', ['scenario_id', 'generation_theme']))
    ...
    for key in output_from_collect.keys():
        if key.startswith('V'):
            ...
            if raw_results['4c_fav'][-1] + raw_results['repeat2_fav'][-1] == 0:
                continue
            likelihood = [(raw_results['4c_fav'][i] / max(raw_results['4c_fav'][-1], 1) + raw_results['repeat2_fav'][i] / max(raw_results['repeat2_fav'][-1], 1)) / 2 for i in range(4)]
            for i in range(4):
                result[str(15 + i)].append(likelihood[i])
        else:
            raw_results = output_from_collect[key]
            if  raw_results['ab'][-1] + raw_results['compare'][-1] + raw_results['repeat'][-1]== 0:
                continue
            dim = get_dim(key, look_up_dict)
            if dim == -1:
                continue
            likelihood = sum([raw_results[t][0] / max(1, raw_results[t][-1]) for t in ['ab', 'compare', 'repeat']])
            likelihood /= 3
