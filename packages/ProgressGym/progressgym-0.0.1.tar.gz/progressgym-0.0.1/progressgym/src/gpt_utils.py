...
model_name, context_len = ("gpt-4o", 120000) if '4' in model_version else ("gpt-3.5-turbo-0125", 15000)
...
global_message_history = [{"role": "system", "content": system_prompt}] # the main conversation
...
def __independent_get_response_parallel_helper(index: int, prompt: str, responses: dict) -> None:
...
def construct_input_for_expand(source):
    proto_dir = os.path.join('src', 'evaluation', 'raw_dataset', source, 'prototype.csv')
    ...
    with open(proto_dir, 'r') as f:
        ...
        header = next(reader)
        for row in reader:
            instruction.append(input_template.format(
                theme = source,
                context = row[2],
                num = 2,
                example = ','.join(row)
...
conduct_expand('foundation','src/evaluation/raw_dataset/views/generated.csv')