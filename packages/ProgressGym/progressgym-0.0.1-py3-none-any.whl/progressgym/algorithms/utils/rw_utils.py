...
def truncate_dataset(data: Data, max_samples: int) -> Data:
    ...
    if original_size <= max_samples:
        print(f"Dataset {data.data_name} already has size {original_size} less than or equal to upper limit {max_samples}. Skipping truncation.")
        return data
    ...
    print(f"Truncated dataset {data.data_name} from size {original_size} to {len(list(truncated_data.all_passages()))}.")
...
max_samples: int = (2500 if 'MAX_PREFERENCE_SAMPLES' not in os.environ else int(os.environ['MAX_PREFERENCE_SAMPLES']))
...
if use_dataset == 'values':
    ...
    def transformation(dic: Dict) -> Dict:
        assert dic['question_type'] in ['ab', 'compare', 'repeat', '4c_fav', 'repeat2_fav'], f"question type {dic['question_type']} not recognized"
        return {
            'instruction': dic['instruction'],
            'input': dic['input'],
            'output': (
                ['A', 'B'] if dic['question_type'] == 'ab' else
                ['yes', 'no'] if dic['question_type'] == 'compare' else
                [dic['action1'], dic['action2']] if dic['question_type'] == 'repeat' else
                ['A', 'B'] if dic['question_type'] == '4c_fav' else  # we can only manage two options
                [dic['action1'], dic['action2']] if dic['question_type'] == 'repeat2_fav' else
                ['', '']
            )
        }
    ...
    default_rw_data.set_key_fields(prompt_field_name='instruction', query_field_name='input', response_field_name='output')
    ...
    if not os.path.exists('../../shared_storage/downloaded_datasets/hh-rlhf/harmless-base/train_data.json'):
    ...
    default_rw_data = load_dataset('Anthropic/hh-rlhf', data_dir = "harmless-base", split = "train", ignore_verifications=True)
    default_ppo_data = Data('ppo_hh_raw', 'preference', '../../shared_storage/downloaded_datasets/hh-rlhf/harmless-base/train_data.json')
...
def get_rw_query(examinee: ExamineeBase, sample_size: int = 5000) -> Tuple[List[Dict], List[str], List[str], List[str], List[str]]:
    ...
    if isinstance(rw_data, Data):
        ...
        raise ValueError("HH-RLHF dataset is multi-round dialogue dataset, and is not suitable for our single-round QA setting.")

        random_indices = random.sample(range(len(rw_data)), min(sample_size, len(rw_data)))
    ...
    query = [
        {
            ...
            'input': query_input_template % (f'{original_instruction[i]}\n{original_input[i]}'.strip(), action1[i], action2[i]),
            'id': i
        }
...
def elicit_rw_preference(examinee: ExamineeBase, judge: JudgeBase, backend: Literal['deepspeed', 'trl'] = 'deepspeed', aligned: bool = False) -> Data:
    """Elicit preference from the judge for reward modeling."""

    print("initiate rw dataset construction")
    save_path = os.path.join('output', 'rlhf_results', f'preference_{examinee.instance_id}_{judge.instance_id}_{examinee.current_timestep}.json')
    
    query, choice1, choice2, original_instruction, original_input = get_rw_query(examinee)
    ...
    if backend == "trl":
        ...
        for i, dic in tqdm(enumerate(query)):
            ...
            try:
                ...
                print(f"Skipping sample due to error: {type(e)} {e}. Possibly due to over-sized query.")
                continue
            ...
            if 'yes' in answer.lower():
                ...
                write_log('invalid response from judge, ' + str(response) + '|| response over', log_name='rlhf')
        ...
        for i, dic in tqdm(enumerate(query_with_pred)):
            ...
            def get_status(answer: str) -> int:
                ...
                choice1_letter = 'B' if 'Response 1: """B"""' in original_instruction[i] else 'A'
                ...
                if len(choice1[i]) > 5 and len(choice2[i]) > 5:
                    ...
                    if (dist1 <= len(choice1[i])//3 and 
                        dist2 >= len(choice2[i]) - len(choice2[i])//3 and
                        dist1 + 4 <= dist2 and
                        dist1 * 2 < dist2):
                        return 1
                    
                    if (dist2 <= len(choice2[i])//3 and
                        dist1 >= len(choice1[i]) - len(choice1[i])//3 and
                        dist2 + 4 <= dist1 and
                        dist2 * 2 < dist1):
                        return 2
            ...
            if status == 1:
                ...
                if aligned:
                    rw_data.append([])
                write_log('invalid response from judge, ' + str(dic) + '|| response over', log_name='rlhf')    
    ...
    rw_data_reg = Data(f'preference_{examinee.instance_id}_{judge.instance_id}_{examinee.current_timestep}', data_type = 'preference', data_path = save_path) 
    rw_data_reg.set_key_fields(prompt_field_name='instruction', query_field_name='input', response_field_name='output')
    return rw_data_reg