...
def parse_grouped_window(f: Feature) -> Tuple[WindowConfigResolved, None] | Tuple[None, str]:
    ...
    if isinstance(has_many_parent, UnderscoreItem):
        projections, filters, error = extract_filters_and_projections(has_many_parent)
        if error is not None:
            return None, error

        if len(projections) != 0:
            return None, "projections for the group_by should appear in the `.agg` call, like `.agg(_.amount.sum())`"
    ...
    parsed_filters, error = clean_filters(joined_class, filters)
    if error is not None:
        return None, error
...
def extract_filters_and_projections(
    expr: Underscore,
) -> Tuple[List[UnderscoreAttr], List[UnderscoreBinaryOp], str | None]:
    projections: list[UnderscoreAttr] = []
    filters: list[UnderscoreBinaryOp] = []
    if not isinstance(expr, UnderscoreItem):
        return [], [], "expected a feature, like `_.amount`, or a filter, like `_.amount > 0`"
    ...
    for k in keys:
        if isinstance(k, UnderscoreBinaryOp):
            ...
            return projections, filters, "expected a feature, like `_.amount`, or a filter, like `_.amount > 0`"

    return projections, filters, None


def clean_filters(joined_class: Type[Features], filters: list[UnderscoreBinaryOp]) -> Tuple[List[Filter], str | None]:
    parsed_filters = []
    for filt in filters:
        ...
        if op not in ("==", "!=", ">", "<", ">=", "<=", "in"):
            return [], f"expected a boolean operation for the filter, like `_.amount > 0`, but found `{op}`"

        if isinstance(left, UnderscoreAttr):
            try:
                left = getattr(joined_class, left._chalk__attr)  # pyright: ignore[reportPrivateUsage]
            except Exception:
                return (
                    [],
                    f"could not find feature '{left._chalk__attr}' in the joined class",  # pyright: ignore[reportPrivateUsage]
                )

        if isinstance(right, UnderscoreAttr):
            try:
                right = getattr(joined_class, right._chalk__attr)  # pyright: ignore[reportPrivateUsage]
            except Exception:
                return (
                    [],
                    f"could not find feature '{right._chalk__attr}' in the joined class",  # pyright: ignore[reportPrivateUsage]
                )

        parsed_filter = Filter(lhs=left, operation=op, rhs=right)
        parsed_filters.append(parsed_filter)

    return parsed_filters, None
    ...
    if isinstance(getitem_expression, UnderscoreItem):
        ...
        for k in keys:
            if isinstance(k, UnderscoreBinaryOp):
                ...
                return None, "expected a feature, like `_.amount`, or a filter, like `_.amount > 0`"
