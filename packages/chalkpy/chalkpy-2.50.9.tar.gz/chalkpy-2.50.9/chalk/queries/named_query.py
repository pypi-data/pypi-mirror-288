...
class NamedQuery:
    def __init__(
        ...
        *,
        ...
        if (name, version) in NAMED_QUERY_REGISTRY:
            raise ValueError(
                f"Named query with name {name} and version {version} already exists. Cron names must be unique."
            )
        ...
        self.input = input and [str(f) for f in input]
        self.output = output and [str(f) for f in output]
        self.tags = tags
        self.filename = caller_filename
        self.description = description
        self.owner = owner
        self.meta = meta
        self.staleness = staleness and {str(k): v for k, v in staleness.items()}
        self.planner_options = planner_options and {k: str(v) for k, v in planner_options.items()}
