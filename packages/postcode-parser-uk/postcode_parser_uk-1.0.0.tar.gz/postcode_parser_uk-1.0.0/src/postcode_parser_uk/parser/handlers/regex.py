...
class RegexParsingHandler(ParsingHandler):
    ...
    def __init__(self) -> None:
        self._bs7666_patterns = [
            r"^[A-Z]{1,2}[0-9R][0-9A-Z]? ?[0-9][ABDEFGHJLNPQRSTUWXYZ]{2}$",
            r"^BFPO ?[0-9]{1,4}$",
            r"^([AC-FHKNPRTV-Y]\d{2}|D6W)? ?[0-9AC-FHKNPRTV-Y]{4}$",
        ]
    ...
    def _handle_standard_case(self, match: re.Match) -> Result[UKPostCode]:
        ...
        return Result.success(
            value=UKPostCode(
                area=match.group(0)[0],
                district=match.group(0)[1:3].strip(),
                sector=match.group(0)[-3],
