...
class BaseDatasetDirectoryManager(ABC, Generic[TDatasetJson]):
    ...
    def load_dataset_json(self, *, validate: bool = False) -> TDatasetJson:
        ...
        dataset_json_data = (
            self._load_dataset_json_data_from_file(dataset_json_path, parser=parser)
            if self.revision_id is None
            else self._load_dataset_json_data_from_git(dataset_json_path, parser=parser)
        )
    ...
    def _load_dataset_json_data_from_file(self, dataset_json_path: Path, *, parser: JsonParser) -> Json:
        if self.revision_id is not None:
            raise LatestRevisionOnlyOperation(revision_id=self.revision_id)

        try:
            return parser.parse_file(dataset_json_path)
    ...
    def _load_dataset_json_data_from_git(self, dataset_json_path: Path, *, parser: JsonParser) -> Json:
        ...
        try:
            return self._git_provider_directory_manager.load_json_file(
                dataset_json_path, parser=parser, revision_id=self.revision_id
            )
