...
def pebble_count(photo_file, obj_color='yellow', obj_size_mm=190.5, min_area=50):        
    ...
    bbox = grain_data.loc[[1], ['bbox-1','bbox-0','bbox-3','bbox-2']].apply(lambda x: [i for i in x],axis=1).values[0]
    ...
    gpd.GeoDataFrame([],geometry=[box(*bbox)]).plot(ax=ax[0],ec='r',fc='None',lw=2)
