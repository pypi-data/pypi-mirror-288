...
def pebble_count(photo_file, obj_color='yellow', obj_size_mm=190.5, min_area=50):        
    ...
    bbox = grain_data.loc[[1], [col for col in grain_data.columns if 'bbox' in col]].apply(lambda x: [i for i in x],axis=1).values[0]
    ...
    gpd.GeoDataFrame([],geometry=[box(*bbox)]).plot(ax=ax[0],color='r',lw=2)
