...
def photon_calib(dim,wv):
    ...
    for wv in range(len(wv_step)):
        # print(wv)
        Wavelength = np.zeros([x_det,y_det],dtype = object)
        Time = np.zeros([x_det,y_det],dtype = object)
        # detect = np.zeros([x_det,y_det],dtype = object)

        for i in range(x_det):
            for j in range(y_det):
                
                Wavelength[i,j] = np.ones(shape=2000) * wv_step[wv]
                # step = np.array(range(1,2000))
                # Wavelength[i,j][step * 500] = wv_step[wv]
                Time[i,j] = np.linspace(0+500,1e6-500,2000, dtype = int)
                # detect[i,j] = [Wavelength[i,j],Time[i,j]]
