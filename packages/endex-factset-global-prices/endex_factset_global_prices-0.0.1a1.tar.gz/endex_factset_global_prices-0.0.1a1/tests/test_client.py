...
from endex_factset_global_prices._exceptions import (
    APIStatusError,
    APITimeoutError,
    APIResponseValidationError,
    EndexFactsetGlobalPricesError,
)
...
class TestEndexFactsetGlobalPrices:
    ...
    def test_validate_headers(self) -> None:
        ...
        with pytest.raises(EndexFactsetGlobalPricesError):
            client2 = EndexFactsetGlobalPrices(
    ...
    def test_base_url_env(self) -> None:
        with update_env(ENDEX_FACTSET_GLOBAL_PRICES_BASE_URL="http://localhost:5000/from/env"):
            client = EndexFactsetGlobalPrices(username=username, password=password, _strict_response_validation=True)
...
class TestAsyncEndexFactsetGlobalPrices:
    ...
    def test_base_url_env(self) -> None:
        with update_env(ENDEX_FACTSET_GLOBAL_PRICES_BASE_URL="http://localhost:5000/from/env"):
            client = AsyncEndexFactsetGlobalPrices(
                username=username, password=password, _strict_response_validation=True
            )
            assert client.base_url == "http://localhost:5000/from/env/"
