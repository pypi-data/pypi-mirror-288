...
class PriceListParams(TypedDict, total=False):
    ...
    request for a single day and 50 per multi-day request. The number of unique
    currencies present in the requested ids is limited to 50 per request._</p>
    _<p>Make note, GET Method URL request lines are also limited to a total length
    of 8192 bytes (8KB). In cases where the service allows for thousands of ids,
    which may lead to exceeding this request line limit of 8KB, it's advised for any
