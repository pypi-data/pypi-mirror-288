    ...
    for index, (site_id, local_image_path, food_item,local_image_folder,image_dict_name,s3_image_path,reference_image,reference_image_gt,rework_g,model_final_name,
        ...
        if annotation_data_res:
            ...
            if s3_image_path is np.nan:
                ...
                main_image_dict['bb_path'] = None
                main_image_dict['mask_path'] = None
            ref_image_dict = {}
            if reference_image is np.nan:
                if logger:
                    logger.error(f"No reference image found for image {s3_image_path}. Skipping annotation.")
                ref_image_dict['bb_path'] = None
                ref_image_dict['mask_path'] = None
            else:
            ...
            try:
                ...
                if logger:
                    logger.error(f"Error downloading annotation data for ref image {reference_image}")
