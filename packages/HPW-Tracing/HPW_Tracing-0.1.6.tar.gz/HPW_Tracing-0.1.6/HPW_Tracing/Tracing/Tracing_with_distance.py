...
def tracing_with_node(graph: nx.Graph, dict: pd.DataFrame, start_node, direction: str, distance: int = None) -> Tuple[Dict[str, float], pd.DataFrame]:
    ...
    try:
        ...
        print('start searching the temp nodes')
        if start_node in graph.nodes():
            print('node is in the temp nodes')
            pass
        else:
            raise ValueError(f'Node {start_node} not found in the GIS data')
    ...
    # if UFID is not in the graph.edges, mark it as "non"
    if not all('UFID' in graph.edges[edge] for edge in bfs_edges):
        for edge in bfs_edges:
            if 'UFID' not in graph.edges[edge]:
                graph.edges[edge]['UFID'] = 'non'
