...
# ---------------------------------------------------------------------------------------------------------------------------------------
    def __init__(self):   
        if UHuggingfaceChatApi.__instance != None:
            ...
            self.mapModel = {}
            self.packageName:str = "evo_package_huggingface"
            self.mapAssistantHF = {}
# ---------------------------------------------------------------------------------------------------------------------------------------
        ...
        try:
            ...
            if self.isUseDefault:
                ...
                for model in arrayModel:
                    ...
                    eChatModel.doGenerateID(f"{self.packageName}_{model}", isHash=True)
                    eChatModel.name = model
...
#<        
            ...
            for resp in clientChat.chat(
                    ...
                    if resp is None:
                        ...
                        message = resp["token"]
                    eApiText =  await cAssistantCallback.doParser(message) #self.__doGetEApiTextEvent(eOpenaiInfo, event)
        ...
        try:
            ...
            if len(self.mapModel) == 0:
                ...
                for model in arrayModel:
                    ...
                    eChatModel.name = model.displayName
                    eChatModel.typeInput = 0 
                    eChatModel.apiPackage = self.packageName
                    eChatModel.callback = self
                    eChatMapModel.mapEChatModel.doSet(eChatModel)
