...
class Unit_Test_Generator:
    ...
    def initialize_unit_test_generator(self):
        ...
        self.unit_test_cap = self.config["unit_test_cap"] if "unit_test_cap" in self.config else None

        self.unit_test_generator = Model(config=self.config)
    ...
    def generate_unit_tests(self, init_input: list, temperature=None):
        ...
        if self.unit_test_cap is not None and self.unit_test_cap >= 1:
            user_prompt = [f"Given the following query, generate a set of {self.unit_test_cap} unit tests that would evaluate the correctness of responses to this query.\n"]
        else:
            user_prompt = [f"Given the following query, generate a set of unit tests that would evaluate the correctness of responses to this query.\n"]
        ...
        for retry in range(10):
            try:
                ...
                unit_tests = unit_tests[:self.unit_test_cap] if self.unit_test_cap is not None else unit_tests
                ...
                print(f"Problematic unit tests: {output[0]}")
    ...
    def parse_unit_tests_output(self, output):
        ...
        """
        #pdb.set_trace()
        if isinstance(output, list) and len(output) > 0 and [isinstance(test, str) for test in output]:
            return output
        else:
            ...
            #pdb.set_trace()
            assert(isinstance(unit_tests, list) and len(unit_tests) > 0 and [isinstance(test, str) for test in unit_tests])
