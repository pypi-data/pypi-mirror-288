...
from arena_hard_auto_utils import (
    ...
    generate_candidates_with_archon_using_together_api,
...
def get_answer(
    ...
    pipeline=None, generation_config=None, archon=None
):
        ...
        for j in range(len(question["turns"])):
            ...
            if api_type == "anthropic":
                ...
                output = chat_completion_together_ai(model=endpoint_info["model_name"],
                                                     ...
                                                     max_tokens=max_tokens)
            elif api_type == "archon":
                #output = generate_candidates_with_archon_using_together_api(messages=conv,
                #                                                            temperature=endpoint_info["temperature"],
                #                                                            max_new_tokens=max_tokens,
                #                                                            archon_sample_models=endpoint_info["archon_sample_models"],
                #                                                            archon_sample_count=endpoint_info["archon_sample_count"],
                #                                                            archon_fusion_model=endpoint_info["archon_fusion_model"])
                assert len(conv) == 1
                output = archon.generate(conv, temperature=temperature)
...
if __name__ == "__main__":
    ...
    for model in settings["model_list"]:
        ...
        if endpoint_info["api_type"] == "archon":
            with open(endpoint_info["config"], 'r') as file:
                loaded_config = json.load(file)
            archon = Archon(loaded_config)
        else:
            archon = None
