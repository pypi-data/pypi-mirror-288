...
from archon.arena_hard_auto_utils import (
    load_questions,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--setting-file", type=str, default="arena_hard_auto/judge_config.yaml"
    )
    parser.add_argument(
        "--endpoint-file", type=str, default="arena_hard_auto/api_config.yaml"
    )
    ...
    question_file = os.path.join(configs["bench_name"], "arena-questions.jsonl")
    answer_dir = os.path.join("outputs", configs["bench_name"])
    ...
    output_files = {}
    output_dir = (
        f"outputs/{configs['bench_name']}/model_judgment/{configs['judge_model']}"
    )
    for model in models:
        ...
        for model in models:
            ...
            for question in questions:
                ...
                if model in model_to_full_name:
                    kwargs["answer"] = model_answers[model_to_full_name[model]][
                        question_id
                    ]
                else:
