...
from heaobject.root import is_heaobject_dict, is_heaobject_dict_list, is_primitive_list, is_primitive, DesktopObjectDict, are_permissions_read_only
...
def _get_template(sub: str, wstl_actions: list[dict[str, Any]], tvars: dict[str, Any], data: Sequence[DesktopObjectDict] | None = None, attribute_permissions: Sequence[dict[str, Sequence[str]]] = None) -> dict:
    """
    ...
    for wstl_action in wstl_actions:
        if _is_in_target('cj-template', wstl_action):
            ...
            cj_template['data'] = []
            data_zero = data[0] if len(data) > 0 else None
            attribute_permissions_ = attribute_permissions[0] if len(attribute_permissions) > 0 else {}
