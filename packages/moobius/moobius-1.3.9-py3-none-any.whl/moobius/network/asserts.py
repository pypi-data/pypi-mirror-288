...
def _button_arg_assert(ba, base_message, path):
    ...
    if not ba.get('values'): # String means no need to value.
        if ba.get('type') == 'string':
            ba = ba.copy()
            ba['values'] = ['nooone']
    ...
    if subty=='update_characters':
        ...
        def _each_button(x, base_message, the_path):
            each_btn = {'button_id':'pressA', 'button_name':'press this A',
                        'new_window':True, 'arguments':[_button_arg_assert], 'bottom_buttons':[{'text':'txt', 'type':'confirm'}]}
