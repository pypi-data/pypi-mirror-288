...
class Moobius:
    ...
    async def download(self, source, fullpath=None, auto_dir=None, overwrite=True, bytes=False, headers=None): """Calls self.http_api.download."""; return await self.http_api.download(source, fullpath, auto_dir, overwrite, bytes, headers)
    ...
    async def send_update_context_menu(self, menu_elements, channel_id, recipients): """Calls self.ws_client.update_context_menu using self.client_id. Converts recipients to a group_id if a list."""; return await self.ws_client.update_context_menu(self.client_id, menu_elements, channel_id, await self._update_rec(recipients, True))
    async def send_update_style(self, style_content, channel_id, recipients): """Calls self.ws_client.update_style using self.client_id. Converts recipients to a group_id if a list."""; return await self.ws_client.update_style(style_content, self.client_id, channel_id, await self._update_rec(recipients, True))
