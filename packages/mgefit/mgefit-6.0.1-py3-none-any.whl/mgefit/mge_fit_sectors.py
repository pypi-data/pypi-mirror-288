...
class mge_fit_sectors:
    ...
    def __init__(self, radius, angle, counts, eps,
        ...
        if linear:
            ...
            mp = capfit(self._fitfunc, pars, bounds=bounds.T, abs_step=0.01, tied=tied,
...
#----------------------------------------------------------------------------
    ...
    def _linear_fit(self, sol, chi2Best, lrbounds, qbounds, ngauss):
        ...
        while True:
            ...
            mp = capfit(self._fitfunc, sol.ravel(), bounds=bounds.T, abs_step=0.01)
            sol = mp.x

            if 2*mp.cost < chi2Best:
                chi2Best = 2*mp.cost
