...
class SerialNumberGenerator:
    ...
    def __init__(self, type:str, gcp:str, indicator:str,count:str, length:int, sequence_start:str = "0",charset:str = "0", prefix:str = "0", suffix:str = "0", sparseness:str = "0", pack_level:str = "0", item_ref:str = "0") -> None:
        ...
        self.type = type
        if self.type not in ["Random", "Sequential"]:
...
class SSCCGenerator(SerialNumberGenerator):
    def __init__(self, type:str, gcp:str, last_sscc:str|None, count:str, extension_digit:str, length:int=17):
    ...
    @override
    def generate_sequential_numeric_nos(self) -> Dict:
        if self.last_sscc is None:
            ...
            start = int(self.last_sscc[:-1])+1
            end = start+int(self.count)
            nums = range(start, end)
        
        ssccList = [str(x)+self.__get_check_digit(self.extension_digit+self.gcp+str(x)) for x in nums]
